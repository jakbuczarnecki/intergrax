...
resolve_profiles() {
  case "$PROFILE" in
    all)
      add_profiles core queue rag data secrets observability cloud heavy vllm all
      ;;
    core|queue|rag|data|secrets|observability|cloud|heavy|vllm)
    ...
    *)
      ...
      echo "Profiles: default, minimal, core, queue, rag, data, secrets, observability, cloud, heavy, vllm, p6, all"
      exit 1
      ;;
  esac
}
