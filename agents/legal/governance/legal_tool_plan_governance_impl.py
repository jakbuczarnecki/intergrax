...
1. Build your :class:`~legal.config.legal_agent_config.LegalAgentConfig`.
2. Set ``legal_tool_plan_governance`` to one of:
   ...
   * Your own subclass of :class:`~legal.governance.legal_tool_plan_governance_port.LegalToolPlanGovernancePort`
     for production policy.
   * :class:`~legal.governance.legal_platform_policy_governance.ResolvingLegalToolPlanGovernance`
     with a :class:`~legal.governance.legal_platform_policy_governance.LegalExecutionPolicyPort`
     implementation (platform factory), or :class:`~legal.governance.legal_platform_policy_governance.DualLegalGovernanceService`
     for post-run + pre-bridge in one object, or :func:`~legal.governance.legal_agent_governance_wiring.with_dual_legal_governance`.
...
3. Run the agent via :class:`~intergrax.agents.agent_engine.AgentEngine` as usual.
   :func:`~legal.pipeline.legal_execution_loop.run_legal_dynamic_execution_loop`
   calls ``adjust_legal_tool_plan`` automatically when the field is not ``None``.

**What they must do:** Return a :class:`~legal.domain.legal_tool_plan.LegalToolPlan`
...
from legal.governance.legal_tool_plan_governance_port import LegalToolPlanGovernancePort
