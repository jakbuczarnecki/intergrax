...
class _DisputeScenarioLLMStub(LLMAdapter):
    provider = "dispute_scenario"
    model = "dispute_scenario-stub"
    ...
    def generate_messages(
        ...
        *,
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(content=f"dispute_scenario: {content[:200]}")
        return build_adapter_response(content="dispute_scenario: (empty)")


class _DisputeScenarioPipeline(RuntimePipeline):
    async def _inner_run(self, state: RuntimeState) -> RuntimeAnswer:
        ...
        answer = f"dispute_scenario: {message}"
        if state.runtime_answer is not None:
            state.runtime_answer.answer = answer
        if state.runtime_answer is None:
            raise RuntimeError("dispute_scenario pipeline did not produce runtime_answer.")
...
class PipelineBundle:
    ...
    pipeline: _DisputeScenarioPipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_DisputeScenarioLLMStub(),
        pipeline=_DisputeScenarioPipeline(),
    )
