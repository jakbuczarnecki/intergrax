...
class _IntergraxAssistantLLMStub(LLMAdapter):
    provider = "intergrax_assistant"
    model = "intergrax_assistant-stub"
    ...
    def generate_messages(
        ...
        *,
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(content=f"intergrax_assistant: {content[:200]}")
        return build_adapter_response(content="intergrax_assistant: (empty)")


class _IntergraxAssistantPipeline(RuntimePipeline):
    async def _inner_run(self, state: RuntimeState) -> RuntimeAnswer:
        ...
        answer = f"intergrax_assistant: {message}"
        if state.runtime_answer is not None:
            state.runtime_answer.answer = answer
        if state.runtime_answer is None:
            raise RuntimeError("intergrax_assistant pipeline did not produce runtime_answer.")
...
class PipelineBundle:
    ...
    pipeline: _IntergraxAssistantPipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_IntergraxAssistantLLMStub(),
        pipeline=_IntergraxAssistantPipeline(),
    )
