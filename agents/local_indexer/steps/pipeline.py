...
class _LocalIndexerLLMStub(LLMAdapter):
    provider = "local_indexer"
    model = "local_indexer-stub"
    ...
    def generate_messages(
        ...
        *,
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(content=f"local_indexer: {content[:200]}")
        return build_adapter_response(content="local_indexer: (empty)")


class _LocalIndexerPipeline(RuntimePipeline):
    async def _inner_run(self, state: RuntimeState) -> RuntimeAnswer:
        ...
        answer = f"local_indexer: {message}"
        if state.runtime_answer is not None:
            state.runtime_answer.answer = answer
        if state.runtime_answer is None:
            raise RuntimeError("local_indexer pipeline did not produce runtime_answer.")
...
class PipelineBundle:
    ...
    pipeline: _LocalIndexerPipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_LocalIndexerLLMStub(),
        pipeline=_LocalIndexerPipeline(),
    )
