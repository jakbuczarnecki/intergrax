...
class DisputeIntakeAgent(ReflexAgent):
    """DSW intake agent — typed Reflex pattern (ACP-MIG-4)."""

    contract_id = "dispute_intake"
    capabilities = tuple(CAPABILITIES)
    cognitive_pattern = CognitivePattern.REFLEX
    main_step_id = "dispute_intake_step"
    ...
    def build_context(self, request: RuntimeRequest) -> RuntimeContext:
        return build_agent_runtime_context(request, PrefixStubLLMAdapter(prefix="dispute_intake"))
    ...
    async def act(self, step_ctx: AgentStepContext, reasoning):
        return prefixed_act_output(prefix="dispute_intake", step_ctx=step_ctx, reasoning=reasoning)

    def evaluate(self, step_ctx: AgentStepContext, output: dict[str, object]):
        return evaluate_complete(step_ctx, output, reason="dispute_intake_goal_met")
