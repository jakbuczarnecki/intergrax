...
class _DisputeIntakeLLMStub(LLMAdapter):
    provider = "dispute_intake"
    model = "dispute_intake-stub"
    ...
    def generate_messages(
        ...
        *,
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(content=f"dispute_intake: {content[:200]}")
        return build_adapter_response(content="dispute_intake: (empty)")


class _DisputeIntakePipeline(RuntimePipeline):
    async def _inner_run(self, state: RuntimeState) -> RuntimeAnswer:
        ...
        answer = f"dispute_intake: {message}"
        if state.runtime_answer is not None:
            state.runtime_answer.answer = answer
        if state.runtime_answer is None:
            raise RuntimeError("dispute_intake pipeline did not produce runtime_answer.")
...
class PipelineBundle:
    ...
    pipeline: _DisputeIntakePipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_DisputeIntakeLLMStub(),
        pipeline=_DisputeIntakePipeline(),
    )
