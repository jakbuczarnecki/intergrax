...
class _VendorDiscoveryLLMStub(LLMAdapter):
    provider = "vendor_discovery"
    model = "vendor_discovery-stub"
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(
                    content=f"vendor_discovery: {content[:200]}"
                )
        return build_adapter_response(content="vendor_discovery: (empty)")


class _VendorDiscoveryPipeline(RuntimePipeline):
...
class PipelineBundle:
    ...
    pipeline: _VendorDiscoveryPipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_VendorDiscoveryLLMStub(),
        pipeline=_VendorDiscoveryPipeline(),
    )
