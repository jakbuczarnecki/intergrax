...
class _DisputeStrategistLLMStub(LLMAdapter):
    provider = "dispute_strategist"
    model = "dispute_strategist-stub"
    ...
    def generate_messages(
        ...
        *,
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(content=f"dispute_strategist: {content[:200]}")
        return build_adapter_response(content="dispute_strategist: (empty)")


class _DisputeStrategistPipeline(RuntimePipeline):
    async def _inner_run(self, state: RuntimeState) -> RuntimeAnswer:
        ...
        answer = f"dispute_strategist: {message}"
        if state.runtime_answer is not None:
            state.runtime_answer.answer = answer
        if state.runtime_answer is None:
            raise RuntimeError("dispute_strategist pipeline did not produce runtime_answer.")
...
class PipelineBundle:
    ...
    pipeline: _DisputeStrategistPipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_DisputeStrategistLLMStub(),
        pipeline=_DisputeStrategistPipeline(),
    )
