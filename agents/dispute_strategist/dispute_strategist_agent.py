...
class DisputeStrategistAgent(ReflexAgent):
    """DSW strategist agent — typed Reflex pattern (ACP-MIG-4)."""

    contract_id = "dispute_strategist"
    capabilities = tuple(CAPABILITIES)
    cognitive_pattern = CognitivePattern.REFLEX
    main_step_id = "dispute_strategist_step"
    ...
    async def act(self, step_ctx: AgentStepContext, reasoning):
        return prefixed_act_output(prefix="dispute_strategist", step_ctx=step_ctx, reasoning=reasoning)

    def evaluate(self, step_ctx: AgentStepContext, output: dict[str, object]):
        return evaluate_complete(step_ctx, output, reason="dispute_strategist_goal_met")
