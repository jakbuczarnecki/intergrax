...
"""GR-7-A2-R1/R2 — effective ExternalEffectContract from provider capabilities."""
...
def test_create_reconciliation_not_supported_even_with_status_polling() -> None:
    contract = external_work_effect_contract_for_action(
        ACTION_CREATE_EXTERNAL_WORK,
        _caps(supports_status_polling=True, supports_idempotency=True),
    )
    assert contract.safety.reconciliation is ExternalEffectCapabilitySupport.NOT_SUPPORTED
    assert contract.reconciliation_probe_refs == ()


def test_create_idempotency_and_reconciliation_are_independent() -> None:
    contract = external_work_effect_contract_for_action(
        ACTION_CREATE_EXTERNAL_WORK,
        _caps(supports_idempotency=True, supports_status_polling=True),
    )
    assert contract.safety.idempotency is ExternalEffectCapabilitySupport.SUPPORTED
    assert contract.safety.reconciliation is ExternalEffectCapabilitySupport.NOT_SUPPORTED


def test_accept_reconciliation_requires_status_polling() -> None:
    supported = external_work_effect_contract_for_action(
        ACTION_ACCEPT_QUOTE,
    ...
    assert supported.reconciliation_probe_refs == ("get_work",)
...
def test_cancel_reconciliation_requires_status_polling() -> None:
    supported = external_work_effect_contract_for_action(
        ACTION_CANCEL_EXTERNAL_WORK,
        _caps(supports_status_polling=True),
    )
...
def test_create_unknown_posture_escalate_when_no_idempotency_despite_status_polling() -> None:
    contract = external_work_effect_contract_for_action(
        ACTION_CREATE_EXTERNAL_WORK,
        _caps(supports_idempotency=False, supports_status_polling=True),
    )
    assert (
...
def test_create_unknown_posture_idempotent_repeat_without_reconciliation() -> None:
    contract = external_work_effect_contract_for_action(
        ACTION_CREATE_EXTERNAL_WORK,
        _caps(supports_idempotency=True, supports_status_polling=True),
    )
