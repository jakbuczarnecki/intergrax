# © Artur Czarnecki. All rights reserved.

from __future__ import annotations

from intergrax.runtime.nexus.agents.acp_stub_reflex import (
    build_agent_runtime_context,
    evaluate_complete,
    perceive_run_input,
    reason_passthrough,
)
from intergrax.agents.authoring.patterns.diagnostic_reflex import DiagnosticReflexAgent
from intergrax.contracts.agent_run import AgentRunResult
from intergrax.contracts.agent_run_enums import CognitivePattern
from intergrax.contracts.agent_step_context import AgentStepContext
from intergrax.contracts.capability import CapabilityMatchResult
from intergrax.llm_adapters.contracts.llm_adapter import LLMAdapter
from intergrax.runtime.nexus.engine.runtime_context import RuntimeContext
from intergrax.runtime.nexus.responses.response_schema import RuntimeRequest
from intergrax.runtime.nexus.tracing.trace_models import DiagnosticPayload
from intergrax.contracts.task_envelope import TaskEnvelope, routing_capability_from_envelope
from tool_selection_qualifier.capabilities import CAPABILITIES
from tool_selection_qualifier.contract import build_agent_contract
from tool_selection_qualifier.diagnostics import tool_selection_diagnostic_from_output
from tool_selection_qualifier.steps.tool_selection_job import run_tool_selection_job


class ToolSelectionQualifierAgent(DiagnosticReflexAgent):
    """LKW qualification agent for real multi-tool LLM selection (DIAG-FUNCTIONAL-Q2)."""

    contract_id = "tool_selection_qualifier"
    capabilities = tuple(CAPABILITIES)
    cognitive_pattern = CognitivePattern.REFLEX
    main_step_id = "tool_selection_qualifier_step"

    def __init__(self, llm_adapter: LLMAdapter) -> None:
        self._llm_adapter = llm_adapter

    def get_contract(self):
        return build_agent_contract()

    def can_handle(self, task: TaskEnvelope) -> CapabilityMatchResult:
        capability = routing_capability_from_envelope(task)
        supported = set(CAPABILITIES)
        if capability is None or capability in supported:
            return CapabilityMatchResult(
                matched=True,
                agent_id=self.contract_id,
                matched_capabilities=list(supported),
                score=1.0,
                rationale="tool selection qualification capability",
            )
        return CapabilityMatchResult(matched=False, rationale="capability not supported")

    def build_context(self, request: RuntimeRequest) -> RuntimeContext:
        return build_agent_runtime_context(request, self._llm_adapter)

    async def perceive(self, step_ctx: AgentStepContext):
        return perceive_run_input(step_ctx, self)

    async def reason(self, step_ctx: AgentStepContext, observation):
        return reason_passthrough(step_ctx, observation)

    async def act(self, step_ctx: AgentStepContext, reasoning):
        _ = reasoning
        return await run_tool_selection_job(step_ctx)

    def evaluate(self, step_ctx: AgentStepContext, output: dict[str, object]):
        return evaluate_complete(step_ctx, output, reason="tool_selection_qualification_complete")

    def build_diagnostic_payloads(self, output: dict[str, object]) -> list[DiagnosticPayload]:
        return [tool_selection_diagnostic_from_output(output)]

    async def on_run_end(self, result: AgentRunResult) -> None:
        output = result.output
        if not isinstance(output, dict):
            return
        summary = output.get("tool_selection_summary")
        if isinstance(summary, dict) and summary:
            result.structured_data["tool_selection_summary"] = dict(summary)
