...
class _DisputeAnalystLLMStub(LLMAdapter):
    provider = "dispute_analyst"
    model = "dispute_analyst-stub"
    ...
    def generate_messages(
        ...
        *,
        ...
        for msg in reversed(messages):
            ...
            if content:
                return build_adapter_response(content=f"dispute_analyst: {content[:200]}")
        return build_adapter_response(content="dispute_analyst: (empty)")


class _DisputeAnalystPipeline(RuntimePipeline):
    async def _inner_run(self, state: RuntimeState) -> RuntimeAnswer:
        ...
        answer = f"dispute_analyst: {message}"
        if state.runtime_answer is not None:
            state.runtime_answer.answer = answer
        if state.runtime_answer is None:
            raise RuntimeError("dispute_analyst pipeline did not produce runtime_answer.")
...
class PipelineBundle:
    ...
    pipeline: _DisputeAnalystPipeline


def build_pipeline() -> PipelineBundle:
    return PipelineBundle(
        llm_adapter=_DisputeAnalystLLMStub(),
        pipeline=_DisputeAnalystPipeline(),
    )
