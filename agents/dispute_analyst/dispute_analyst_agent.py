...
class DisputeAnalystAgent(ReflexAgent):
    """DSW analyst agent — typed Reflex pattern (ACP-MIG-4)."""

    contract_id = "dispute_analyst"
    capabilities = tuple(CAPABILITIES)
    cognitive_pattern = CognitivePattern.REFLEX
    main_step_id = "dispute_analyst_step"
    ...
    def build_context(self, request: RuntimeRequest) -> RuntimeContext:
        return build_agent_runtime_context(request, PrefixStubLLMAdapter(prefix="dispute_analyst"))
    ...
    async def act(self, step_ctx: AgentStepContext, reasoning):
        return prefixed_act_output(prefix="dispute_analyst", step_ctx=step_ctx, reasoning=reasoning)

    def evaluate(self, step_ctx: AgentStepContext, output: dict[str, object]):
        return evaluate_complete(step_ctx, output, reason="dispute_analyst_goal_met")
