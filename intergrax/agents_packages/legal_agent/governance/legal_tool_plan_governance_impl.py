...
1. Build your :class:`~intergrax.agents_packages.legal_agent.config.legal_agent_config.LegalAgentConfig`.
2. Set ``legal_tool_plan_governance`` to one of:
   ...
   * Your own subclass of :class:`~intergrax.agents_packages.legal_agent.governance.legal_tool_plan_governance_port.LegalToolPlanGovernancePort`
     for production policy.
   * :class:`~intergrax.agents_packages.legal_agent.governance.legal_platform_policy_governance.ResolvingLegalToolPlanGovernance`
     with a :class:`~intergrax.agents_packages.legal_agent.governance.legal_platform_policy_governance.LegalExecutionPolicyPort`
     implementation (platform factory), or :class:`~intergrax.agents_packages.legal_agent.governance.legal_platform_policy_governance.DualLegalGovernanceService`
     for post-run + pre-bridge in one object, or :func:`~intergrax.agents_packages.legal_agent.governance.legal_agent_governance_wiring.with_dual_legal_governance`.
...
3. Run the agent via :class:`~intergrax.agents.agent_engine.AgentEngine` as usual.
   :func:`~intergrax.agents_packages.legal_agent.pipeline.legal_execution_loop.run_legal_dynamic_execution_loop`
   calls ``adjust_legal_tool_plan`` automatically when the field is not ``None``.

**What they must do:** Return a :class:`~intergrax.agents_packages.legal_agent.domain.legal_tool_plan.LegalToolPlan`
...
from intergrax.agents_packages.legal_agent.governance.legal_tool_plan_governance_port import LegalToolPlanGovernancePort
