...
class LlamaCppEmbeddingProvider(EmbeddingProvider):
    """
    llama.cpp embedding provider (OpenAI-compatible ``/v1/embeddings``).

    Requires a llama.cpp server started with ``--embeddings``, e.g.
    ``llama-server --embeddings -hf nomic-ai/nomic-embed-text-v1.5-GGUF``.
    """
