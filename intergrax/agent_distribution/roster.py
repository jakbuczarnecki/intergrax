...
SCHEMA_MANIFEST_DEFAULT_AGENT_DECLARATION_V1: Final = (
    "manifest_default_agent_declaration.v1"
)
...
class ManifestDefaultAgentDeclaration(BaseModel):
    """Neutral Tier-0 manifest default roster row — host adapts ApplicationManifest (§13.1)."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: str = SCHEMA_MANIFEST_DEFAULT_AGENT_DECLARATION_V1
    ...
    def _serialize_config(
        self, value: Mapping[str, DistributionJsonValue]
...
class EffectiveRosterEntry(BaseModel):
    ...
    @classmethod
    def _validate_merged_config_raw(
        cls, value: object
    ) -> dict[str, DistributionJsonValue]:
        if not isinstance(value, Mapping):
...
class EffectiveRoster(BaseModel):
    ...
    @field_validator(
        "application_id", "application_environment_id", "manifest_release_id"
    )
    @classmethod
    ...
    def with_revision_id(self) -> EffectiveRoster:
        ...
        return self.model_copy(
            update={"effective_roster_revision_id": self.compute_revision_id()}
        )
