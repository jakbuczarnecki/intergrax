...
class _ProductionCapacityConfiguredAllowEvaluator:
    """Explicit production-host policy: configured service principal may scale."""

    def evaluate(self, request: ControlPlaneMutationRequest) -> PolicyDecision:
        del request
        return PolicyDecision(
            action=PolicyAction.ALLOW,
            reason="production_capacity_policy_configured",
            policy_rule_id="production_capacity.scale_probe",
        )
...
def build_production_capacity_governance(
    ...
    *,
    ...
    """Build explicit single-tenant production capacity governance from host profile."""
    ...
    return ProductionCapacityGovernance(
        principal=principal,
        mutation_authorization_boundary=ControlPlaneMutationAuthorizationBoundary(
            evaluator=_ProductionCapacityConfiguredAllowEvaluator(),
        ),
