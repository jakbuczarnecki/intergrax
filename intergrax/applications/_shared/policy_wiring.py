...
def _build_declarative_policy_composition(
    ...
    *,
...
) -> _DeclarativePolicyComposition:
    if policy_rules is None:
        raise ValueError("policy_rules is required for declarative policy composition")
    ...
    if discover:
        handler_outcome = load_policy_rule_plugin_report(registry, policy=load_policy)
        load_report = handler_outcome.report
        external_handler_provenance = handler_outcome.handler_provenance
        definition_outcome = load_policy_definition_plugin_report(
            registry,
    ...
    return _DeclarativePolicyComposition(
        runtime=DeclarativePolicyRuntime(
            registry=registry,
            rules=rules,
            load_report=load_report,
