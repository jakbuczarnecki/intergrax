...
def build_workspace_registry(repo_root: Path) -> dict[str, WorkspacePackage]:
    ...
    registry[_norm(root_name)] = WorkspacePackage(
        dist_name=root_name,
        kind="platform",
        member_path=".",
        dir_name=".",
...
def _classify_dep(
    *,
    ...
    if name.lower().startswith("intergrax-") and name.lower().endswith("-application"):
        if importer_kind == "agent":
            raise ValueError(
                "AGENT_TIER_VIOLATION: "
                f"importing agent {importer_label} depends on forbidden "
...
def _format_cycle(path: list[str], closing: str) -> str:
    chain = path + [closing]
    arrows = "\n→ ".join(chain)
    return f"AGENT_DEPENDENCY_CYCLE:\n{arrows}"
...
def load_application_runtime_graph(
    ...
    app_entry = registry.get(_norm(app_dist)) if app_dist else None
    ...
    direct_keys = {_norm(e.dist_name) for e in direct_agent_entries}
    visit_state: dict[str, _VisitState] = {
        key: _VisitState.UNVISITED for key in registry if registry[key].kind == "agent"
    }
    # Also track agents discovered during traversal that may not yet be keyed.
    for entry in direct_agent_entries:
        visit_state.setdefault(_norm(entry.dist_name), _VisitState.UNVISITED)
    ...
    def dfs(entry: WorkspacePackage) -> None:
        ...
        if state is _VisitState.VISITING:
            raise ValueError(_format_cycle(stack, entry.dist_name))
        ...
        for dep in agent_deps:
            ...
            if dep_key not in direct_keys and all(
                _norm(t.dist_name) != dep_key for t in transitive_entries
            ):
