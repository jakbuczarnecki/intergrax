# © Artur Czarnecki. All rights reserved.
# Intergrax framework – proprietary and confidential.

"""CodeCraftSessionManager — craft_id lifecycle per tenant/task (ECC-2)."""

from __future__ import annotations

from threading import RLock
from uuid import uuid4

from intergrax.codecraft.contracts import CodeCraftSession
from intergrax.tools.registry.wiring import ToolWiringContext


class CodeCraftSessionManager:
    """Task-scoped in-memory craft session store."""

    def __init__(self) -> None:
        self._sessions: dict[str, CodeCraftSession] = {}
        self._lock = RLock()

    def open(
        self,
        *,
        goal: str,
        task_id: str,
        tenant_id: str,
        mode: str,
        language: str = "python",
        max_iterations: int = 8,
        craft_id: str | None = None,
    ) -> CodeCraftSession:
        sid = craft_id or f"craft_{uuid4().hex[:12]}"
        session = CodeCraftSession(
            craft_id=sid,
            task_id=task_id,
            tenant_id=tenant_id,
            goal=goal,
            mode=mode,
            language=language,
            max_iterations=max_iterations,
        )
        with self._lock:
            self._sessions[sid] = session
        return session

    def get(self, craft_id: str) -> CodeCraftSession | None:
        with self._lock:
            return self._sessions.get(craft_id)

    def save(self, session: CodeCraftSession) -> None:
        with self._lock:
            self._sessions[session.craft_id] = session

    def dispose(self, craft_id: str) -> CodeCraftSession | None:
        with self._lock:
            session = self._sessions.pop(craft_id, None)
        if session is None:
            return None
        disposed = session.model_copy(update={"disposed": True, "status": "disposed"})
        return disposed

    def list_for_task(self, task_id: str) -> list[CodeCraftSession]:
        with self._lock:
            return [item for item in self._sessions.values() if item.task_id == task_id and not item.disposed]


_default_manager: CodeCraftSessionManager | None = None


def get_session_manager(ctx: ToolWiringContext) -> CodeCraftSessionManager:
    raw = ctx.extras.get("codecraft_session_manager")
    if isinstance(raw, CodeCraftSessionManager):
        return raw
    global _default_manager
    if _default_manager is None:
        _default_manager = CodeCraftSessionManager()
    return _default_manager
