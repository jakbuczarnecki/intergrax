...
def vendor_knowledge_sync_idempotency_key(job: VendorKnowledgeSyncJob) -> str:
    if job.trigger_delivery_id is None:
        ...
        return (
            ...
            f"{job.tenant_id}:{job.operation_id}:{job.binding_id}:"
            f"{job.mode.value}:{phase}:{job.recovery_attempt}"
        )
    return (
        f"{VENDOR_KNOWLEDGE_SYNC_TASK_NAME}:"
        f"{job.tenant_id}:{job.operation_id}:{job.binding_id}:"
        f"{job.mode.value}:{job.trigger_delivery_id}:{job.recovery_attempt}"
    )
