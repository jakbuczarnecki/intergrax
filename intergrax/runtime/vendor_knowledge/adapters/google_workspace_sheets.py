...
"""Google Workspace Sheets knowledge source adapter."""
...
GOOGLE_SHEETS_SPREADSHEET_SCOPE_TYPE = "google_workspace_sheets_spreadsheet"

GOOGLE_SHEETS_CURSOR_VERSION = "google_workspace.sheets.cursor.v1"

GOOGLE_SHEETS_ITEM_METADATA_VERSION = "google_workspace.sheets.item.v1"

GOOGLE_SHEETS_STRUCTURED_RECORD_SCHEMA = (
    "google_workspace.sheets.spreadsheet.knowledge.v1"
)

GOOGLE_SHEETS_STRUCTURED_RECORD_MIME_TYPE = (
    "application/vnd.intergrax.google-workspace-sheets+json"
)

_GOOGLE_SHEETS_ITEM_TYPE = "google_workspace_sheets_spreadsheet"
...
_DEPENDENCY_UNAVAILABLE_MESSAGE = (
    "Google Workspace Sheets knowledge dependency is unavailable"
)
...
_INTEGRATION_REQUIRED_MESSAGE = (
    "Google Workspace Sheets knowledge adapter requires "
...
class _GoogleSheetsReconciliationCursor(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, strict=True)

    schema_version: Literal["google_workspace.sheets.cursor.v1"]
    scope_fingerprint: str = Field(repr=False)
    complete: Literal[True]


class GoogleWorkspaceSheetsKnowledgeAdapter:
    """Thin mapping from Google Workspace Sheets to neutral knowledge models."""
    ...
    def source_kind(self) -> str:
        return GOOGLE_SHEETS_SOURCE_KIND
    ...
    async def read_page(
        ...
        *,
        ...
        validated_source = self._validate_source(source)
        spreadsheet_id = validated_source.scope.remote_scope_id
        self._validate_limit(limit)
        decoded = self._decode_cursor(cursor, spreadsheet_id=spreadsheet_id)
        ...
        spreadsheet = await self._invoke_integration(
            lambda: google_integration.read_sheets_spreadsheet(
                spreadsheet_id=spreadsheet_id,
            ),
        )
        validated_spreadsheet = self._reconstruct_spreadsheet(
            spreadsheet,
            expected_spreadsheet_id=spreadsheet_id,
        )
        record = _build_structured_record(validated_spreadsheet)
        content_hash = _compute_content_hash(record)
        descriptor = self._spreadsheet_to_descriptor(
            validated_spreadsheet,
            content_hash=content_hash,
        )
        upsert = KnowledgeChange(
            kind=KnowledgeChangeKind.UPSERT,
            remote_id=validated_spreadsheet.spreadsheet_id,
            descriptor=descriptor,
        )
        checkpoint = self._encode_complete_cursor(spreadsheet_id=spreadsheet_id)
    ...
    async def fetch_content(
        ...
        *,
        ...
        return KnowledgeContent(
            ...
            mime_type=GOOGLE_SHEETS_STRUCTURED_RECORD_MIME_TYPE,
            content_hash=content_hash,
        )
    ...
    def _reconstruct_source(self, source: object) -> KnowledgeSourceRef:
        try:
            ...
            if type(scope_raw) is not KnowledgeSourceScope:
            ...
            raw_scope_id = scope_raw.remote_scope_id
            if type(raw_scope_id) is not str or raw_scope_id != raw_scope_id.strip():
                raise ValueError("noncanonical scope id")
    ...
    def _validate_scope_spreadsheet_id(self, value: object) -> str:
        if type(value) is not str:
            raise ValueError("invalid spreadsheet id type")
        trimmed = value.strip()
        if not trimmed:
            raise ValueError("blank spreadsheet id")
        if _ASCII_CONTROL.search(trimmed):
            raise ValueError("control character in spreadsheet id")
        if len(trimmed) > _MAX_SPREADSHEET_ID_LENGTH:
            raise ValueError("spreadsheet id too long")
        if "/" in trimmed or "\\" in trimmed:
            raise ValueError("path separator in spreadsheet id")
        return trimmed
    ...
    def _validate_source(self, source: KnowledgeSourceRef) -> KnowledgeSourceRef:
        ...
        if scope.remote_scope_type != GOOGLE_SHEETS_SPREADSHEET_SCOPE_TYPE:
            raise self._invalid_scope_error()
        if scope.parameters:
            raise self._invalid_scope_error()
        try:
            validated_id = self._validate_scope_spreadsheet_id(scope.remote_scope_id)
        except ValueError:
            raise self._invalid_scope_error() from None
    ...
    def _scope_fingerprint(self, spreadsheet_id: str) -> str:
        payload = f"google_workspace\x00sheets\x00{spreadsheet_id}"
        return hashlib.sha256(payload.encode("utf-8")).hexdigest()

    def _encode_complete_cursor(self, *, spreadsheet_id: str) -> KnowledgeCursor:
        cursor = _GoogleSheetsReconciliationCursor(
            schema_version=GOOGLE_SHEETS_CURSOR_VERSION,
            scope_fingerprint=self._scope_fingerprint(spreadsheet_id),
            complete=True,
        )
        return self._encode_cursor(cursor)

    def _encode_cursor(self, cursor: _GoogleSheetsReconciliationCursor) -> KnowledgeCursor:
        payload = cursor.model_dump()
        raw = json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
        encoded = base64.urlsafe_b64encode(raw).decode("ascii").rstrip("=")
        return KnowledgeCursor(value=encoded, version=GOOGLE_SHEETS_CURSOR_VERSION)
    ...
    def _decode_cursor(
        ...
        *,
        spreadsheet_id: str,
    ) -> _GoogleSheetsReconciliationCursor | None:
        if cursor is None:
            return None
        try:
            ...
            if not outer_value or outer_value != outer_value.strip():
                raise ValueError("cursor whitespace")
            if len(outer_value) > _MAX_ENCODED_CURSOR_LENGTH:
                raise ValueError("cursor too long")
            if "=" in outer_value or _CURSOR_ALPHABET.fullmatch(outer_value) is None:
                raise ValueError("cursor encoding")
            if reconstructed_cursor.version != GOOGLE_SHEETS_CURSOR_VERSION:
            ...
            decoded = _GoogleSheetsReconciliationCursor.model_validate(data)
        ...
        if decoded.scope_fingerprint != self._scope_fingerprint(spreadsheet_id):
            raise self._invalid_cursor_error()
        return decoded

    def _reconstruct_spreadsheet(
        self,
        spreadsheet: object,
        *,
        ...
        try:
            if type(spreadsheet) is not GoogleSheetsSpreadsheet:
                raise ValueError("invalid spreadsheet type")
            snapshot = spreadsheet.model_dump(mode="python")
            validated = GoogleSheetsSpreadsheet(**snapshot)
        except Exception:
            raise self._invalid_provider_response_error() from None
        if validated.spreadsheet_id != expected_spreadsheet_id:
            raise self._invalid_provider_response_error()
        return validated

    def _spreadsheet_to_descriptor(
        self,
        ...
        *,
        ...
        return KnowledgeItemDescriptor(
            ...
            item_type=_GOOGLE_SHEETS_ITEM_TYPE,
    ...
    def _reconstruct_descriptor(self, item: object) -> KnowledgeItemDescriptor:
        try:
            ...
            if (
                ...
                or not isinstance(revision_data, dict)
                or not isinstance(provenance_data, dict)
            ):
    ...
    def _validate_descriptor(
        ...
        *,
        ...
        try:
            if item.identity.remote_id != spreadsheet_id:
                raise ValueError("identity remote id mismatch")
            if item.provenance.remote_id != spreadsheet_id:
            ...
            if item.item_type != _GOOGLE_SHEETS_ITEM_TYPE:
            ...
            if (
                revision.version is not None
                or revision.etag is not None
                or revision.acl_hash is not None
                or revision.updated_at is not None
            ):
            ...
            if type(metadata) is not dict or set(metadata.keys()) != _METADATA_KEYS:
                raise ValueError("metadata keys mismatch")
            if metadata.get("schema_version") != GOOGLE_SHEETS_ITEM_METADATA_VERSION:
                raise ValueError("metadata schema mismatch")
            if metadata.get("structured_record_schema") != GOOGLE_SHEETS_STRUCTURED_RECORD_SCHEMA:
                raise ValueError("structured record schema mismatch")
            if metadata.get("native_mime_type") != GOOGLE_SHEETS_NATIVE_MIME_TYPE:
                raise ValueError("native mime mismatch")
            if type(metadata.get("sheet_count")) is not int:
                raise ValueError("sheet count type mismatch")
            if type(metadata.get("named_range_count")) is not int:
                raise ValueError("named range count type mismatch")
            if (
...
def _build_structured_record(spreadsheet: GoogleSheetsSpreadsheet) -> dict[str, Any]:
    return {
        "schema_version": GOOGLE_SHEETS_STRUCTURED_RECORD_SCHEMA,
        "spreadsheet_id": spreadsheet.spreadsheet_id,
        "title": spreadsheet.title,
        "locale": spreadsheet.locale,
        "time_zone": spreadsheet.time_zone,
        "recalculation_interval": (
            spreadsheet.recalculation_interval.value
            if spreadsheet.recalculation_interval is not None
            else None
        ),
        "sheets": [sheet.model_dump(mode="json") for sheet in spreadsheet.sheets],
        "named_ranges": [
            named_range.model_dump(mode="json")
            for named_range in spreadsheet.named_ranges
        ],
    }
...
def register_google_workspace_sheets_knowledge_adapter(
