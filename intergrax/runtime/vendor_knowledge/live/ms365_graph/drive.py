"""Microsoft Graph Drive live list capability."""
...
MSGRAPH_DRIVE_LIST_CAPABILITY_ID: Final[str] = (
    f"vendor.{MS365_GRAPH_COLLABORATION_SUITE_PROVIDER_ID}.drive.list"
)
MSGRAPH_DRIVE_LIST_REQUEST_SCHEMA_REF: Final[str] = (
    "schema://vendor-knowledge/live/"
    f"{MS365_GRAPH_COLLABORATION_SUITE_PROVIDER_ID}/drive/list/request/v1"
)
MSGRAPH_DRIVE_LIST_RESULT_SCHEMA_REF: Final[str] = (
    "schema://vendor-knowledge/live/"
    f"{MS365_GRAPH_COLLABORATION_SUITE_PROVIDER_ID}/drive/list/result/v1"
)
...
class MsGraphDriveListLiveRequestV1(BaseModel):
    """Strict bounded input for one Drive metadata page."""
...
def build_msgraph_drive_list_descriptor() -> LiveCapabilityDescriptorV1:
    """Build the single supported Microsoft Graph Drive live descriptor."""
    ...
    return LiveCapabilityDescriptorV1(
        ...
        supported_resource_types=(MSGRAPH_DRIVE_SCOPE_TYPE,),
        request_schema_ref=MSGRAPH_DRIVE_LIST_REQUEST_SCHEMA_REF,
        result_schema_ref=MSGRAPH_DRIVE_LIST_RESULT_SCHEMA_REF,
...
class MsGraphDriveListLiveHandlerV1(LiveCapabilityHandlerV1):
    """Stateless live mapping over the existing public Drive adapter boundary."""
    ...
    request_schema_ref = MSGRAPH_DRIVE_LIST_REQUEST_SCHEMA_REF
    result_schema_ref = MSGRAPH_DRIVE_LIST_RESULT_SCHEMA_REF
    expected_request_model = MsGraphDriveListLiveRequestV1

    def __init__(self, adapter: MsGraphDriveKnowledgeAdapter | None = None) -> None:
        self._adapter = adapter or MsGraphDriveKnowledgeAdapter()
    ...
    async def execute(
        ...
        *,
        ...
        try:
            drive_id = self._validate_call(call)
            ...
            if not isinstance(integration, Ms365GraphCollaborationSuiteIntegration):
                raise VendorKnowledgeError(
                    ...
                    safe_message=(
                        "Microsoft Graph Drive live capability requires "
            ...
            source = KnowledgeSourceRef(
                ...
                scope=KnowledgeSourceScope(
                    remote_scope_id=drive_id,
                    remote_scope_type=MSGRAPH_DRIVE_SCOPE_TYPE,
                    safe_display_name="Microsoft Graph Drive",
                    parameters={},
                ),
            )
            effective_limit = min(
    ...
    def _validate_call(self, call: ValidatedLiveCapabilityCallV1) -> str:
        try:
            call.assert_identity()
        except (TypeError, ValueError):
            raise VendorKnowledgeError(
                code=VendorKnowledgeErrorCode.INVALID_SCOPE,
                safe_message="Microsoft Graph Drive live capability identity is invalid",
                provider_id=self.provider_id,
                source_kind=self.source_kind,
                retryable=False,
            ) from None
        if (
            ...
            raise VendorKnowledgeError(
                code=VendorKnowledgeErrorCode.INVALID_SCOPE,
                safe_message="Microsoft Graph Drive live capability scope is invalid",
                provider_id=self.provider_id,
                source_kind=self.source_kind,
                retryable=False,
            )
        if call.remote_resource_id is None:
            raise VendorKnowledgeError(
                code=VendorKnowledgeErrorCode.INVALID_SCOPE,
                safe_message="Microsoft Graph Drive live resource scope is missing",
                provider_id=self.provider_id,
                source_kind=self.source_kind,
                retryable=False,
            )
        try:
            return validate_msgraph_drive_id(call.remote_resource_id)
        except (TypeError, ValueError):
            raise VendorKnowledgeError(
                code=VendorKnowledgeErrorCode.INVALID_SCOPE,
                safe_message="Microsoft Graph Drive live resource scope is invalid",
                provider_id=self.provider_id,
                source_kind=self.source_kind,
                retryable=False,
            ) from None
    ...
    def _validate_request(
        ...
        if not isinstance(call.validated_request, self.expected_request_model):
            raise VendorKnowledgeError(
                ...
                safe_message="Microsoft Graph Drive live request is invalid",
                provider_id=self.provider_id,
                source_kind=self.source_kind,
                retryable=False,
            )
        ...
        *,
        ...
        for change in changes:
            if change.remote_id in seen_remote_ids:
                raise VendorKnowledgeError(
                    code=VendorKnowledgeErrorCode.INVALID_PROVIDER_RESPONSE,
                    safe_message="Microsoft Graph Drive live page contains duplicate item IDs",
                    provider_id=self.provider_id,
                    source_kind=self.source_kind,
                    retryable=False,
                )
    ...
    def _map_change(
        ...
        *,
        ...
        if descriptor is None:
            safe_display_name = "Deleted Drive item"
            normalized = {
            ...
            if isinstance(drive_item_kind, str):
                normalized["drive_item_kind"] = drive_item_kind
            for key in ("mime_type", "size_bytes"):
