...
"""Project Status live capability handler."""
...
PROJECT_STATUS_READ_CAPABILITY_ID: Final[str] = (
    f"vendor.{PROJECT_STATUS_PROVIDER_ID}.{PROJECT_STATUS_SOURCE_KIND}.read"
)
PROJECT_STATUS_READ_REQUEST_SCHEMA_REF: Final[str] = (
    "schema://vendor-knowledge/live/project_status/project/read/request/v1"
)
PROJECT_STATUS_READ_RESULT_SCHEMA_REF: Final[str] = (
...
class ProjectStatusReadLiveRequestV1(BaseModel):
    """Strict bounded request for one project status read."""
...
def build_project_status_read_descriptor() -> LiveCapabilityDescriptorV1:
    return LiveCapabilityDescriptorV1(
        ...
        request_schema_ref=PROJECT_STATUS_READ_REQUEST_SCHEMA_REF,
        result_schema_ref=PROJECT_STATUS_READ_RESULT_SCHEMA_REF,
        max_result_items=1,
...
def _normalized_content(snapshot: ProjectStatusSnapshotV1) -> str:
    payload = {
...
class ProjectStatusReadLiveHandlerV1(LiveCapabilityHandlerV1):
    provider_id = PROJECT_STATUS_PROVIDER_ID
    integration_kind = IntegrationCategory.ISSUE_TRACKER
    source_kind = PROJECT_STATUS_SOURCE_KIND
    capability_id = PROJECT_STATUS_READ_CAPABILITY_ID
    ...
    request_schema_ref = PROJECT_STATUS_READ_REQUEST_SCHEMA_REF
    result_schema_ref = PROJECT_STATUS_READ_RESULT_SCHEMA_REF
    expected_request_model = ProjectStatusReadLiveRequestV1
    ...
    async def execute(
        ...
        *,
        ...
        try:
            ...
            if not isinstance(integration, ProjectStatusIntegration):
                raise self._vendor_error(
                    VendorKnowledgeErrorCode.INVALID_PROVIDER_RESPONSE
                )
            snapshot = await integration.read_project_status(
                project_id=request.project_id,
            )
            if type(snapshot) is not ProjectStatusSnapshotV1:
                raise self._vendor_error(
                    VendorKnowledgeErrorCode.INVALID_PROVIDER_RESPONSE
                )
            ...
            item = LiveCapabilityResultItemV1(
                remote_item_id=f"project:{snapshot.project_id}:status",
                safe_display_name=f"Project status {snapshot.project_id}",
                content=content,
    ...
    def _map_exception(self, exc: BaseException) -> str:
        ...
        if isinstance(exc, ProjectStatusNotFoundError):
            return "live_provider_not_found"
        if isinstance(exc, ProjectStatusReadError):
