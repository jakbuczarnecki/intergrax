...
"""Security Status live capability handler."""
...
SECURITY_STATUS_READ_CAPABILITY_ID: Final[str] = (
    f"vendor.{SECURITY_STATUS_PROVIDER_ID}.{SECURITY_STATUS_SOURCE_KIND}.read"
)
SECURITY_STATUS_READ_REQUEST_SCHEMA_REF: Final[str] = (
    "schema://vendor-knowledge/live/security_status/security/read/request/v1"
)
SECURITY_STATUS_READ_RESULT_SCHEMA_REF: Final[str] = (
    "schema://vendor-knowledge/live/security_status/security/read/result/v1"
)
...
class SecurityStatusReadLiveRequestV1(BaseModel):
...
def build_security_status_read_descriptor() -> LiveCapabilityDescriptorV1:
    return LiveCapabilityDescriptorV1(
        ...
        request_schema_ref=SECURITY_STATUS_READ_REQUEST_SCHEMA_REF,
        result_schema_ref=SECURITY_STATUS_READ_RESULT_SCHEMA_REF,
        max_result_items=1,
...
def _normalized_content(snapshot: SecurityStatusSnapshotV1) -> str:
    payload = {
        "project_id": snapshot.project_id,
        "status": snapshot.status,
        "blockers": [
            {"id": blocker.id, "status": blocker.status.value}
...
class SecurityStatusReadLiveHandlerV1(LiveCapabilityHandlerV1):
    provider_id = SECURITY_STATUS_PROVIDER_ID
    integration_kind = IntegrationCategory.SECURITY_SCANNER
    source_kind = SECURITY_STATUS_SOURCE_KIND
    capability_id = SECURITY_STATUS_READ_CAPABILITY_ID
    ...
    request_schema_ref = SECURITY_STATUS_READ_REQUEST_SCHEMA_REF
    result_schema_ref = SECURITY_STATUS_READ_RESULT_SCHEMA_REF
    expected_request_model = SecurityStatusReadLiveRequestV1
    ...
    async def execute(
        ...
        *,
        ...
        try:
            ...
            if not isinstance(integration, SecurityStatusIntegration):
            ...
            if type(snapshot) is not SecurityStatusSnapshotV1:
                raise self._vendor_error(
                    VendorKnowledgeErrorCode.INVALID_PROVIDER_RESPONSE
                )
            ...
            item = LiveCapabilityResultItemV1(
                remote_item_id=f"security:{snapshot.project_id}:status",
                safe_display_name=f"Security status {snapshot.project_id}",
                content=content,
    ...
    def _map_exception(self, exc: BaseException) -> str:
        ...
        if isinstance(exc, SecurityStatusNotFoundError):
            return "live_provider_not_found"
        if isinstance(exc, SecurityStatusReadError):
