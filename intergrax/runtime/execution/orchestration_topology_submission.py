...
from intergrax.contracts.orchestration_topology import (
    ...
    OrchestrationSlotRecoveryError,
...
class _OrchestrationExecutionRecord(Generic[PayloadT, ResultT]):
    ...
    recovery_results: dict[tuple[str, str], OrchestrationSlotOutcome[ResultT]] = field(
        default_factory=dict,
    )
...
class CanonicalOrchestrationTopologySubmissionPort(
    ...
    def __post_init__(self) -> None:
        if self._production_topology and self._slot_mse_policy is None:
    ...
    def restore_execution_record(
        ...
        *,
        ...
        payloads: dict[OrchestrationSlotId, PayloadT],
        outcomes: dict[OrchestrationSlotId, OrchestrationSlotOutcome[ResultT]],
        continuable_slots: set[OrchestrationSlotId] | None = None,
    ) -> None:
        """Rehydrate an orchestration execution record from durable recovery state."""
        validate_orchestration_topology(topology)
        validate_orchestration_scheduling_policy(scheduling_policy)
        graph = orchestration_topology_to_execution_graph(
            topology,
        ...
        self._registry.records[str(execution_id)] = _OrchestrationExecutionRecord(
            ...
            continuable_slots=set(continuable_slots or ()),
        )
    ...
    async def recover_failed_slot(
        ...
        *,
        ...
        record = self._registry.records.get(str(request.execution_id))
        if record is None:
            raise OrchestrationSlotRecoveryError(
                "unknown orchestration topology execution",
                code="wrong_topology",
            )
        if request.slot_id not in record.payloads:
        ...
        cache_key = (str(request.slot_id), request.correlation_id)
        cached = record.recovery_results.get(cache_key)
        if cached is not None:
            updated = dict(record.outcomes)
            updated[request.slot_id] = cached
            record.outcomes = updated
        ...
        if prior.status is OrchestrationSlotStatus.SKIPPED:
            raise OrchestrationSlotRecoveryError(
                "skipped slot cannot be recovered",
                code="terminal_slot",
            )
        if request.slot_id in record.continuable_slots:
            raise OrchestrationSlotRecoveryError(
                "governed continuation slot must use continue_slot",
                code="require_human",
            )
        ...
        record.recovery_results[cache_key] = recovered
        updated = dict(record.outcomes)
        updated[request.slot_id] = recovered
        record.outcomes = updated
        return build_orchestration_result(
            record.topology,
            outcomes_by_slot=updated,
...
def build_lab_orchestration_topology_submission_port(
    ...
    *,
    ...
    """NON-PRODUCTION — LAB / TEST / QUALIFICATION ONLY.

    Explicit policy-less topology composition (raw slot executors allowed when policy omitted).
    """
    return _build_canonical_orchestration_topology_submission_port(
        nexus_loop,
        slot_mse_policy=slot_mse_policy,
...
def build_orchestration_topology_submission_port(
    ...
    *,
    ...
    """Production-safe composition root wiring topology submission to the canonical Nexus host."""
    if slot_mse_policy is None:
        raise OrchestrationTopologyMseCompositionError(
            "canonical orchestration topology submission requires "
            "OrchestrationTopologySlotMsePolicy; use "
            "build_lab_orchestration_topology_submission_port for non-production composition",
        )
...
def build_lab_orchestration_topology_continuation_port(
    ...
    *,
    ...
    """NON-PRODUCTION — LAB / TEST / QUALIFICATION ONLY. Explicit non-production continuation port."""
    return _build_canonical_orchestration_topology_submission_port(
        nexus_loop,
        slot_mse_policy=slot_mse_policy,
        production_topology=False,
...
def build_orchestration_topology_continuation_port(
    ...
    *,
    ...
    """Production-safe composition root wiring exact slot continuation to the canonical Nexus host."""
    if slot_mse_policy is None:
        raise OrchestrationTopologyMseCompositionError(
            "canonical orchestration topology continuation requires "
            "OrchestrationTopologySlotMsePolicy; use "
            "build_lab_orchestration_topology_continuation_port for non-production composition",
        )
