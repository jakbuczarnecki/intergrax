...
class ProblemListQueryCursorCodec:
    ...
    def __init__(self, *, secret: bytes = _DEFAULT_LIST_CURSOR_SECRET) -> None:
...
def encode_list_index_data(*, problem_id: ProblemId, last_seen_at: datetime, status: ProblemStatus) -> dict[str, str]:
    return {
...
def decode_list_index_data(data: object) -> tuple[ProblemId, datetime, ProblemStatus]:
    ...
    return ProblemId(problem_id), _decode_datetime(last_seen_at), ProblemStatus(status)
...
def _invert_last_seen_micros(last_seen_at: datetime) -> int:
    micros = int(last_seen_at.timestamp() * 1_000_000)
