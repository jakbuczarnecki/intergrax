...
"""Tier-1 hooks for live LLM routing snapshot refresh (M-LLM-X.12.3 · X-13.7)."""
...
def routing_runtime_config_from_task(task: Task) -> RuntimeConfig | None:
    """Resolve Nexus runtime config attached to a task when present."""
...
def sync_routing_for_task_llm_call(task: Task, *, step_index: int | None = None) -> None:
    """Refresh routing snapshot when auxiliary Nexus paths invoke an LLM."""
    config = routing_runtime_config_from_task(task)
    if config is None:
        return
...
def wire_secondary_llm_routing_surfaces(config: RuntimeConfig) -> None:
    """Attach routing snapshot refresh to secondary LLM surfaces on ``RuntimeConfig``."""
    if config.llm_routing_snapshot is None and config.llm_routing_context is None:
        return
