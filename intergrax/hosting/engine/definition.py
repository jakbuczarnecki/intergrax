...
class HostedApplicationDefinition:
    ...
    hook_registrations: dict[HostedApplicationHookPoint, tuple[HostedApplicationHook, ...]]
    event_subscriptions: tuple[HostedApplicationEventSubscription, ...]
    enabled_components: dict[str, ResolvedComponentRegistration]
    ...
    def public_view(self) -> HostedApplicationDefinitionPublicView:
        ...
        return HostedApplicationDefinitionPublicView(
            ...
            event_subscription_ids=tuple(
                subscription.subscription_id for subscription in self.event_subscriptions
            ),
...
def _validate_component_graph(
    ...
    for level_index, level in enumerate(levels):
        for component_id in level:
            ...
            object.__setattr__(resolved, "dependency_level", level_index)

    return enabled


def _find_cycle_path(enabled: dict[str, ResolvedComponentRegistration]) -> list[str]:
    visited: set[str] = set()
    stack: set[str] = set()
...
def _compute_pre_runtime_closure(
    ...
    required_roots = sorted(
        component_id
        for component_id, resolved in enabled.items()
        if resolved.registration.required
    )
    closure: set[str] = set()
...
def resolve_hosted_application_definition(
    ...
    public_payload = {
        ...
        "subscription_ids": [
            subscription.subscription_id for subscription in profile.event_subscriptions
        ],
