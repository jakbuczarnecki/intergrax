...
from intergrax.tools.providers.websearch.fetch_batch_contracts import WebsearchFetchBatchInput, WebsearchFetchBatchOutput
from intergrax.tools.providers.websearch.fetch_batch_handler import WebsearchFetchBatchHandler
...
def websearch_fetch_batch_contract() -> ToolContract:
    return ToolContract(
        tool_id=WEBSEARCH_FETCH_BATCH_TOOL_ID,
        name="websearch.fetch_batch",
        description=(
            "Fetch and extract readable text from multiple HTTP(S) URLs in one call. "
            "Use when several links need full page content after search."
        ),
        description_short="Fetch text from multiple URLs.",
        input_schema=WebsearchFetchBatchInput,
        output_schema=WebsearchFetchBatchOutput,
        error_mapping={},
        side_effects=False,
        injects_context=True,
        category="retrieval",
        risk_level=ToolRiskLevel.LOW,
        tags=("websearch", "fetch", "batch", "context"),
    )
