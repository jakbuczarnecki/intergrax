# © Artur Czarnecki. All rights reserved.
# Intergrax framework – proprietary and confidential.

from intergrax.utils.lazy_export import export_from_bundle

__all__ = [
    "ALGOLIA_SEARCH_PROVIDER_PROVIDER_ID",
    "AlgoliaSearchProviderIntegration",
    "AlgoliaSearchProviderIntegrationConfig",
    "AlgoliaSearchProviderClient",
    "create_algolia_search_provider",
    "create_algolia_search_provider_integration",
    "register_algolia_integration",
]

_BUNDLE_EXPORTS = frozenset(
    {
        "create_algolia_search_provider",
        "create_algolia_search_provider_integration",
    }
)

_INTEGRATION_EXPORTS = frozenset(
    {
        "ALGOLIA_SEARCH_PROVIDER_PROVIDER_ID",
        "AlgoliaSearchProviderIntegration",
        "AlgoliaSearchProviderIntegrationConfig",
        "AlgoliaSearchProviderClient",
    }
)


_CONTRACT_INTEGRATION_EXPORTS = frozenset(
    {
        "ALGOLIA_SEARCH_PROVIDER_PROVIDER_ID",
        "AlgoliaSearchProviderIntegration",
        "AlgoliaSearchProviderIntegrationConfig",
        "AlgoliaSearchProviderClient",
    }
)

def __getattr__(name: str):
    if name == "register_algolia_integration":
        from intergrax.integrations.providers.search_provider.algolia.register import register_algolia_integration

        return register_algolia_integration
    if name in _BUNDLE_EXPORTS:
        from intergrax.integrations.providers.search_provider.algolia import bundle as _bundle

        return export_from_bundle(_bundle, name, _BUNDLE_EXPORTS)
    if name in _INTEGRATION_EXPORTS:
        from intergrax.integrations.providers.search_provider.algolia import integration as _integration

        return export_from_bundle(_integration, name, _INTEGRATION_EXPORTS)
    if name in _CONTRACT_INTEGRATION_EXPORTS:
        from intergrax.integrations.providers.search_provider.algolia import integration as _integration

        return export_from_bundle(_integration, name, _CONTRACT_INTEGRATION_EXPORTS)

    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
