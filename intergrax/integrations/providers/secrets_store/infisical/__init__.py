# © Artur Czarnecki. All rights reserved.
# Intergrax framework – proprietary and confidential.

from intergrax.utils.lazy_export import export_from_bundle

__all__ = [
    "INFISICAL_SECRETS_STORE_PROVIDER_ID",
    "InfisicalSecretsStoreIntegration",
    "InfisicalSecretsStoreIntegrationConfig",
    "InfisicalSecretsStoreClient",
    "create_infisical_secrets_store",
    "create_infisical_secrets_store_integration",
    "register_infisical_integration",
]

_BUNDLE_EXPORTS = frozenset(
    {
        "create_infisical_secrets_store",
        "create_infisical_secrets_store_integration",
    }
)

_INTEGRATION_EXPORTS = frozenset(
    {
        "INFISICAL_SECRETS_STORE_PROVIDER_ID",
        "InfisicalSecretsStoreIntegration",
        "InfisicalSecretsStoreIntegrationConfig",
        "InfisicalSecretsStoreClient",
    }
)


_CONTRACT_INTEGRATION_EXPORTS = frozenset(
    {
        "INFISICAL_SECRETS_STORE_PROVIDER_ID",
        "InfisicalSecretsStoreIntegration",
        "InfisicalSecretsStoreIntegrationConfig",
        "InfisicalSecretsStoreClient",
    }
)

def __getattr__(name: str):
    if name == "register_infisical_integration":
        from intergrax.integrations.providers.secrets_store.infisical.register import register_infisical_integration

        return register_infisical_integration
    if name in _BUNDLE_EXPORTS:
        from intergrax.integrations.providers.secrets_store.infisical import bundle as _bundle

        return export_from_bundle(_bundle, name, _BUNDLE_EXPORTS)
    if name in _INTEGRATION_EXPORTS:
        from intergrax.integrations.providers.secrets_store.infisical import integration as _integration

        return export_from_bundle(_integration, name, _INTEGRATION_EXPORTS)
    if name in _CONTRACT_INTEGRATION_EXPORTS:
        from intergrax.integrations.providers.secrets_store.infisical import integration as _integration

        return export_from_bundle(_integration, name, _CONTRACT_INTEGRATION_EXPORTS)

    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
