...
profile = IntegrationProfile(observability_backend="langsmith")
...
Langsmith hosts a contract-based observability vendor integration alongside the legacy query facade.



```python

from intergrax.integrations.providers.observability_backend.langsmith import (
...
integration = create_langsmith_observability_integration(transport=my_transport, enabled=True)
...
- **``LangsmithObservabilityIntegration``** derives from **`ObservabilityVendorIntegrationContract`**

- **Legacy query facade** — ``create_langsmith_observability_backend()`` / ``ObservabilityBackend`` — remains backward-compatible
...
- **Transport required when enabled** — ``enabled=True`` without ``LangsmithObservabilityTransport`` raises ``IntegrationConfigurationError`` immediately (no silent broken export)
...
``register_langsmith_integration()`` still registers the legacy query facade only; registry v2 / contract registry wiring is deferred.
