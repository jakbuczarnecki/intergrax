...
OpenSearch hosts a contract-based observability vendor integration alongside the legacy query facade.



```python

from intergrax.integrations.providers.observability_backend.opensearch import (
...
integration = create_opensearch_observability_integration(transport=my_transport, enabled=True)
...
- **``OpensearchObservabilityIntegration``** derives from **`ObservabilityVendorIntegrationContract`**

- **Legacy query facade** — ``create_opensearch_observability_backend()`` / ``ObservabilityBackend`` — remains backward-compatible
...
- **Transport required when enabled** — ``enabled=True`` without ``OpensearchObservabilityTransport`` raises ``IntegrationConfigurationError`` immediately (no silent broken export)
...
``register_opensearch_integration()`` still registers the legacy query facade only; registry v2 / contract registry wiring is deferred.
