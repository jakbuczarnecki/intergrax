...
New Relic hosts a contract-based observability vendor integration alongside the legacy query facade.



```python

from intergrax.integrations.providers.observability_backend.newrelic import (
...
integration = create_newrelic_observability_integration(transport=my_transport, enabled=True)
...
``register_newrelic_integration()`` still registers the legacy query facade only; registry v2 / contract registry wiring is deferred.
