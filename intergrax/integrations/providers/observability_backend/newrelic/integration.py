...
"""Newrelic observability vendor integration (INTEGRATIONS-2C · INTEGRATIONS-2E runtime cutover)."""
...
class NewRelicObservabilityIntegrationConfig(ObservabilityVendorIntegrationConfig):
    """Typed config for Newrelic observability vendor integration."""
...
class NewRelicObservabilityTransport(Protocol):
    ...
    async def send_observability_payload(self, payload: ObservabilityVendorPayload) -> None:
        """Deliver a policy-sanitized vendor payload to Newrelic."""


class NewRelicObservabilityIntegration(ObservabilityVendorIntegrationContract[NewRelicObservabilityIntegrationConfig]):
    """
    Single public Newrelic observability entrypoint.

    Legacy catalog factory (create_newrelic_observability_backend) owns catalog query behavior; legacy factories use from_client().
    """
    ...
    def from_client(
        ...
        *,
        ...
        signals = supported_signals or NEWRELIC_SUPPORTED_SIGNALS
    ...
    def from_transport(
        ...
        *,
        ...
        integration = cls.for_provider(
            provider_id=NEWRELIC_OBSERVABILITY_PROVIDER_ID,
            supported_signals=supported_signals or NEWRELIC_SUPPORTED_SIGNALS,
            display_name="Newrelic",
            config=NewRelicObservabilityIntegrationConfig(enabled=enabled),
        )
