...
profile = IntegrationProfile(observability_backend="signoz")
...
SigNoz hosts a contract-based observability vendor integration alongside the legacy query facade.



```python

from intergrax.integrations.providers.observability_backend.signoz import (
...
integration = create_signoz_observability_integration(transport=my_transport, enabled=True)
...
- **``SignozObservabilityIntegration``** derives from **`ObservabilityVendorIntegrationContract`**

- **Legacy query facade** — ``create_signoz_observability_backend()`` / ``ObservabilityBackend`` — remains backward-compatible
...
- **Transport required when enabled** — ``enabled=True`` without ``SignozObservabilityTransport`` raises ``IntegrationConfigurationError`` immediately (no silent broken export)
...
``register_signoz_integration()`` still registers the legacy query facade only; registry v2 / contract registry wiring is deferred.
