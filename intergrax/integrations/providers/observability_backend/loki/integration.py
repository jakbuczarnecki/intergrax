...
LOKI_OBSERVABILITY_PROVIDER_ID = "loki"

_LOKI_SUPPORTED_SIGNALS: tuple[ObservabilityVendorSignal, ...] = (
...
@runtime_checkable
class LokiObservabilityTransport(Protocol):
    ...
    async def send_observability_payload(self, payload: ObservabilityVendorPayload) -> None:
        """Deliver a policy-sanitized vendor payload to Loki."""


class LokiObservabilityIntegration(ObservabilityVendorIntegrationContract[LokiObservabilityIntegrationConfig]):
    """
    Single public Loki observability entrypoint.

    Legacy catalog factory (create_loki_observability_backend) owns catalog query behavior; legacy factories use from_client().
    """
    ...
    def from_transport(
        cls,
        transport: LokiObservabilityTransport,
        *,
        enabled: bool = False,
        supported_signals: tuple[ObservabilityVendorSignal, ...] | None = None,
    ) -> LokiObservabilityIntegration:
    ...
    @property
    def transport(self) -> LokiObservabilityTransport | None:
