...
profile = IntegrationProfile(observability_backend="honeycomb")
...
Honeycomb hosts a contract-based observability vendor integration alongside the legacy query facade.



```python

from intergrax.integrations.providers.observability_backend.honeycomb import (
...
integration = create_honeycomb_observability_integration(transport=my_transport, enabled=True)
...
- **``HoneycombObservabilityIntegration``** derives from **`ObservabilityVendorIntegrationContract`**

- **Legacy query facade** — ``create_honeycomb_observability_backend()`` / ``ObservabilityBackend`` — remains backward-compatible
...
- **Transport required when enabled** — ``enabled=True`` without ``HoneycombObservabilityTransport`` raises ``IntegrationConfigurationError`` immediately (no silent broken export)
...
``register_honeycomb_integration()`` still registers the legacy query facade only; registry v2 / contract registry wiring is deferred.
