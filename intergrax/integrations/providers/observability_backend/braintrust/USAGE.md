...
Braintrust hosts a contract-based observability vendor integration alongside the legacy query facade.



```python

from intergrax.integrations.providers.observability_backend.braintrust import (
...
integration = create_braintrust_observability_integration(transport=my_transport, enabled=True)
...
- **``BraintrustObservabilityIntegration``** derives from **`ObservabilityVendorIntegrationContract`**

- **Legacy query facade** — ``create_braintrust_observability_backend()`` / ``ObservabilityBackend`` — remains backward-compatible
...
- **Transport required when enabled** — ``enabled=True`` without ``BraintrustObservabilityTransport`` raises ``IntegrationConfigurationError`` immediately (no silent broken export)
...
``register_braintrust_integration()`` still registers the legacy query facade only; registry v2 / contract registry wiring is deferred.
