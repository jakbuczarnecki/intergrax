# © Artur Czarnecki. All rights reserved.
# Intergrax framework – proprietary and confidential.

from intergrax.utils.lazy_export import export_from_bundle

__all__ = [
    "TRIVY_SECURITY_SCANNER_PROVIDER_ID",
    "TrivySecurityScannerIntegration",
    "TrivySecurityScannerIntegrationConfig",
    "TrivySecurityScannerClient",
    "create_trivy_security_scanner",
    "create_trivy_security_scanner_integration",
    "register_trivy_integration",
]

_BUNDLE_EXPORTS = frozenset(
    {
        "create_trivy_security_scanner",
        "create_trivy_security_scanner_integration",
    }
)

_INTEGRATION_EXPORTS = frozenset(
    {
        "TRIVY_SECURITY_SCANNER_PROVIDER_ID",
        "TrivySecurityScannerIntegration",
        "TrivySecurityScannerIntegrationConfig",
        "TrivySecurityScannerClient",
    }
)


_CONTRACT_INTEGRATION_EXPORTS = frozenset(
    {
        "TRIVY_SECURITY_SCANNER_PROVIDER_ID",
        "TrivySecurityScannerIntegration",
        "TrivySecurityScannerIntegrationConfig",
        "TrivySecurityScannerClient",
    }
)

def __getattr__(name: str):
    if name == "register_trivy_integration":
        from intergrax.integrations.providers.security_scanner.trivy.register import register_trivy_integration

        return register_trivy_integration
    if name in _BUNDLE_EXPORTS:
        from intergrax.integrations.providers.security_scanner.trivy import bundle as _bundle

        return export_from_bundle(_bundle, name, _BUNDLE_EXPORTS)
    if name in _INTEGRATION_EXPORTS:
        from intergrax.integrations.providers.security_scanner.trivy import integration as _integration

        return export_from_bundle(_integration, name, _INTEGRATION_EXPORTS)
    if name in _CONTRACT_INTEGRATION_EXPORTS:
        from intergrax.integrations.providers.security_scanner.trivy import integration as _integration

        return export_from_bundle(_integration, name, _CONTRACT_INTEGRATION_EXPORTS)

    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
