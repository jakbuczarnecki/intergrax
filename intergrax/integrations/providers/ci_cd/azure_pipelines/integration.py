...
AzurePipelinesCiCdClient = CiCdBackend

class AzurePipelinesCiCdIntegration(CiCdIntegrationContract):
    """
    Single public Azure Pipelines ci cd entrypoint.

    Legacy catalog factory (create_azure_pipelines_ci_cd) owns catalog behavior; legacy factories use from_client().
    """
    ...
    def health(self) -> HealthStatus:
        ...
        if isinstance(result, HealthStatus):
            return result
        return HealthStatus(
            slug=AZURE_PIPELINES_CI_CD_PROVIDER_ID,
            healthy=bool(result),
            detail="azure_pipelines ready probe",
        )
