...
GitlabCiCiCdClient = CiCdBackend

class GitlabCiCiCdIntegration(CiCdIntegrationContract):
    """
    Single public Gitlab Ci ci cd entrypoint.

    Legacy catalog factory (create_gitlab_ci_ci_cd) owns catalog behavior; legacy factories use from_client().
    """
    ...
    def health(self) -> HealthStatus:
        ...
        if isinstance(result, HealthStatus):
            return result
        return HealthStatus(
            slug=GITLAB_CI_CI_CD_PROVIDER_ID,
            healthy=bool(result),
            detail="gitlab_ci ready probe",
        )
