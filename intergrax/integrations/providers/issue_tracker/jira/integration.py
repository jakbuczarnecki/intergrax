...
class JiraIssueTrackerIntegration(IssueTrackerIntegrationContract):
    ...
    Legacy catalog factory (create_jira_integration) delegates to this class.
    """

    config: JiraIssueTrackerIntegrationConfig = JiraIssueTrackerIntegrationConfig()
    _client: _JiraIssueTrackerClient | None = PrivateAttr(default=None)
    ...
    def from_client(
        cls,
        client: _JiraIssueTrackerClient,
        *,
        enabled: bool = False,
    ) -> JiraIssueTrackerIntegration:
        integration = cls.for_provider(
