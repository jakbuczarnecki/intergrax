...
def sqlite_collaborative_activity_read_store(
    db_path: str,
) -> SQLiteCollaborativeActivityReadStore:
    path = Path(db_path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def collaborative_activity_read_store_from_sqlite_bundle(
    bundle: CollaborativeWorkMaterializedRepositories,
) -> SQLiteCollaborativeActivityReadStore:
    owner = collaborative_work_core_repositories(bundle).store
    if not isinstance(owner, SQLiteCollaborativeWorkStore):
        raise TypeError("bundle store is not SQLiteCollaborativeWorkStore")
    return SQLiteCollaborativeActivityReadStore(owner)


def postgresql_collaborative_activity_read_store(
    *,
    config: PostgreSQLIntegrationConfig | None = None,
    connection_factory: Callable[[], Any] | None = None,
    schema_name: str | None = None,
    ...
    try:
        ...
        raise IntegrationConfigurationError(
            "PostgreSQL Collaborative Activity read store could not be opened"
        ) from exc
    return PostgreSQLCollaborativeActivityReadStore(store)


def collaborative_activity_read_store_from_postgresql_bundle(
    bundle: CollaborativeWorkMaterializedRepositories,
) -> PostgreSQLCollaborativeActivityReadStore:
    owner = collaborative_work_core_repositories(bundle).store
    if not isinstance(owner, PostgreSQLCollaborativeWorkStore):
        raise TypeError("bundle store is not PostgreSQLCollaborativeWorkStore")
    return PostgreSQLCollaborativeActivityReadStore(owner)
