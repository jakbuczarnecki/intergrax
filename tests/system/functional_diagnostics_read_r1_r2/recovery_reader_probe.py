...
"""Subprocess reader for DIAG-FUNCTIONAL-READ-R1-R2 Mongo recovery proof."""
...
def main() -> int:
    uri = os.environ["DIAG_R1R2_MONGO_URI"]
    collection = os.environ["DIAG_R1R2_COLLECTION"]
    tenant_id = os.environ["DIAG_R1R2_TENANT"]
    task_id = validate_task_id(os.environ["DIAG_R1R2_TASK"])
    run_id = validate_run_id(os.environ["DIAG_R1R2_RUN"])
    page_size = int(os.environ["DIAG_R1R2_PAGE_SIZE"])
    expected_count = int(os.environ["DIAG_R1R2_EXPECTED_COUNT"])
