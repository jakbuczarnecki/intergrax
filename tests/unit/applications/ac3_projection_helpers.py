...
"""Test helpers for AC-3 revision-bound registry projection."""
...
from intergrax.applications.contracts.manifest import ApplicationManifest, AgentBinding
from intergrax.applications.contracts.agent_ref import qualname_for_callable

_TEST_DIGEST = "sha256:" + ("a" * 64)
_TEST_ROSTER_DIGEST = "sha256:" + ("e" * 64)
_TEST_LOCK_ID = "lock-1"
_TEST_LOCK_DIGEST = "sha256:" + ("b" * 64)
_TEST_GRAPH_DIGEST = "sha256:" + ("c" * 64)
_TEST_ARTIFACT = "sha256:" + ("d" * 64)
_TEST_RELEASE = "test-release-1"
...
def build_test_registry_projection(
    manifest: ApplicationManifest,
...
def build_test_registry_projection_bundle(
    ...
    *,
    ...
    entries = tuple(_roster_entry_from_binding(binding, package_digest=package_digest) for binding in bindings)
    ...
    revision = RuntimeRevision(
        ...
        effective_roster_revision_id=roster.effective_roster_revision_id or _TEST_ROSTER_DIGEST,
        installed_agent_package_digests=(package_digest,),
        materialized_runtime_lock_id=_TEST_LOCK_ID,
        materialized_runtime_lock_digest=_TEST_LOCK_DIGEST,
