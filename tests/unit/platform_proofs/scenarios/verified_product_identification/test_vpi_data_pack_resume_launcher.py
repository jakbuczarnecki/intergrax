...
from platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume import (
...
def test_path_preflight_missing_build_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.vpi_data_pack_resume_config.VPI_DATA_PACK_RESUME_BUILD_SOURCE_SHA",
        "test-sha",
    )
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.vpi_data_pack_resume_config.VPI_DATA_PACK_ARTIFACT_ROOT",
        output_root,
    )
    build_source = tmp_path / "build-source" / "test-sha"
    build_source.mkdir(parents=True)
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.vpi_data_pack_resume_config.VPI_DATA_PACK_BUILD_SOURCE_ROOT",
    ...
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.vpi_data_pack_resume_config.VPI_DATA_PACK_DATASET_PATH",
        dataset,
    )
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.vpi_data_pack_resume_config.VPI_DATA_PACK_DATASET_MANIFEST_PATH",
        manifest,
    )
    python_stub = tmp_path / "python.exe"
    python_stub.write_bytes(b"")
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.vpi_data_pack_resume_config.VPI_DATA_PACK_CUDA_PYTHON",
        python_stub,
    )
    plan = resolve_vpi_data_pack_resume_launch_plan()
...
def test_active_writer_guard_fails_when_pid_alive(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume._windows_pid_alive",
...
def test_run_resume_dry_run_does_not_start_build(
    ...
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume.assert_no_active_canonical_writer",
        lambda: None,
    )
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume.assert_cuda_preflight",
        lambda _path: "test-gpu",
    )
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume.assert_path_preflight",
        lambda _plan: None,
    )
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume._read_build_progress_counts",
        lambda _root: (1200, 3771, 1201, 1200),
    )
    monkeypatch.setattr(
        "platform_proofs.scenarios.verified_product_identification.scripts.dataset.operator_lifecycle.run_vpi_data_pack_resume.resolve_vpi_data_pack_resume_launch_plan",
        resolve_vpi_data_pack_resume_launch_plan,
    )
