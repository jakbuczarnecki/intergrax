...
from intergrax.integrations.providers.prometheus.bundle import (
...
def test_create_prometheus_integration_bundle() -> None:
    ...
    assert isinstance(bundle.observability_backend, PrometheusObservabilityBackend)
...
def test_opens_creates_httpx_client_when_not_injected() -> None:
    ...
    with patch(
        "intergrax.integrations.providers.prometheus.opens._create_http_client",
        return_value=mock_client,
    ) as create_mock:
        from intergrax.integrations.providers.prometheus.opens import open_prometheus_rest_client
