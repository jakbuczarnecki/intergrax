...
from intergrax.integrations.providers.aws.bundle import (
    AwsIntegrationBundle,
    create_aws_cloud_platform,
    create_aws_integration,
)
from intergrax.integrations.providers.aws.config import (
    ENV_AWS_REGION,
    ENV_AWS_ROLE_ARN,
...
def test_opens_assumes_role_when_configured() -> None:
    ...
    with patch(
        "intergrax.integrations.providers.aws.opens._build_base_session",
        return_value=base_session,
    ):
        with patch("intergrax.integrations.providers.aws.opens._import_boto3") as import_mock:
            assumed_session = MagicMock()
            import_mock.return_value.Session.return_value = assumed_session
            from intergrax.integrations.providers.aws.opens import open_aws_boto_session
