...
from intergrax.integrations.providers.qdrant.adapter import QdrantVectorStoreIntegration
from intergrax.integrations.providers.qdrant.bundle import create_qdrant_vector_store
from intergrax.integrations.providers.qdrant.config import QdrantIntegrationConfig
...
def test_qdrant_opens_imports_client_and_builds_rag_store() -> None:
    ...
    with patch("intergrax.integrations.providers.qdrant.opens._import_qdrant_client") as import_mock:
        with patch(
            "intergrax.rag.vectorstore.providers.qdrant_vector_store.QdrantVectorStore",
            return_value=mock_rag_store,
        ) as rag_cls:
            from intergrax.integrations.providers.qdrant.opens import open_qdrant_vector_store
...
def test_chroma_opens_imports_chromadb_and_builds_rag_store() -> None:
    ...
    with patch("intergrax.integrations.providers.chroma.opens._import_chromadb") as import_mock:
        with patch(
...
def test_chroma_missing_package_raises_configuration_error() -> None:
    ...
    with patch(
        "intergrax.integrations.providers.chroma.opens._import_chromadb",
