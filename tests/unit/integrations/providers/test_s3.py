...
from intergrax.integrations.providers.s3.bundle import (
    S3IntegrationBundle,
    create_s3_integration,
    create_s3_object_storage,
)
from intergrax.integrations.providers.s3.config import ENV_S3_BUCKET, ENV_S3_PREFIX, S3IntegrationConfig
...
def test_opens_builds_boto_client_when_not_injected() -> None:
    ...
    with patch(
        "intergrax.integrations.providers.s3.opens.open_s3_boto_session",
        return_value=MagicMock(region_name="eu-central-1"),
    ) as session_mock:
        session_mock.return_value.client.return_value = mock_client
        from intergrax.integrations.providers.s3.opens import open_s3_bucket_client
...
def test_missing_boto3_raises_configuration_error() -> None:
    config = _s3_config()
    with patch(
        "intergrax.integrations.providers.s3.opens._import_boto3",
        side_effect=IntegrationConfigurationError("missing boto3"),
    ):
        from intergrax.integrations.providers.s3.opens import open_s3_object_storage
