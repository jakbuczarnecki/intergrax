...
from intergrax.integrations.providers.pinecone.adapter import PineconeVectorStoreIntegration
from intergrax.integrations.providers.pinecone.bundle import (
    PineconeIntegrationBundle,
    create_pinecone_integration,
    create_pinecone_vector_store,
)
from intergrax.integrations.providers.pinecone.config import (
...
def test_pinecone_config_requires_api_key() -> None:
    with patch("intergrax.integrations.providers.pinecone.opens._import_pinecone"):
...
def test_opens_imports_pinecone_and_builds_rag_store() -> None:
    ...
    with patch("intergrax.integrations.providers.pinecone.opens._import_pinecone") as import_mock:
        with patch(
            "intergrax.rag.vectorstore.providers.pinecone_vector_store.PineconeVectorStore",
            return_value=mock_rag_store,
        ) as rag_cls:
            from intergrax.integrations.providers.pinecone.opens import open_pinecone_vector_store
...
def test_missing_pinecone_package_raises_configuration_error() -> None:
    config = _pinecone_config()
    with patch(
        "intergrax.integrations.providers.pinecone.opens._import_pinecone",
