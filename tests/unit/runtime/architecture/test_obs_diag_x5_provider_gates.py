...
"""OBS-DIAG-X5 — vendor boundary, inventory integrity, and config-leak gates."""
...
def test_obs_diag_x5_inventory_proof_modules_exist() -> None:
    ...
    for row in OBS_DIAG_X5_PROVIDER_INVENTORY:
        for attr in ("live_proof_module", "failure_recovery_proof_module"):
            module = getattr(row, attr)
            if module is None:
                continue
...
def test_obs_diag_x5_contracts_have_no_vendor_specific_config_fields() -> None:
    violations: list[str] = []
    for path in _python_files(_CONTRACTS_ROOT):
