...
"""EE-B3-C — AC-07/AC-08 retry and recovery context mutation abuse."""
...
def test_ee_b3_c_retry_wrong_tenant_does_not_mutate_source_lifecycle() -> None:
...
def test_ee_b3_c_retry_preserves_run_id_on_authorized_transition() -> None:
    lifecycle = AttemptLifecycleService(InMemoryAttemptLifecycleStore())
    service = ExecutionAttemptRetryService(lifecycle)
    tenant = "tenant-b3c"
    run_id = mint_run_id()
...
def test_ee_b3_c_deny_failure_class_not_retried() -> None:
    result = evaluate_execution_retry_eligibility(
        ExecutionRetryEligibilityRequest(
            classification=classify_execution_failure(
                kind=ExecutionFailureKind.GOVERNANCE_DENIED,
            ),
            attempt_number=1,
            max_attempts=5,
        ),
...
def test_ee_b3_c_recovery_rejects_authority_expansion() -> None:
    task_id = mint_task_id()
    task = Task(
        task_id=task_id,
...
def test_ee_b3_c_recovery_rejects_tampered_tenant_in_binding() -> None:
    checkpoint = TaskCheckpoint(
        task_id=mint_task_id(),
        tenant_id="tenant-a",
        resume_token="rt",
