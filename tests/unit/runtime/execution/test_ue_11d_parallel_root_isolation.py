...
def _assert_root_identity_isolation(pair: ConcurrentPairResult) -> None:
    root_a = pair.root_a
    root_b = pair.root_b
...
def _assert_root_authority_isolation(pair: ConcurrentPairResult) -> None:
...
def _assert_root_budget_isolation(pair: ConcurrentPairResult) -> None:
    a = pair.root_a.before_yield
    b = pair.root_b.before_yield
...
def _assert_no_context_cross_talk(pair: ConcurrentPairResult) -> None:
...
async def test_ue_11d_parallel_root_identity_isolation(iteration: int) -> None:
    del iteration
...
@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(_ITERATIONS))
async def test_ue_11d_parallel_root_authority_isolation(iteration: int) -> None:
    del iteration
    pair = await _run_concurrent_root_pair(llm_calls_a=1, llm_calls_b=1)
    _assert_root_authority_isolation(pair)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(_ITERATIONS))
async def test_ue_11d_parallel_root_budget_isolation(iteration: int) -> None:
    del iteration
...
@pytest.mark.asyncio
@pytest.mark.parametrize("iteration", range(_ITERATIONS))
async def test_ue_11d_no_context_cross_talk(iteration: int) -> None:
    del iteration
    pair = await _run_concurrent_root_pair(llm_calls_a=1, llm_calls_b=2)
