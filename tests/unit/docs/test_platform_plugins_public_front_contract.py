...
"""PROMO-P0-3B3: Platform Plugins public domain hub front."""
...
DOC_PATH = REPO_ROOT / "docs" / "project" / "architecture" / "PLATFORM_PLUGINS.md"
...
def test_readme_links_canonical_architecture_doc() -> None:
    ...
    assert "Platform Plugins" in readme_text
...
def test_public_front_structure() -> None:
    ...
    assert text.splitlines()[0].strip() == "# Platform Plugins"
    ...
    assert "DOMAIN-OWNED CAPABILITY CONTRACTS" in front
    assert "Platform Plugins ≠ universal execution engine" in front
...
def test_maintainer_history_moved_below_public_front() -> None:
    ...
    assert "**Program roadmap:**" in text
    assert "PLATFORM-PLUGIN-2 frozen" in text
    assert "PLATFORM-PLUGIN-1..9 program **closed**" in text
