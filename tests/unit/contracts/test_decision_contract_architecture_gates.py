...
"""Static architecture gates for MP-4B Decision contract surface."""
...
_CONTRACT_PATH = _REPO_ROOT / "intergrax" / "contracts" / "decision.py"
...
_REQUIRED_SYMBOLS = (
    ...
    "class DecisionContractInvariantError",
...
def test_mp4b_contract_module_has_no_forbidden_imports() -> None:
    violations: list[str] = []
    for lineno, module in _collect_imports(_CONTRACT_PATH):
        ...
        if any(token in lowered for token in _FORBIDDEN_IMPORT_SUBSTRINGS):
            violations.append(f"{_CONTRACT_PATH.name}:{lineno} imports {module}")
    assert not violations, "\n".join(violations)


def test_mp4b_contract_module_has_no_forbidden_dynamic_patterns() -> None:
    violations = _collect_forbidden_ast_usage(_CONTRACT_PATH)
    assert not violations, "\n".join(violations)


def test_mp4b_contract_module_defines_decision_surface() -> None:
    source = _CONTRACT_PATH.read_text(encoding="utf-8")
    for symbol in _REQUIRED_SYMBOLS:
        assert symbol in source


def test_mp4b_contract_module_has_no_forbidden_decision_fields() -> None:
    forbidden_fields = (
        "artifact_status",
        "approval_status",
        "execution_status",
