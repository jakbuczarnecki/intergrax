...
def test_invalid_empty_failure_id_rejected() -> None:
    clock = FixedClock()
    recorder = DiagnosticsRecorder(
        clock=clock,
        application_id="test_app",
        ...
        profile_digest="sha256:" + "a" * 64,
        definition_digest="sha256:" + "b" * 64,
        failure_id_generator=lambda: "",
    )
    from intergrax.hosting.errors import HostedApplicationDiagnosticError

    with pytest.raises(HostedApplicationDiagnosticError, match="failure_id must not be empty"):
        recorder.record_primary_failure(
...
def test_invalid_whitespace_failure_id_rejected() -> None:
    clock = FixedClock()
    recorder = DiagnosticsRecorder(
        clock=clock,
        application_id="test_app",
        ...
        profile_digest="sha256:" + "a" * 64,
        definition_digest="sha256:" + "b" * 64,
        failure_id_generator=lambda: "bad id",
    )
    from intergrax.hosting.errors import HostedApplicationDiagnosticError

    with pytest.raises(HostedApplicationDiagnosticError, match="whitespace"):
        recorder.record_primary_failure(
            phase=HostedApplicationFailurePhase.RUNTIME_START,
            source_kind="engine",
            source_id="startup",
...
def test_overlong_failure_id_rejected() -> None:
    clock = FixedClock()
    recorder = DiagnosticsRecorder(
        clock=clock,
        application_id="test_app",
        ...
        profile_digest="sha256:" + "a" * 64,
        definition_digest="sha256:" + "b" * 64,
        failure_id_generator=lambda: "f" + ("a" * 300),
    )
    from intergrax.hosting.errors import HostedApplicationDiagnosticError

    with pytest.raises(HostedApplicationDiagnosticError, match="at most"):
        recorder.record_primary_failure(
            phase=HostedApplicationFailurePhase.RUNTIME_START,
            source_kind="engine",
            source_id="startup",
...
def test_naive_diagnostic_clock_rejected() -> None:
    from datetime import datetime

    class NaiveClock(FixedClock):
    ...
    with pytest.raises(HostedApplicationDiagnosticError, match="naive timestamp"):
        recorder.record_primary_failure(
            phase=HostedApplicationFailurePhase.RUNTIME_START,
            source_kind="engine",
            source_id="startup",
...
def test_valid_deterministic_custom_failure_ids() -> None:
    clock = FixedClock()
    ids = iter(["failure.custom.one", "failure.custom.two"])
    ...
    primary = recorder.record_primary_failure(
        phase=HostedApplicationFailurePhase.COMPONENT_START,
        source_kind="component",
        source_id="worker",
        exc=RuntimeError("boom"),
    ...
    secondary = recorder.record_secondary_failure(
        phase=HostedApplicationFailurePhase.ROLLBACK,
        source_kind="cleanup",
        source_id="stop",
        exc=RuntimeError("secondary"),
    ...
    assert primary.failure_id == "failure.custom.one"
    assert secondary.failure_id == "failure.custom.two"
