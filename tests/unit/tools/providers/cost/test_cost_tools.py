...
from intergrax.tools.providers.cost.contracts import CostCheckQuotaInput, CostForecastSpendInput, CostGetRunBudgetInput
...
def test_cost_forecast_spend_from_envelopes() -> None:
    ctx = ToolWiringContext(
        cost_envelopes=(
            BudgetEnvelope(scope=BudgetScope.TENANT, scope_id="t-1", limit_amount=100.0, spent_amount=80.0),
        )
    )
    out = cost_forecast_spend(ctx, CostForecastSpendInput(growth_multiplier=1.2, baseline_spend_ratio=0.5))
    assert len(out.forecasts) == 1
    assert out.forecasts[0].projected_spend > 80.0


def test_cost_forecast_envelopes_not_configured() -> None:
    with pytest.raises(RuntimeError, match="cost_envelopes_not_configured"):
        cost_forecast_spend(ToolWiringContext(), CostForecastSpendInput())
