...
@pytest.mark.asyncio
async def test_idempotent_continuity_resume_refs_already_absent() -> None:
    service, ctx = _harness()
    continuity = ctx["continuity_repo"].get(worker_instance_id=_WORKER_ID)
    assert continuity is not None
    assert _EVIDENCE_REF not in continuity.unresolved_problem_refs
    request = _orchestration_request(continuity_expected_revision=continuity.revision)
    conflict = AutonomousWorkRevisionConflict(
        "conflict",
        entity_kind="WorkerInstance",
    ...
    assert first.episode.continuity_resume_revision == continuity.revision
...
@pytest.mark.asyncio
async def test_idempotent_continuity_resume_refs_already_absent_survives_restart() -> None:
    service, ctx = _harness()
    continuity = ctx["continuity_repo"].get(worker_instance_id=_WORKER_ID)
    assert continuity is not None
    assert _EVIDENCE_REF not in continuity.unresolved_problem_refs
