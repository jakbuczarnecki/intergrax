...
from intergrax.integrations.registry.catalog_manifests import CHROMA, PGVECTOR, QDRANT
...
from tests.integration.memory.e2e.chroma_session_turn_index_real_vendor_support import (
    ChromaSessionTurnIndexQualificationEnv,
    assert_chroma_backend_unavailable,
    build_chroma_memory_rag_stack,
    build_qualification_env,
    build_vector_session_turn_index_store,
    chroma_backend_side_scope_markers,
    chroma_reachable,
    chroma_topology_metadata,
    close_chroma_integration,
    drop_qualification_collections,
...
_EVIDENCE_SOURCE = "chroma_session_turn_index_real_vendor_qualification"
_PROOF_KIND = MemoryProviderDurabilityProofKind.REAL_VENDOR_RECONNECT


def _require_chroma() -> None:
    if not chroma_reachable():
        pytest.skip("Chroma not configured (set INTERGRAX_CHROMA_HOST/PORT)")
    try:
        ensure_chroma_available()
...
@pytest.fixture
def chroma_sti_env() -> ChromaSessionTurnIndexQualificationEnv:
    _require_chroma()
    run_id = unique_qualification_run_id()
    env = build_qualification_env(qualification_run_id=run_id)
    yield env
    try:
        drop_qualification_collections(env)
    except Exception as exc:
        pytest.fail(f"qualification collection cleanup failed: {exc}")


def test_memory_core_has_no_chromadb_imports() -> None:
    forbidden_prefixes = ("chromadb",)
...
async def _run_chroma_sti_qualification(
    env: ChromaSessionTurnIndexQualificationEnv,
    run_id: str,
) -> object:
    create, dispose = session_turn_index_store_factory(env)

    class _ChromaStiFactory(MemoryProviderInstanceFactory[SessionTurnIndexStore]):
    ...
    return await runner.qualify(
        descriptor=MemoryProviderDescriptor(
            ...
            backing_provider_id=CHROMA_VECTOR_STORE_PROVIDER_ID,
        ),
        ...
        factories=MemoryProviderCapabilityFactories(session_turn_index_store=_ChromaStiFactory()),
    )


@pytest.mark.asyncio
async def test_chroma_topology_metadata(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    _, integration = build_vector_session_turn_index_store(chroma_sti_env)
    try:
        meta = chroma_topology_metadata(integration)
        assert meta["backend_provider_id"] == CHROMA_VECTOR_STORE_PROVIDER_ID
        assert meta["chromadb_version"]
        assert meta["deployment"] == "http-server"
        assert meta["metric"] in {"cosine", "l2"}
    finally:
        close_chroma_integration(integration)


@pytest.mark.asyncio
async def test_chroma_backend_side_scope_filtering_uses_chroma_where() -> None:
    markers = chroma_backend_side_scope_markers()
    assert "tenant_id" in markers
    assert "$eq" in markers


@pytest.mark.asyncio
async def test_chroma_sti_behavioral_qualification_produces_trusted_evidence(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    run_id = chroma_sti_env.qualification_run_id
    result = await _run_chroma_sti_qualification(chroma_sti_env, run_id)
    ...
    assert evidence.backing_provider_id == CHROMA_VECTOR_STORE_PROVIDER_ID
    assert evidence.qualification_run_id == run_id


@pytest.mark.asyncio
async def test_chroma_sti_turns_survive_client_reconnect(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    tenant_id = chroma_sti_env.tenant_id
    session_id = f"sess-reconnect-{chroma_sti_env.qualification_run_id}"
    marker = f"reconnect-marker-{chroma_sti_env.qualification_run_id}"
    entry_id = f"entry-reconnect-{chroma_sti_env.qualification_run_id}"

    store_a, integration_a = build_vector_session_turn_index_store(chroma_sti_env, tenant_id=tenant_id)
    try:
        ...
        close_chroma_integration(integration_a)

    store_b, integration_b = build_vector_session_turn_index_store(chroma_sti_env, tenant_id=tenant_id)
    try:
        ...
        close_chroma_integration(integration_b)


@pytest.mark.asyncio
async def test_chroma_sti_durability_evidence_bundle(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    run_id = chroma_sti_env.qualification_run_id
    result = await _run_chroma_sti_qualification(chroma_sti_env, run_id)
    ...
    assert dur.backing_provider_id == CHROMA_VECTOR_STORE_PROVIDER_ID
    assert dur.proof_kind is _PROOF_KIND


@pytest.mark.asyncio
async def test_chroma_cross_tenant_isolation_two_stores(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    marker = f"tenant-secret-{chroma_sti_env.qualification_run_id}"
    session_id = f"sess-x-{chroma_sti_env.qualification_run_id}"

    store_a, int_a = build_vector_session_turn_index_store(
        chroma_sti_env,
        tenant_id="tenant-5f-a",
    )
    try:
        await store_a.upsert_turn(
            tenant_id="tenant-5f-a",
            ...
            message=ChatMessage(
                role="user",
                content=marker,
                entry_id=f"entry-a-{chroma_sti_env.qualification_run_id}",
            ),
        )
    finally:
        close_chroma_integration(int_a)

    store_b, int_b = build_vector_session_turn_index_store(
        chroma_sti_env,
        tenant_id="tenant-5f-b",
    )
    try:
        await store_b.upsert_turn(
            ...
            message=ChatMessage(
                ...
                entry_id=f"entry-b-{chroma_sti_env.qualification_run_id}",
            ),
        )
        hits = await store_b.search_turns(
            query=marker,
        ...
        close_chroma_integration(int_b)


@pytest.mark.asyncio
async def test_chroma_duplicate_turn_id_is_deterministic(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    tenant_id = chroma_sti_env.tenant_id
    session_id = f"sess-dup-{chroma_sti_env.qualification_run_id}"
    entry_id = f"entry-dup-{chroma_sti_env.qualification_run_id}"
    message = ChatMessage(role="user", content="dup-content", entry_id=entry_id)
    store, integration = build_vector_session_turn_index_store(chroma_sti_env, tenant_id=tenant_id)
    try:
...
@pytest.mark.asyncio
async def test_chroma_tombstone_survives_reconnect(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    tenant_id = chroma_sti_env.tenant_id
    session_id = f"sess-tomb-{chroma_sti_env.qualification_run_id}"
    entry_id = f"entry-tomb-{chroma_sti_env.qualification_run_id}"
    marker = f"tombstone-{chroma_sti_env.qualification_run_id}"

    store_a, int_a = build_vector_session_turn_index_store(chroma_sti_env, tenant_id=tenant_id)
    try:
        await store_a.upsert_turn(
            tenant_id=tenant_id,
            session_id=session_id,
    ...
    store_b, int_b = build_vector_session_turn_index_store(chroma_sti_env, tenant_id=tenant_id)
    try:
        await store_b.tombstone_turn(entry_id)
    finally:
        close_chroma_integration(int_b)

    store_c, int_c = build_vector_session_turn_index_store(chroma_sti_env, tenant_id=tenant_id)
    try:
        ...
        close_chroma_integration(int_c)


@pytest.mark.asyncio
async def test_chroma_backend_unavailable_fails_explicitly() -> None:
    assert_chroma_backend_unavailable()
...
def _sti_qualification_registry(
    run_id: str,
    *,
    backing_provider_id: str = CHROMA_VECTOR_STORE_PROVIDER_ID,
) -> object:
...
def test_product_session_manager_requires_trusted_chroma_sti_evidence(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    from intergrax.applications.contracts.environment_profile import ApplicationEnvironmentProfile

    env = ApplicationEnvironmentProfile.product_defaults(profile_id="mem.5f.product.gate")
    ...
    tenant_id = chroma_sti_env.tenant_id
    stack, integration = build_chroma_memory_rag_stack(chroma_sti_env, tenant_id=tenant_id)
    ...
    try:
        with pytest.raises(MemoryProviderAdmissionError) as exc_info:
            build_session_manager_from_environment(
                ...
                rag_stack=stack,
                integration_profile=IntegrationProfile(vector_store=CHROMA),
            )
        assert (
...
@pytest.mark.asyncio
async def test_product_episodic_recall_with_trusted_chroma_sti_evidence(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    from intergrax.applications.contracts.environment_profile import ApplicationEnvironmentProfile

    env = ApplicationEnvironmentProfile.product_defaults(profile_id="mem.5f.product.e2e")
    env.memory_profile = MemoryProfile(
        enable_user_memory=False,
        enable_long_term_memory=False,
        enable_session_vector_index=True,
    ...
    tenant_id = chroma_sti_env.tenant_id
    turn_text = f"product-e2e-{chroma_sti_env.qualification_run_id}"
    registry = _sti_qualification_registry(chroma_sti_env.qualification_run_id)
    integration_profile = IntegrationProfile(vector_store=CHROMA)
    wiring = MemoryPlatformWiring(
        session_storage=InMemorySessionStorage(),
        user_profile_store=InMemoryUserProfileStore(),
    ...
    stack_a, integration_a = build_chroma_memory_rag_stack(chroma_sti_env, tenant_id=tenant_id)
    ...
    stack_b, integration_b = build_chroma_memory_rag_stack(chroma_sti_env, tenant_id=tenant_id)
...
def test_product_chroma_runtime_with_pgvector_sti_evidence_fails(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    from intergrax.applications._shared.memory_vector_wiring import build_session_turn_index_store
    from intergrax.applications.contracts.environment_profile import ApplicationEnvironmentProfile

    env = ApplicationEnvironmentProfile.product_defaults(profile_id="mem.5f.pgvector.evidence")
    env.memory_profile = MemoryProfile(enable_session_vector_index=True)
    tenant_id = chroma_sti_env.tenant_id
    stack, integration = build_chroma_memory_rag_stack(chroma_sti_env, tenant_id=tenant_id)
    registry = _sti_qualification_registry(
...
def test_product_chroma_runtime_with_adapter_only_evidence_fails(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    from intergrax.applications._shared.memory_vector_wiring import build_session_turn_index_store
    from intergrax.applications.contracts.environment_profile import ApplicationEnvironmentProfile
    ...
    env = ApplicationEnvironmentProfile.product_defaults(profile_id="mem.5f.adapter.evidence")
    env.memory_profile = MemoryProfile(enable_session_vector_index=True)
    tenant_id = chroma_sti_env.tenant_id
    stack, integration = build_chroma_memory_rag_stack(chroma_sti_env, tenant_id=tenant_id)
    registry = InMemoryMemoryProviderQualificationEvidenceRegistry()
    registry.register(
        MemoryProviderQualificationEvidence(
            ...
            capability=MemoryProviderCapabilityKind.SESSION_TURN_INDEX_STORE,
            status=MemoryProviderQualificationStatus.QUALIFIED,
            qualification_run_id=chroma_sti_env.qualification_run_id,
            reference_time_iso="2025-01-01T00:00:00+00:00",
            evidence_source=_EVIDENCE_SOURCE,
...
def test_product_qdrant_runtime_with_chroma_sti_evidence_fails(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    ...
    env = ApplicationEnvironmentProfile.product_defaults(profile_id="mem.5f.rev.qdrant")
    env.memory_profile = MemoryProfile(enable_session_vector_index=True)
    stack = RagStack(
        profile=RagProfile(),
...
def test_product_pgvector_runtime_with_chroma_sti_evidence_fails(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    from unittest.mock import MagicMock
    ...
    env = ApplicationEnvironmentProfile.product_defaults(profile_id="mem.5f.rev.pgvector")
    env.memory_profile = MemoryProfile(enable_session_vector_index=True)
    stack = RagStack(
        profile=RagProfile(),
        vectorstore_manager=MagicMock(),
...
@pytest.mark.asyncio
async def test_application_episodic_recall_after_chroma_reconnect(
    chroma_sti_env: ChromaSessionTurnIndexQualificationEnv,
) -> None:
    ...
    tenant_id = chroma_sti_env.tenant_id
    turn_text = f"episodic-e2e-{chroma_sti_env.qualification_run_id}"
...
def test_composite_identity_descriptor() -> None:
    ...
    assert identity.backing_provider_id == CHROMA_VECTOR_STORE_PROVIDER_ID
    assert identity.capability is MemoryProviderCapabilityKind.SESSION_TURN_INDEX_STORE


def test_chroma_service_restart_not_executed() -> None:
    """P2: Chroma service restart proof deferred; reconnect only qualifies as REAL_VENDOR_RECONNECT."""
