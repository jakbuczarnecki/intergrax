...
HARNESS_02_PROPAGATION_MATRIX: tuple[Harness02PropagationRow, ...] = (
    ...
    Harness02PropagationRow(
        ...
        source="task cancellation + RunBudget wall time",
        boundary="RuntimeToolInvoker.invoke",
        propagated_state="metadata flag; parallel wall-time via budget_ticks (partial)",
        enforcement="cancellation only before retry attempts; wall time not on every invoke",
        terminal_result="TASK_CANCELLED / BudgetExceededError (path-dependent)",
        evidence=_nid(_H02, "test_harness_02_tool_invoker_cancellation_not_before_first_attempt"),
        status="GAP",
    ),
    Harness02PropagationRow(
        flow_id="H02-llm-provider-boundary",
        source="RunBudget / ledger consumption",
        boundary="llm_adapters",
        propagated_state="consume_llm_call; provider timeout from contract",
        enforcement="no peek_active_execution_global_deadline at adapter boundary",
        terminal_result="provider timeout / budget counters",
        evidence=_nid(_H02, "test_harness_02_global_deadline_not_wired_to_llm_path"),
        status="GAP",
    ),
    Harness02PropagationRow(
