...
class IndirectPromptInjectionAgent(ReflexAgent):
    """TODO: implement scenario agent contract."""

    contract_id = "indirect_prompt_injection"
    capabilities = ("indirect_prompt_injection.run",)

    def can_handle(self, task_context: TaskContext) -> CapabilityMatchResult:
        if task_context.capability in (None, "indirect_prompt_injection.run"):
            return CapabilityMatchResult(
                matched=True,
                agent_id="indirect_prompt_injection",
                matched_capabilities=["indirect_prompt_injection.run"],
                score=1.0,
