...
**Implementation note (VPI-IMPLEMENTATION-4):** Canonical embedding configuration frozen as swappable scenario env (`VPI_EMBEDDING_PROVIDER` / `VPI_EMBEDDING_MODEL` / `VPI_EMBEDDING_DIMENSION`); platform `EmbeddingProvider` ABI reused (no VPI vendor provider). Reference default: `hf` + `BAAI/bge-m3` @ 1024. Decision: [`EMBEDDING_PROVIDER_DECISION.md`](EMBEDDING_PROVIDER_DECISION.md).

**Implementation note (VPI-IMPLEMENTATION-5):** Reusable provider-neutral storage bootstrap foundation under `storage_bootstrap/`, `ingest/`, `integrations/`, `composition/`, and `bootstrap.py`. Reference PostgreSQL catalog + Qdrant search adapters; manifest compatibility, Gate 0 embedding probe, bounded streaming ingest, checkpoint/resume. Architecture: [`BOOTSTRAP_ARCHITECTURE.md`](BOOTSTRAP_ARCHITECTURE.md). **Status:** architecture complete; bounded live E2E depends on local PostgreSQL/Qdrant and reference BGE-M3 Gate 0 qualification.
