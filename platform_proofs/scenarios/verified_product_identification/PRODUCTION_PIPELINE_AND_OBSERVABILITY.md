...
- **Platform projection (P1B):** optional `PlatformProjectingProductIdentificationObservationSink` projects redaction-safe stage facts to the canonical `RuntimeEvent` spine via `ApplicationExecutionStageSignal` (scenario-owned schema unchanged; additive adapter). **Correlation mismatch** between observation `run_id` and bound `ApplicationExecutionCorrelation` is a contract violation (`ObservationSinkError`) — never silently skipped. **Projection emission failure** surfaces as public `ApplicationExecutionStageSignalEmissionError` at the platform boundary (including mandatory evidence persistence failures on the canonical bus path), translated to `ObservationSinkError`; existing `BEST_EFFORT` / `REQUIRED` recorder policy remains authoritative. Stage signals use non-critical event delivery priority today (`CriticalEventDeliveryError` not applicable). **VPI runtime adoption / Execution Engine / governance:** **PENDING**.
