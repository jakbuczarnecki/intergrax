...
def _ci_smoke_job() -> Job:
    return Job(
        ...
        (
            _uv_sync(),
            Step("CI smoke unit tests", ["uv", "run", "python", "scripts/run_ci_smoke_pytest.py"]),
            Step(
                ...
                [
                    ...
                    "run",
                    "python",
                    "scripts/check_ci_gate_test_purity.py",
                ],
...
def _gate_governance_tier_job() -> Job:
    ...
    for index, script in enumerate(scripts):
        if index:
            command.append("&&")
        parts = script.split()
        command.extend(["uv", "run", "python", f"scripts/{parts[0]}", *parts[1:]])
    return Job(
...
def _gate_governance_wiring_job() -> Job:
    ...
    for index, script in enumerate(scripts):
        if index:
            command.append("&&")
        command.extend(["uv", "run", "python", f"scripts/{script}"])
        ...
        (
            ...
            Step(
                ...
                [
                    ...
                    "run",
                    "python",
                    "scripts/phase_w_adapt_closeout_gate.py",
                    "--enforce-l4-runtime",
                ],
            ),
            Step(
            ...
            Step(
                ...
                ["uv", "run", "python", "scripts/export_harness_spec_schemas.py"],
            ),
            Step(
                "Export capability catalog feed",
                ["uv", "run", "python", "scripts/export_capability_catalog_feed.py"],
            ),
        ),
    )
