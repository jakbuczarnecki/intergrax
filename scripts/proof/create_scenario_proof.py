...
def build_design_scenario_spec(title: str, *, slug: str) -> str:
    ...
    body = (
        ...
        "> If the proof evaluator, evidence packaging, and HTML report are removed, does "
        "the application/runtime still produce enough structured execution information to "
        "reconstruct its material decisions, actions, observations, challenges, "
        "recoveries, diagnostics, and terminal result?\n\n"
        "Required answer: **YES**. If **NO**, the Scenario is not acceptable — Proof "
        "cannot be the sole recorder.\n\n"
        ...
        "- **Observability coverage:** Which must be observable via production-path "
        "`TraceEvent` / `ToolCallTrace` / typed diagnostics?\n"
        "- **Explainability:** What explicit bounded rationale/objective is required per "
        "material decision?\n"
        "- **Evidence linkage:** How are decisions linked to evidence IDs?\n"
        "- **Action correlation:** How are tool/action selections linked to execution "
        "traces?\n"
        ...
        "- **Proof consumption:** How does Proof/report consume canonical trace without "
        "inventing missing explanations?\n"
        "- **Machine-readable artifact:** Expected projection (e.g. `PlatformProofEvidence` "
        "v3 steps/graph) — not a Proof-only logger.\n"
        "- **Application Observability Test result:** Required answer **YES** before "
        ...
        "| domain output | |\n\n"
        "**PROOF DOES NOT OWN:** fabricated rationale; reconstructed model intent not "
        "present in runtime artifacts; post-hoc explanation generated by another LLM.\n\n"
        "### Desired behavior\n\n"
        "_To be qualified._\n\n"
        "### Step-by-step story\n\n"
        "_To be qualified._\n\n"
