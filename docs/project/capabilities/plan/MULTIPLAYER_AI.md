...
**Current active task:** **MP-5F — BLOCKED** (source adapters / integration — **MP-5F-B1 — CLOSED**; **MP-5F-B2 — CLOSED**; **MP-5F-B3 — BLOCKED** (UCL canonical workspace ownership missing); **MP-5F-B4 — WAITING**). MP-5E **`ContextViewComposer`** / **`DefaultContextViewComposer`** — **APPROVED / CLOSED** (`intergrax/contracts/context_view_composition.py`).
