...
Multiplayer AI is **not** Slack chat, group conversation, shared memory, execution pause/resume machinery, or multi-agent orchestration relabeled. Channels are **adapters**; LKW is a **reference consumer**, not owner of platform primitives; **Governance/HITL** and **`ExecutionContinuationPort`** own execution pause/resume — Multiplayer exposes collaborative binding/projection over those canonical capabilities, not parallel Decision/HITL authority.

## Current Enterprise Maturity Boundary

**Canonical Multiplayer maturity SSOT.** Other docs must link or summarize this section — they must not invent a competing status matrix.

**Summary:** Multiplayer enterprise core through MP-7 is implemented/closed in its certified scopes. External-agent interoperability (MP-8) and advanced product-facing collaboration UX (MP-9) remain future work.

### ENTERPRISE CORE IMPLEMENTED (MP-1…MP-7)

| Slice | Final status | Exact certified scope (do not inflate) |
| ----- | ------------ | -------------------------------------- |
| **MP-0** | **ACCEPTED / CLOSED** | Architecture baseline + roadmap (docs); accepted as dependency of MP-1+ |
| **MP-1** | **CLOSED / FINAL INDEPENDENT REVIEW PASS** | Principal, WorkspaceMembership, Delegation / effective authority |
| **MP-2** | **APPROVED / CLOSED** | Shared Work (WorkItem, Assignment, lifecycle) |
| **MP-3** | **ENTERPRISE CERTIFIED / CLOSED** | WorkArtifact / WorkArtifactVersion (MP-3A…MP-3H) |
| **MP-4R** | **CLOSED** (MP-4 **FORMALLY CLOSED**; MP-4D docs **CLOSED**) | Canonical Decision / Governance / Execution binding & projection — Decision authority **not** Multiplayer-owned |
| **MP-5** | **ENTERPRISE CERTIFIED / CLOSED** | Principal-scoped ContextView (MP-5A…MP-5H) |
| **MP-6** | **ENTERPRISE CERTIFIED / CLOSED** | Collaborative Activity + provenance (MP-6A…MP-6H) |
| **MP-7** | **ENTERPRISE BOUNDARY CERTIFIED / CLOSED** | **Multiplayer Tier-3 consumability boundary certified** — **not** LKW Multiplayer product adoption complete |

### CAPABILITY EXPANSION PLANNED (MP-8…MP-9)
...
### Explicitly not claimed / deferred
...
> [!NOTE]
> Distinguish **platform/core E2E** (slice-certified MP-1…MP-7) from **product-facing end-to-end UX workflow** (not established; MP-9 future). Do not substitute adjacent capabilities (LKW, Slack, HITL, UCL) for missing Multiplayer primitives.
...
**Related canon:** [`COLLABORATIVE_WORK`](../../architecture/COLLABORATIVE_WORK.md) · [`../plan/MULTIPLAYER_AI.md`](../plan/MULTIPLAYER_AI.md) · qualification: [`MP-5H`](../../maintainers/qualification/MP-5H_FINAL_ENTERPRISE_CERTIFICATION.md) · [`MP-6`](../../maintainers/qualification/MP-6_FINAL_ENTERPRISE_CERTIFICATION.md) · [`MP-7D`](../../maintainers/qualification/MP-7D_FINAL_REFERENCE_CONSUMER_BOUNDARY_ENTERPRISE_CERTIFICATION.md) · ADR-MP-006 / ADR-MP-007 / ADR-MP-008
...
| **Shared work** | MP-2 - Collaborative Work; **CLOSED** |
...
| **Context view** | MP-5 — **ENTERPRISE CERTIFIED / CLOSED**; principal-scoped; composes UCL/CE/Memory/Knowledge |
| **Activity / provenance** | MP-6 — **ENTERPRISE CERTIFIED / CLOSED**; **MP-6A–G — CLOSED / RECERTIFIED or CERTIFIED**; **MP-6H — CLOSED / CERTIFIED** ([`MP-6_FINAL_ENTERPRISE_CERTIFICATION.md`](../../maintainers/qualification/MP-6_FINAL_ENTERPRISE_CERTIFICATION.md); subject to independent audit) |
| **LKW relation** | MP-7 — Tier-3 consumability boundary certified; LKW is reference consumer - not owner |
| **HITL relation** | **REUSED** Governance/HITL + `ExecutionContinuationPort` — not Decision owner; Nexus internal orchestration only |
| **External agent interoperability** | MP-8 — **PLANNED / NOT STARTED**; AgentDirectory ≠ AgentRegistry; Principal kind ≠ AgentDirectory |
| **Current maturity** | **ENTERPRISE CORE IMPLEMENTED** (MP-1…MP-7); **CAPABILITY EXPANSION PLANNED** (MP-8…MP-9); full product-facing Multiplayer UX E2E **not established** |
| **Go deeper** | [Visual architecture](#visual-architecture-mp-final-1) · [Engineering canon](#engineering-canon) · [§Roadmap summary](#roadmap-summary) |
...
Work Artifacts
  ↓
typed binding / projection (MP-4R4+)
  ↓
Canonical Decision / Governance (platform-owned)
  ↓
...
| **Governance/HITL + ExecutionContinuationPort** | Public continuation boundary → Execution Engine; Decision ≠ HITL; Nexus **internal orchestration implementation only** |
...
## Visual architecture (MP-FINAL-1)

Diagrams below are a **visual index** of this SSOT. They do not introduce new public contracts, ownership, or authority. **Platform operates on contracts, not implementations.**

**Legend:** solid edge = owned dependency / runtime flow · dashed edge = reused / reference integration · rounded contract node = public replaceable seam · persistence adapters (e.g. production-qualified store) are **not** platform ABI.

### Diagram 1 — Capability ownership map

```mermaid
flowchart TB
  T3["Tier-3 Applications<br/>(LKW = reference consumer)"]
  COMP["Application Composition"]
  CW["Collaborative Work<br/>semantic owner"]
  P["Principal / Membership / Delegation"]
  WI["WorkItem / Assignment"]
  WA["WorkArtifact"]
  BIND["Decision Binding / projection"]
  CV["ContextView<br/>(eligibility / projection)"]
  ACT["Collaborative Activity"]
  DEC["Decision / Governance<br/>REUSED — not CW-owned"]
  EXE["Execution<br/>REUSED — reference only"]
  CE["Context Engineering / UCL<br/>REUSED"]
  MEM["Memory / RAG / Knowledge<br/>REUSED"]
  OBS["Observability / Evidence<br/>REUSED — factual refs"]

  T3 -->|contracts| COMP --> CW
  CW --> P
  CW --> WI
  CW --> WA
  CW --> BIND
  CW --> CV
  CW --> ACT
  BIND -.->|associates only| DEC
  WI -.->|ExecutionProvenanceRef| EXE
  CV -.->|canonical refs| CE
  CV -.->|canonical refs| MEM
  ACT -.->|evidence refs| OBS
```
...
```mermaid
flowchart TB
  CONS["Tier-3 consumer"]
  PUB["Public contracts<br/>(replaceable seams)"]
  COMP2["Composition roots<br/>(host / CW wiring)"]
...
```mermaid
flowchart LR
  PR["Principal"] --> MEMB["Membership"]
  MEMB --> DEL["Delegation"]
  DEL --> EFF["Effective authority"]
  EFF --> POL["Policy layers"]
  POL --> MSE["MeaningfulSideEffectAuthorizationPort<br/>(replaceable contract)"]
  MSE -->|ALLOW| MUT["Authoritative mutation"]
  MSE -->|missing authority / policy| DENY["DENY — fail-closed"]
```
...
flowchart TB
  WI2["WorkItem"]
  ASG["Assignments<br/>(0..N principals)"]
  EXR["Executions<br/>(reference only — not 1:1 Task)"]
  ART["WorkArtifacts / versions"]
  DB["Decision bindings<br/>(association only)"]
...
WorkItem is **not** a Nexus Task. Multiple executions may relate to one WorkItem.
...
flowchart TB
  ...
  PORTS["Source composition ports<br/>(replaceable)"]
  ADP["Source adapters<br/>(translate only)"]
  REFS["Canonical refs from:<br/>CW · Memory · Knowledge/RAG · UCL"]
  VIEW["ContextView"]

  SCOPE --> VIS --> PORTS --> ADP --> REFS --> VIEW
```

**ContextView owns eligibility/projection** — not source truth, not payload hydration.
...
flowchart LR
  SRC["Source domain"]
  SAD["Source adapter"]
  PUBP["CollaborativeActivityPublicationPort<br/>(replaceable)"]
  ING["Ingestion policy"]
  INT["Append intent"]
  STORE["Append store"]
  READ["Authorized read"]
  CONS2["Activity consumer"]

  SRC --> SAD --> PUBP --> ING --> INT --> STORE --> READ --> CONS2
  SRC -.->|cannot bypass into store| STORE
```

Collaborative Activity ≠ Observability / RuntimeEvent technical log. Observability remains a reused factual plane; Diagnostics **interprets** evidence and is **not** Multiplayer authority.
...
```mermaid
flowchart LR
  subgraph CORE["ENTERPRISE CORE CLOSED"]
    MP1[MP-1] --> MP2[MP-2] --> MP3[MP-3] --> MP4R[MP-4R] --> MP5[MP-5] --> MP6[MP-6] --> MP7[MP-7]
  end
  subgraph FUTURE["FUTURE / NOT STARTED"]
    MP8[MP-8] --> MP9[MP-9]
  end
  MP7 -.-> MP8
```
...
- **MP-FINAL-2 — CLOSED / RECERTIFIED** (after **MP-FINAL-2-C1**). Initial qualification proved Failure→Evidence→Diagnostics→operator projection; independent audit found the operator result type lived in runtime implementation namespace (**PARTIAL PASS**). C1 established the public operator-facing contract boundary in `intergrax.contracts.diagnostics`.
- Evidence: [`MP-FINAL-2_MULTIPLAYER_DIAGNOSTICS_OPERABILITY_E2E_CERTIFICATION.md`](../../maintainers/qualification/MP-FINAL-2_MULTIPLAYER_DIAGNOSTICS_OPERABILITY_E2E_CERTIFICATION.md), [`MP-FINAL-2-C1_OPERATOR_FACING_DIAGNOSTICS_CONTRACT_BOUNDARY_QUALIFICATION.md`](../../maintainers/qualification/MP-FINAL-2-C1_OPERATOR_FACING_DIAGNOSTICS_CONTRACT_BOUNDARY_QUALIFICATION.md).
...
flowchart LR
  ...
  FDA --> PROJ["FunctionalOperatorProjector<br/>implementation"]
  PROJ --> CTR["Operator contract<br/>FunctionalDiagnosticOperatorProjection"]
  CTR --> OPS["Operator"]
...
- **MP-9 = PLANNED / NOT STARTED.**
- Mermaid / documentation visual architecture **≠** product UI.
- Do not treat this hub's diagrams as a product-facing collaboration UX surface.

## External Agent Boundary

- **MP-8 = PLANNED / NOT STARTED.**
- Existing Principal kind `EXTERNAL_AGENT` (semantic kind) and ExternalWork / `ExternalWorkIntegration` may be reused later — they do **not** complete MP-8.
- **Agent kind ≠ AgentDirectory.** AgentDirectory / governed external-agent interoperability remains future work.

## Engineering canon

**Status:** **MP-1 — CLOSED / FINAL INDEPENDENT REVIEW PASS** — **MP-2 — APPROVED / CLOSED** — **MP-3 — ENTERPRISE CERTIFIED / CLOSED** — **MP-4R CLOSED** / **MP-4 FORMALLY CLOSED** — **MP-5 — ENTERPRISE CERTIFIED / CLOSED** (**MP-5A — APPROVED / CLOSED**; **MP-5 ownership — FROZEN**; ADR-MP-006) — **MP-6 — ENTERPRISE CERTIFIED / CLOSED** — **MP-7 — ENTERPRISE BOUNDARY CERTIFIED / CLOSED** (Tier-3 consumability boundary). **MP-8 / MP-9 — PLANNED / NOT STARTED**. Canonical maturity: [Current Enterprise Maturity Boundary](#current-enterprise-maturity-boundary).
...
Multiplayer AI is a **cross-layer Intergrax platform capability** for governed, multi-principal collaboration: shared work, durable collaborative outputs, **collaborative association/projection over canonical Decision and Governance capabilities**, principal-scoped context views, activity and provenance, and interoperability with external agents - without collapsing those primitives into a single product channel, conversation transport, or application-local feature.
...
- Nexus orchestration internals (Nexus is **internal** orchestration strategy only; public continuation is **`ExecutionContinuationPort`**),
...
| **HITL rule** | Collaborative Decision / Approval semantics are distinct from execution pause/resume. Multiplayer **consumes** canonical Governance/HITL and **`ExecutionContinuationPort`**; it does not expose a public Nexus bridge. |
...
```text
MP-0 → MP-1 → MP-2 → MP-3 → MP-4R → MP-5 → MP-6 → MP-7
  [ENTERPRISE CORE CLOSED in certified scopes]
...
MP-8 → MP-9
  [CAPABILITY EXPANSION — PLANNED / NOT STARTED]
```

Canonical status: [Current Enterprise Maturity Boundary](#current-enterprise-maturity-boundary).

| Phase | Summary |
|-------|---------|
| **MP-0** | **ACCEPTED / CLOSED** — canonical architecture and implementation roadmap (this document + plan hub). |
| **MP-1** | **CLOSED** — Principal, WorkspaceMembership, Delegation / effective authority |
| **MP-2** | **CLOSED** — Shared Work: WorkItem, Assignment, lifecycle and concurrency |
| **MP-3** | **ENTERPRISE CERTIFIED / CLOSED** — WorkArtifact and WorkArtifactVersion |
| **MP-4R** | **CLOSED:** canonical Decision / Governance / Execution integration via public contracts (**MP-4R0…MP-4R8 CLOSED**). **MP-4D** = documentation/proof closure only. **Historical MP-4** (pre-rebase Decision/Approval/HITL bridge) **superseded** by ADR-MP-009. |
| **MP-5** | **ENTERPRISE CERTIFIED / CLOSED** — Principal-scoped ContextView |
| **MP-6** | **ENTERPRISE CERTIFIED / CLOSED** — Collaborative Activity + provenance / evidence linkage |
| **MP-7** | **ENTERPRISE BOUNDARY CERTIFIED / CLOSED** — Multiplayer Tier-3 consumability boundary (not full LKW product adoption) |
| **MP-FINAL-1…4** | Final hardening program (docs/visual → diagnostics → capability-wide backend E2E → recertification) |
| **MP-8** | **PLANNED / NOT STARTED** — AgentDirectory / external-agent interoperability |
| **MP-9** | **PLANNED / NOT STARTED** — Advanced collaborative UX / notifications / optional realtime |
...
| UCL (`UNIFIED_CONTEXT_LIFECYCLE`) | Durable/ephemeral context lifecycle, artifact coordination | Principal-scoped ContextView (MP-5 owns visibility policy only) |
| Context Engineering (`CONTEXT_ENGINEERING`) | Budget authority, assembly, provenance of model-facing context | Principal-scoped ContextView (MP-5 owns eligibility; CE owns assembly) |
| Memory (`MEMORY`) | Durable conversation/session ledger | Principal-scoped ContextView (MP-5 owns eligibility; Memory owns stores) |
| RAG / Knowledge (`RAG`, `KNOWLEDGE_SOURCE_INTEGRATIONS`) | Knowledge retrieval and source boundaries | Principal-scoped ContextView (MP-5 owns eligible scope; RAG owns retrieval) |
| Token Optimization (`TOKEN_OPTIMIZATION`, `TOKEN-10E-*`) | Token economy and compaction executors | MP-5 ContextView primitive |
...
| Collaborative Decision binding / projection | MP-4R4+ (future, design gate) | Associates Shared Work with canonical Decision/Governance — **not** a second Decision authority |
...
5. Slack vertical rows are **not** MP-4R; Slack may surface Decisions; **Decision System** owns Decision; **Governance/HITL** owns approval/HITL; Multiplayer may bind/project only.
...
9. `UCL ≠ Principal-scoped ContextView` — UCL owns lifecycle; MP-5 owns principal visibility and composition policy.
10. `Memory ≠ Principal-scoped ContextView` — Memory owns records and recall; MP-5 owns eligibility and projection.
11. `RAG result ≠ Principal-scoped ContextView` — RAG owns retrieval; MP-5 owns eligible knowledge scope.
12. `Context Engineering ≠ Principal-scoped ContextView` — CE owns model-facing assembly and budget; MP-5 owns collaborative visibility.
13. `Token Optimization ≠ MP-5 implementation` — reuse executors; do not relabel TOKEN rows as MP-5.
14. `LKW conversation context ≠ platform Principal-scoped ContextView` — LKW is MP-7 consumer only.
15. `SharedContextView` / `DecisionContextView` ≠ Principal-scoped ContextView — distinct read models; no semantic merge.
16. `ContextView ≠ storage` — no ContextView database, memory store, or vector store in MP-5.
...
```text
WorkArtifactVersion != UCL OptimizationArtifact
Decision != WorkArtifact
Decision != HITL
Approval outcome != WorkArtifactVersion.status
Evidence != execution authorization
```

`WorkArtifactVersion` is the authoritative versioned collaborative output.
WorkArtifacts may outlive executions. A canonical Decision may exist without an
active execution. When execution must pause for human/policy gates, continuation flows
**Governance/HITL → `ExecutionContinuationPort` → Execution Engine**; Multiplayer creates
no second approval, pause, or Decision authority (see ADR-MP-009).
...
**Status:** **APPROVED / CLOSED** — ADR-MP-003 **Accepted; implementation COMPLETE**; COLLAB-WORK-2A…2G **APPROVED / CLOSED**.
...
**Next capability slice:** *(none — MP-2 closed)*. **Historical (at time of earlier slice notes):** MP-5F was once listed here as NEXT; **MP-5F / MP-5 are now ENTERPRISE CERTIFIED / CLOSED**. See [Current Enterprise Maturity Boundary](#current-enterprise-maturity-boundary).
...
**Owning domain:** [`COLLABORATIVE_WORK`](../../architecture/COLLABORATIVE_WORK.md) — frozen by ADR-MP-004

**Frozen semantics:** WorkArtifact aggregate + immutable WorkArtifactVersion; CAS `current_version_id`; atomic create (artifact + first version via `ArtifactPublicationRepository.create_artifact_with_initial_version(...)`); subsequent publish via `publish_version(...)`; typed read/direct ports only for non-authoritative access; neutral `ArtifactContentRef`; principal provenance; optional `ExecutionProvenanceRef`; evidence references version (not owned by CW).

**Reused (not owners):** UCL optimization artifact lifecycle patterns (consumption only); Memory indexing/retrieval; Proof Receipts attestation; optional neutral `ExecutionProvenanceRef`; LKW/channel adapters (consumer mappings only).

**Delivered (gates):** ownership freeze (ADR-MP-004); implementation-ready decomposition (COLLAB-WORK-3).
...
**Next active task:** *(none — MP-3 closed)*. **Historical (superseded current-state claim):** earlier notes listed **MP-5F — NEXT**; MP-5F…MP-5H and **MP-5 — ENTERPRISE CERTIFIED / CLOSED**. See [Current Enterprise Maturity Boundary](#current-enterprise-maturity-boundary).
...
**Frozen semantics:** Collaborative **binding/projection** only; anti-substitution invariants retained under canonical owners; contract-first integration; **no public Nexus dependency** from Multiplayer production modules.

**Reused (not owners):** Decision System; Governed Execution / HITL; `ExecutionContinuationPort`; Evidence Plane; Diagnostics; MP-1 authority.
...
**MP-5A — APPROVED / CLOSED** (architecture / ownership / contract gate only). **MP-5B — APPROVED / CLOSED** (`intergrax/contracts/context_view.py`). **MP-5C — APPROVED / CLOSED** (`intergrax/contracts/context_view_visibility_policy.py`). **MP-5D — APPROVED / CLOSED** — after `ContextViewPolicyDecision`, **source composition ports** (`intergrax/contracts/context_view_source_ports.py`). **MP-5E — APPROVED / CLOSED** — `ContextViewComposer` + `DefaultContextViewComposer` (`intergrax/contracts/context_view_composition.py`, `intergrax/collaborative_work/context_view_composition.py`). **MP-5F — ENTERPRISE SOURCE INTEGRATION CERTIFIED / CLOSED** (B5 adapters: `context_view_source_adapters.py`). **MP-5 — ENTERPRISE CERTIFIED / CLOSED**.
...
**New required (MP-5B+):** Principal-scoped ContextView contracts, visibility policy, replaceable composition policy and composer ports, reference-first entries, external-consumer least-context projection seam.

**Hard invariants:** `ContextView ≠ storage`; authority before retrieval; fail-closed on unresolved principal/membership/delegation/scope; least-context by design; no Nexus or application ownership of public ContextView contracts.

```text
Principal + Membership + Delegation / Effective Authority (MP-1)
  ↓
ContextView policy (MP-5 — Collaborative Work)
  ↓
ContextView composition (replaceable strategy)
  ├── UCL (lifecycle read — consume)
  ├── Memory (eligible recall — consume)
  ├── Knowledge / RAG (eligible retrieve — consume)
  └── Context Engineering (optional downstream assembly — consume)
  ↓
Principal-scoped ContextView (read projection)
  ↓
consumer (runtime, agent, MP-7 LKW, future MP-8 external projection)
```
...
| MP-6 | Collaborative Activity + provenance | **ENTERPRISE CERTIFIED / CLOSED** |
...
**Owner:** **COLLABORATIVE_WORK** (MP-6) — **MP-6 ownership — FROZEN** ([ADR-MP-007](../../technical/adr/entries/2026-09-18/ADR-MP-007.md) **Accepted**). **MP-6 — ENTERPRISE CERTIFIED / CLOSED** (**MP-6H — CLOSED / CERTIFIED**). **MP-6A — CLOSED / RECERTIFIED** (**MP-6A-C1** identity/extensibility/timeline hardening). **MP-6B — CLOSED / RECERTIFIED** (**MP-6B-C1** / **MP-6B-C1-R1**). **MP-6C — CLOSED / RECERTIFIED** (`CollaborativeActivityIngestionService`, trusted publisher context, injectable ingestion policy). **MP-6D — CLOSED / RECERTIFIED** (atomic append store). **MP-6E — CLOSED / RECERTIFIED** (scoped read; authorization before provider). **MP-6F — CLOSED / RECERTIFIED** (source integrations). **MP-6G — CLOSED / CERTIFIED** (E2E qualification).
...
**New required:** Typed `CollaborativeActivity`, `CollaborativeActivityTypeId`, `CollaborativeActivitySourceId`, and publication/read/store ports — [`intergrax/contracts/collaborative_activity.py`](../../../intergrax/contracts/collaborative_activity.py).
...
| MP-6H | Final enterprise certification | **CLOSED / CERTIFIED** |

---

### MP-7 - LKW reference-product adoption

**Status:** **MP-7A — CLOSED / CERTIFIED** · **MP-7B — TIER-3 MULTIPLAYER CONSUMER BOUNDARY QUALIFIED / CLOSED** · **MP-7C-C1-R1 — CLOSED / CERTIFIED** · **MP-7C-C1 — CLOSED / RECERTIFIED** · **MP-7C — CLOSED / RECERTIFIED** (TIER-3 HOST COMPOSITION & BOUNDARY E2E) · **MP-7D — CLOSED / ENTERPRISE CERTIFIED** · **MP-7 — ENTERPRISE BOUNDARY CERTIFIED / CLOSED**

**Intent:** Prove Tier-3 consumability of Multiplayer via public contracts (reference consumer = LKW-shaped host), then adopt product primitives only through explicit MP-owned integration rows — without transferring ownership to LKW.
...
**Architecture gate:** [`MP-7A_LKW_MULTIPLAYER_ADOPTION_ARCHITECTURE_GATE.md`](../../maintainers/qualification/MP-7A_LKW_MULTIPLAYER_ADOPTION_ARCHITECTURE_GATE.md) · [`MP-7B_TIER3_MULTIPLAYER_CONSUMER_BOUNDARY_QUALIFICATION.md`](../../maintainers/qualification/MP-7B_TIER3_MULTIPLAYER_CONSUMER_BOUNDARY_QUALIFICATION.md) · [`MP-7C_TIER3_HOST_COMPOSITION_BOUNDARY_E2E_QUALIFICATION.md`](../../maintainers/qualification/MP-7C_TIER3_HOST_COMPOSITION_BOUNDARY_E2E_QUALIFICATION.md) · [`MP-7D_FINAL_REFERENCE_CONSUMER_BOUNDARY_ENTERPRISE_CERTIFICATION.md`](../../maintainers/qualification/MP-7D_FINAL_REFERENCE_CONSUMER_BOUNDARY_ENTERPRISE_CERTIFICATION.md) · [ADR-MP-008](../../technical/adr/entries/2026-09-19/ADR-MP-008.md) **Accepted** — LKW product workspace **Option B** (`collaborative_workspace_ref`); MP-7B qualifies **boundary consumability**; MP-7C qualifies **Tier-3 host composition boundary E2E**; MP-7D certifies **final Tier-3 consumability boundary** — not Principal/workspace product binding.
...
| MP-7C | **CLOSED / RECERTIFIED** (MP-7C-C1-R1 evaluator contract ownership relocation) |
| MP-7C-C1 | **CLOSED / RECERTIFIED** |
| MP-7C-C1-R1 | **CLOSED / CERTIFIED** |
| MP-7D | **CLOSED / ENTERPRISE CERTIFIED** — Multiplayer Tier-3 consumability boundary |
| MP-7 (program) | **ENTERPRISE BOUNDARY CERTIFIED / CLOSED** (not full LKW product adoption) |
| Deferred product adoption | WorkItem / Assignment / WorkArtifact / Decision / ContextView / Activity / Principal·workspace persistence — outside closed MP-7 boundary |
| MP-8+ | FUTURE — AgentDirectory / advanced collaborative UX |
...
| **ADR-MP-005** | MP-4A collaborative ownership (historical; superseded by ADR-MP-009 for lifecycle ownership) |
| **ADR-MP-009** | MP-4 core rebase on canonical enterprise core (MP-4R0) |
...
WorkItem + Assignment (MP-2)
  ...
  → may bind to canonical Decision (MP-4R4+)
  → emits Activity + evidence (MP-6)

Canonical Decision + Governance/HITL
  → Execution via ExecutionContinuationPort
  → Multiplayer holds references/bindings only (MP-4R)
