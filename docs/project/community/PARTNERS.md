...
```mermaid
flowchart LR
    A[Workflow note] --> B[Fit discussion]
    B --> C[Scope and evidence]
    C --> D[Bounded evaluation]
    D --> E[Review evidence]
    E --> F{Decision}
    F -->|Evidence supports next step| G[CONTINUE]
    F -->|Useful signal, gaps remain| H[REVISE]
