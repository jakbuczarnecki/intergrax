...
- **Strategy coverage (GR-10):** INFERENCE / AGENTIC meaningful-side-effect and HITL paths not enterprise-qualified on production entry points. PRE_MODEL identity boundary: [ADR-GR-10-001](../technical/adr/entries/2026-09-18/ADR-GR-10-001.md) (**GR-10-R2 BLOCKED** until GR-10-R2-C1/R1).
...
| **PRE_MODEL** | **PARTIAL** | Governance | `PolicyEngine.evaluate_pre_llm` ( **`REVISION_REQUIRED`** — [ADR-GR-10-001](../technical/adr/entries/2026-09-18/ADR-GR-10-001.md)) | `PlanningRunner`, `PolicyEnforcingLLMRouter`, `enforce_pre_model_before_structured_inference` (INFERENCE **BLOCKED**) | **ORCHESTRATION**, **AGENTIC** (ACP); **INFERENCE** wiring present but identity **not ADR-compliant** | Planning/agent LLM gates; INFERENCE blocked on `agent_id=""` until GR-10-R2-C1/R1 | Direct adapter bypass = host gap; principal must not come from Evidence |
