...
- **Strategy coverage (GR-10):** **INFERENCE:** PRE_MODEL **public contract** revised (**GR-10-R2-C1** — `principal_id` required); **runtime identity conformance** still **BLOCKED** on **GR-10-R2-R1** ([ADR-GR-10-001](../technical/adr/entries/2026-09-18/ADR-GR-10-001.md)); MSE, Decision-bound effect, HITL, Continuation, and Reliability are **NOT_APPLICABLE** on the inference strategy path (GR-10-R1). **AGENTIC / ORCHESTRATION:** residual capability qualification per §9 matrix (meaningful-side-effect and HITL paths not enterprise-qualified on all production entry points).
...
| **PRE_MODEL** | **PARTIAL** | Governance | `PolicyEngine.evaluate_pre_llm` (`principal_id` required — [ADR-GR-10-001](../technical/adr/entries/2026-09-18/ADR-GR-10-001.md); **GR-10-R2-C1** contract revision) | `PlanningRunner`, `PolicyEnforcingLLMRouter`, `enforce_pre_model_before_structured_inference` | **ORCHESTRATION**, **AGENTIC** (ACP); **INFERENCE** | Planning/agent/inference LLM gates; qualification **BLOCKED** until **GR-10-R2-R1** runtime remediation | Architecture identity source: `principal_id` + optional roster `agent_id` per ADR; runtime conformance **PENDING GR-10-R2-R1** (transitional tenant/lineage fallbacks are not canonical authority); Evidence/Diagnostics do not supply principal; direct adapter bypass = host gap |
