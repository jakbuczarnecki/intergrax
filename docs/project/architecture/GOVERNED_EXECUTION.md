...
- **Strategy coverage (GR-10):** **INFERENCE:** PRE_MODEL identity/public contract and compliant wiring **BLOCKED** on GR-10-R2-C1/R1 ([ADR-GR-10-001](../technical/adr/entries/2026-09-18/ADR-GR-10-001.md)); MSE, Decision-bound effect, HITL, Continuation, and Reliability are **NOT_APPLICABLE** on the inference strategy path (GR-10-R1). **AGENTIC / ORCHESTRATION:** residual capability qualification per §9 matrix (meaningful-side-effect and HITL paths not enterprise-qualified on all production entry points).
...
| **PRE_MODEL** | **PARTIAL** | Governance | `PolicyEngine.evaluate_pre_llm` ( **`REVISION_REQUIRED`** — [ADR-GR-10-001](../technical/adr/entries/2026-09-18/ADR-GR-10-001.md)) | `PlanningRunner`, `PolicyEnforcingLLMRouter`, `enforce_pre_model_before_structured_inference` (INFERENCE qualification **BLOCKED**) | **ORCHESTRATION**, **AGENTIC** (ACP); **INFERENCE** wiring present but identity **not ADR-compliant** | Planning/agent LLM gates; INFERENCE blocked on `agent_id=""` until GR-10-R2-C1/R1 | Architecture identity source: `principal_id` + roster `agent_id` per ADR (**decision PASS**); runtime conformance **PENDING GR-10-R2-R1** (transitional tenant/lineage fallbacks are not canonical authority); Evidence/Diagnostics do not supply principal; direct adapter bypass = host gap |
