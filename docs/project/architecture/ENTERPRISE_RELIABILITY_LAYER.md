...
| **Lifecycle execution handoff** | “Tell execution what to do — don’t do it here.” | After governance, `RecoveryLifecycleHandoffRequest` → `ExecutionLifecyclePort` / `RecoveryLifecycleIntent`; UER mutates lifecycle — see [`RECOVERY_AND_COMPENSATION.md`](RECOVERY_AND_COMPENSATION.md#lifecycle-execution-handoff-boundary-erl-foundation). ERL never applies pause/resume/terminate directly. |
