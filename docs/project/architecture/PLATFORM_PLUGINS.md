...
| **Memory stores** (`intergrax.memory_stores`) | **B** - document convention | Host + `MemoryProfile` | Host materialization resolves platform configuration; integration credentials stay in `IntegrationProfile` for baseline stores | Typed creation context → `create_user_profile_store(context)` / `create_session_storage(context)` / specialized store factories / `create_session_turn_index(context)` | Discouraged for portable plugins | Yes - host materializer invokes factories with bounded contexts | `UserProfileStorePlugin` / `SessionStoragePlugin` / specialized store plugins / `SessionTurnIndexStorePlugin` | Document typed creation-context ownership (MEM-HARDEN-FINAL-1) |
