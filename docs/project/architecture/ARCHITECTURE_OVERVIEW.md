...
- **Multiplayer AI** - extends governed execution toward collaboration among multiple principals (humans, agents, services, and eventually external agents). **Enterprise core through MP-7 is implemented/closed in its certified scopes** (platform/core proof + Tier-3 consumability boundary). External-agent interoperability (MP-8) and advanced product-facing collaboration UX (MP-9) remain future work; full product-facing Multiplayer capability E2E including MP-8 / MP-9 is **not established**. Details: [Multiplayer AI architecture](../capabilities/architecture/MULTIPLAYER_AI.md).
...
- **Autonomous Organization** - explores whether mission, constraints, and resources can drive emergent organizational structure using existing Intergrax execution, governance, decision, memory, and autonomous-work foundations. **Strategic Initiative** — frozen scenario portfolio; architecture evolves through scenario-driven development. **Not** a current platform domain or organizational runtime. Details: [Autonomous Organization](../overview/AUTONOMOUS_ORGANIZATION.md).
