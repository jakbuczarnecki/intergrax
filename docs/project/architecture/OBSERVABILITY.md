...
> [!NOTE]
> **Maturity boundary (FINAL-2 OBS/DIAG SSOT reconciliation @ `a068af8699783641f6cafa2bd4f779853d98c101` — documentation baseline on `development`, not FINAL-3 exact-SHA enterprise qualification):** Core execution evidence (TRACE-1A–1C, ASOF-1/2, BITEMP-1/3), **OBS-ASOF-REBASE**, and **OBS-BITEMP-REBASE** are **implemented and closed** on the qualified harness path. **Five-ID canonical contract:** `RuntimeEvent` requires typed `TaskId`, `RunId`, `AttemptId`, `ExecutionId`, and `EventId` (**PROVEN**). Emit-path coverage for production writers is tracked under **OBS-COVERAGE-1** (P1 **PROVEN**). **Canonical revision ordering provider (TRACE-BITEMP-2)** is **implemented** (SQLite canonical provider; alternate providers remain **TRACE-BITEMP-5**). **E/K/V/S temporal composition** is **implemented** via `HistoricalReconstructionService` (**OBS-BITEMP-REBASE** closed). **Conditional read surfaces:** typed public **TRACE-ASOF-4** / **TRACE-BITEMP-4** query APIs remain optional productization layers. **Platform spine proofs (authoritative, aligned with qualification summary below):** Kafka → worker → execution → diagnostics as **one** external P4 spine **PROVEN** (OBS-DIAG-X4; in-process async spine **P3**); global entry-path zero-bypass **PROVEN** (OBS-DIAG-X3/X3A); surface anti-drift **PROVEN** (X3A). **HITL (split):** **P3** in-process durable rebuild; **P4** OS-process pause/resume (X4); **P4** resume anomaly → durable `Problem` + fresh read (X4A); **external HITL vendor/service NOT_PROVEN**. **Adoption / read (split):** factory composition **PROVEN** (PRODUCT BYPASS = 0); initialized scenario surfaces = 4; universal product/scenario E2E **PARTIAL**; operator read **CORE CONTRACT PROVEN** / **UNIVERSAL HOST EXPOSURE PARTIAL**. **Provider evidence (FINAL-1 / X5A):** platform export semantics proof is separate from catalog telemetry adapters and vendor live qualification — see X5/X5A reconciliation below. **Remaining limitations (explicit):** **OECP** **PLANNED/OPEN**; full **OBS-VENDOR** hardening **OPEN** (OTLP collector slice only); production scale / multi-region **not** certified. **DG-005** process-isolated topology over shared durable evidence is **PROVEN** (scoped — not multi-region/HA). OTLP / vendor backends are **derived export only** — never execution truth. Observability records identity minted by Execution Runtime; it does **not** own Execution identity. Full gap baseline: [`OBS_DIAG_UNIVERSAL_ENTERPRISE_GAP_BASELINE_X1.md`](../maintainers/audits/OBS_DIAG_UNIVERSAL_ENTERPRISE_GAP_BASELINE_X1.md). Qualification anchor OBS-DIAG-X1 @ `19307f1e4c3bfdcdc9ea675f4909383ab1d70878` remains in historical records.
...
**Last reconciled (FINAL-2 OBS/DIAG SSOT) @ documentation baseline `a068af8699783641f6cafa2bd4f779853d98c101` on branch `development`.** This is a **documentation reconciliation baseline**, not FINAL-3 exact-SHA enterprise qualification. Prior code-audit anchor OBS-DIAG-X1 @ `19307f1e4c3bfdcdc9ea675f4909383ab1d70878` remains in qualification records. Documentation commit SHA may differ after docs-only commits; do not treat a docs-only commit as a new code qualification.
...
| **Composition pluginability** | Engine ctor injection vs host replaceability | Engine **PROVEN**; standard host replaceability **PROVEN** (OBS-DIAG-X2 CLOSED) |
...
| P12 HITL interrupt | execution-scoped | same execution unless UEA forks | P3 in-process (`test_obs_universal_spine_hitl_restart_e2e.py`); P4 OS-process pause/resume (OBS-DIAG-X4); X4A resume `Problem`; external HITL vendor **NOT_PROVEN** | **PROVEN** (scoped — external HITL vendor **NOT_PROVEN**) |
