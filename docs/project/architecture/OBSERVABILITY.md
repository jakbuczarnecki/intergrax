...
`RuntimeEventBus.history` is **bounded or disabled** process-local diagnostic state wired through `RuntimeEventHistoryPolicy` / `RuntimeEventHistoryBuffer` (`intergrax.contracts.runtime_event_history`). Custom history providers **must** implement `retention()` with a typed `RuntimeEventHistoryRetention` descriptor (`mode` = `bounded` or `disabled`; finite `capacity` when bounded) and pass composition-time conformance validation — unbounded or mis-declared buffers are rejected at bus wiring. It is **not** canonical execution evidence. Full historical reads, reconstruction, and As-Of projection **must** use `EvidencePersistencePort` (and shared reconstruction readers) — not the bus snapshot. Local ring-buffer eviction **does not** delete persisted evidence and **must not** change publish/record persistence, delivery, or handler dispatch semantics.

`RuntimeEventBus.event_count` and `TaskExecutionMetrics.runtime_events` count events processed on the bus during task execution (Nexus uses a per-`handle_task` baseline on the shared bus). They are **independent** of retained local history size and are **not** canonical evidence counts — use persisted event lists for reconstruction.
