...
| **HITL / restart** | pause / durable rebuild / resume → terminal diagnostics | **P3** in-process rebuild (`test_obs_universal_spine_hitl_restart_e2e.py`); **P4** real OS process pause/resume (OBS-DIAG-X4); external HITL vendor **NOT_PROVEN** (X5) |
...
| HITL full restart proof | **PROVEN (P4 OS process)** | External HITL service / vendor matrix | OBS-DIAG-X5 |
