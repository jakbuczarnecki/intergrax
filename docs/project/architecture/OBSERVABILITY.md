...
**Shutdown:** `close()` sets stop under the admission coordination lock, wakes quota waiters, drains in-flight physical enqueue reservations within `drain_shutdown_timeout_seconds`, enqueues a sentinel, joins the worker, then closes downstream — fail-closed semantics unchanged for callers.
