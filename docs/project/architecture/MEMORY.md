...
| **ContextView source (MP-5F)** | Memory exposes scoped **reference-read** port (`MemoryReferenceReadPort`) — B1 **CLOSED**; MP-5F-B5 adapter (`DefaultMemoryContextSource`) translates refs only — **CLOSED** |
| **MP-5G-C1 reference-read scope** | Default `DefaultMemoryReferenceReader` surface is **tenant + workspace + user/profile** scoped; optional `MemoryScopedResourceRef` exists in the B1 contract but is **not** backed by canonical work-item ownership on `MemoryRecordCanonicalRef` today. Work-item ContextView admission uses explicit MP-5 scope-compatibility policy — Memory candidates never inherit `work_item_id` or `operation_scope` from the ContextView request. |
