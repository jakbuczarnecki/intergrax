...
**Status:** P0 inventory baseline; **W0** strict host capacity guardrails; **W1 FINAL (qualified)** — process-local root admission (W1-A), explicit concurrent work policy (W1-B), absolute global deadline into R1 retry (W1-C). Qualification: [`ENTERPRISE_EXECUTION_SCALE_RESILIENCE_W1_ADMISSION_DEADLINE.md`](../qualification/ENTERPRISE_EXECUTION_SCALE_RESILIENCE_W1_ADMISSION_DEADLINE.md). **W2-A (qualified)** — dependency isolation inventory; **W2-ADR (Accepted)** — [`ADR_ENTERPRISE_DEPENDENCY_CONCURRENCY_ADMISSION.md`](ADR_ENTERPRISE_DEPENDENCY_CONCURRENCY_ADMISSION.md) + `intergrax/contracts/dependency_concurrency_admission.py`; **W2-B1 implemented** (`LocalDependencyConcurrencyAdmission`, process-local); **W2-B2 DONE (qualified, process-local)** — [`ADR_ENTERPRISE_TOOL_DEPENDENCY_ATTEMPT_BOUNDARY.md`](ADR_ENTERPRISE_TOOL_DEPENDENCY_ATTEMPT_BOUNDARY.md) (`DependencyAttemptExecutionBoundary`, Option B; composition opt-in on `RuntimeToolInvoker`); **W2-B3 DONE (qualified, process-local)** — LLM provider dependency admission via shared `DependencyAttemptExecutionBoundary` + `set_llm_provider_dependency_boundary` / `LLMAdapterRegistry.create` wiring; canonical seams `_execute`, `_execute_streaming`, `_provider_dependency_attempt`. **W2-C DONE (qualified, process-local)** — `RetryBudgetPort` + `ProviderRateLimitPort`; `execute_with_resilience` retry loop applies budget → rate limit → circuit breaker → physical attempt (admission inside `LLMAdapter._run_physical_provider_attempt`); bounded `Retry-After` via `max_retry_after_sec`. Qualification: [`ENTERPRISE_EXECUTION_SCALE_RESILIENCE_W2_C_RETRY_RATE_CONTAINMENT.md`](../qualification/ENTERPRISE_EXECUTION_SCALE_RESILIENCE_W2_C_RETRY_RATE_CONTAINMENT.md). Inventory: [`ENTERPRISE_EXECUTION_SCALE_RESILIENCE_W2_DEPENDENCY_ISOLATION_INVENTORY.md`](../qualification/ENTERPRISE_EXECUTION_SCALE_RESILIENCE_W2_DEPENDENCY_ISOLATION_INVENTORY.md).
...
3. **Provider/tool bulkheads** — W2-ADR defines `DependencyConcurrencyAdmissionPort`; W2-B implements process-local admission and wires `LLMAdapter._execute` and `RuntimeToolInvoker` external boundary (not circuit breaker reuse).
