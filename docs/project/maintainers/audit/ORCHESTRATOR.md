...
1. **Read only:** `docs/project/architecture/<DOMAIN>.md` · `docs/project/maintainers/plans/<DOMAIN>.md` · `docs/project/maintainers/audit/<DOMAIN>.md`
...
4. **Classify** findings P0–P4 ([`LAYER_COMPLETION_MODE.md`](../../technical/guides/LAYER_COMPLETION_MODE.md) §Step 3).
5. **Write** `results/<run_id>/<DOMAIN>.md` from [`TEMPLATE_DOMAIN_RESULT.md`](TEMPLATE_DOMAIN_RESULT.md).
6. **Plan sync (allowed):**
   - P2–P4 → add/update rows in `docs/project/maintainers/plans/<DOMAIN>.md` backlog register.
   - P0/P1 → add rows + set `p0_open` / `p1_open` in `progress.json`; **do not implement**.
   - **Do not** edit `docs/project/architecture/<DOMAIN>.md` unless operator approved contract change (note `needs_architecture_sync` in result).
