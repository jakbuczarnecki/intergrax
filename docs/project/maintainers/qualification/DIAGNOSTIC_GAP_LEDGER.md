...
| **Related implementation** | `f2198be56` — ProblemId scope discovery (Slice 1); `causal_transport_scope` — transport reference scope discovery (Slice 2); `runtime_event_scope` — EventId exact RuntimeEvent scope discovery (Slice 3); reassessment R1 — [`DG_002_SCOPE_DISCOVERY_REASSESSMENT.md`](DG_002_SCOPE_DISCOVERY_REASSESSMENT.md) |
| **Qualification result after fix** | **Core architecture qualified.** Functional semantics qualified for ProblemId, transport reference, and EventId. **Enterprise closure blocked** by transport bounded read: `CausalEvidencePersistence.list_for_transport_task()` has no public `limit`/`cursor`; implementations materialize all matching evidence before return; `candidate_limit` bounds projection only, not storage read. **Supported core references:** ProblemId, transport task reference, EventId. **Deferred extension:** correlation-based discovery pending explicit canonical correlation identity/cardinality/persistence contract. **Deferred / not core:** ExecutionId discovery. Does not close DG-004 (transport causal continuity in reconstruction) or DG-005 (topology qualification). **Next:** `DG-002-TRANSPORT-BOUNDED-READ-HARDENING-AUDIT-R1` |
