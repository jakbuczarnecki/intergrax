...
| **Related implementation** | HOST-DIAG-2 typed `DiagnosticSubjectRef` + `signal_subjects` on `DiagnosticOrchestrationRequest`; HOST-DIAG-3 `HostedApplicationDiagnosticEventPublisher` for bounded `APPLICATION_FAILED` projection when product composition supplies tenant binding; **R1 audit** — [`DG_001_PRE_EXECUTION_STARTUP_FAILURE_VISIBILITY_AUDIT.md`](DG_001_PRE_EXECUTION_STARTUP_FAILURE_VISIBILITY_AUDIT.md); **DG-001B R1 architecture freeze** — [`DG_001B_WORKER_BOOTSTRAP_TYPED_FAILURE_PRODUCER_ARCHITECTURE.md`](DG_001B_WORKER_BOOTSTRAP_TYPED_FAILURE_PRODUCER_ARCHITECTURE.md); **DG-001B R5 real qualification** — [`DG_001B_REAL_CONTROLLED_BOOTSTRAP_FAILURE_QUALIFICATION_R5.md`](DG_001B_REAL_CONTROLLED_BOOTSTRAP_FAILURE_QUALIFICATION_R5.md); **DG-001B R6 final closure** — [`DG_001B_FINAL_CLOSURE_R6.md`](DG_001B_FINAL_CLOSURE_R6.md); **DG-001D R1 architecture freeze** — [`DG_001D_SUPERVISOR_PRE_ENGINE_FAILURE_ARCHITECTURE_AUDIT_R1.md`](DG_001D_SUPERVISOR_PRE_ENGINE_FAILURE_ARCHITECTURE_AUDIT_R1.md); **DG-001D R3 HOST-DIAG-3 conformance** — [`DG_001D_SUPERVISOR_HOST_DIAG_3_CONFORMANCE_R3.md`](DG_001D_SUPERVISOR_HOST_DIAG_3_CONFORMANCE_R3.md); **DG-001D R4 real qualification** — [`DG_001D_REAL_SUPERVISOR_PRE_ENGINE_FAILURE_QUALIFICATION_R4.md`](DG_001D_REAL_SUPERVISOR_PRE_ENGINE_FAILURE_QUALIFICATION_R4.md); **DG-001D final closure** — [`DG_001D_FINAL_CLOSURE.md`](DG_001D_FINAL_CLOSURE.md); **remaining surfaces prioritization** — [`DG_001_REMAINING_PRE_EXECUTION_SURFACES_PRIORITIZATION.md`](DG_001_REMAINING_PRE_EXECUTION_SURFACES_PRIORITIZATION.md) |
| **Qualification result after fix** | **Partial (DG-001 overall).** **Qualified sub-slice: DG-001B worker bootstrap visibility = CLOSED / QUALIFIED (R6).** Canonical authority: R5-R1-A attempt `dg001b-r5-r1-a-20260907132655` @ `aafeff9569b1270936da0fbc384d783e14fb3a72`. **Qualified sub-slice: DG-001D supervisor pre-engine visibility = CLOSED / QUALIFIED (final closure).** Canonical authority: R4 attempt `dg001d-r4-20260907145456` @ `a4b04eeeb5956408e847864773337cd396f50f68`. R1 PASS; R2 PASS @ `4dff9d136085fe46fbffc342d1bfc690ce7a8d85`; R3 PASS @ `70c342d4ccd0d65299bf433fbf7b74818e9f1b45`; R4 PASS — real supervisor subprocess, proof-owned failing `HostedApplicationEngineFactory`, Elasticsearch observability, Mongo durable persistence, separate-process `DiagnosticReadService`, recurrence `1 Problem / 2 occurrences`. **Qualified sub-slice: DG-001A hosted default wiring coverage = CLOSED / QUALIFIED.** Canonical authority: [`DG_001A_HOSTED_DEFAULT_HOST_DIAG_3_QUALIFICATION.md`](DG_001A_HOSTED_DEFAULT_HOST_DIAG_3_QUALIFICATION.md) @ qualification session on `development`. Platform default `run_hosted_application` is observability-only by design; canonical product `event_publisher_factory` + `build_hosted_application_diagnostic_event_publisher` qualifies Supervisor → HOST-DIAG-3 → Diagnostics without bypass. **Qualified sub-slice: DG-001C public launcher/bootstrap visibility = CLOSED / QUALIFIED (limit identified).** Canonical authority: [`DG_001C_PUBLIC_LAUNCHER_BOOTSTRAP_QUALIFICATION.md`](DG_001C_PUBLIC_LAUNCHER_BOOTSTRAP_QUALIFICATION.md) @ qualification session on `development`. Public transport wrappers delegate via canonical `uv run --project … python`; PRE-PYTHON / PYTHON-BOOTSTRAP failures do not reach Central Diagnostics by design (no identity/tenant/publisher); HOST-DIAG-3 boundary begins at product/host composition. **Remaining DG-001 surfaces (ordered):** **1** B0–B5-only failures before diagnostic prerequisites. LKW background worker is HOST-DIAG-3 wired; generic default foreground runner remains observability-only unless product overrides factory. |
...
| **First failed/unknown boundary** | ~~Enterprise-scale transport causal-evidence lookup (unbounded public persistence read)~~ **removed** — transport bounded read qualified R3 |
| **Root cause automatically proven** | NO |
| **Missing canonical evidence** | ~~Bounded transport causal-evidence lookup contract~~ **addressed** — `page_for_transport_task` + bounded provider scan |
| **Missing diagnostic capability** | ~~Enterprise-qualified transport scope discovery under high fan-out evidence cardinality~~ **addressed** for transport reference path |
...
| **Related implementation** | `f2198be56` — ProblemId scope discovery (Slice 1); `causal_transport_scope` — transport reference scope discovery (Slice 2); `runtime_event_scope` — EventId exact RuntimeEvent scope discovery (Slice 3); reassessment R1 — [`DG_002_SCOPE_DISCOVERY_REASSESSMENT.md`](DG_002_SCOPE_DISCOVERY_REASSESSMENT.md); bounded-read audit R1 — [`DG_002_TRANSPORT_BOUNDED_READ_HARDENING_AUDIT.md`](DG_002_TRANSPORT_BOUNDED_READ_HARDENING_AUDIT.md); transport bounded read R3 — [`DG_002_TRANSPORT_BOUNDED_READ_R3_QUALIFICATION.md`](DG_002_TRANSPORT_BOUNDED_READ_R3_QUALIFICATION.md); **final closure R4** — [`DG_002_FINAL_CLOSURE.md`](DG_002_FINAL_CLOSURE.md) |
| **Qualification result after fix** | **CLOSED / QUALIFIED.** Core Diagnostic Scope Discovery is enterprise-qualified for ProblemId, transport task reference, and EventId. Three-provider pluginability qualified; tenant isolation hard; identity minting none; transport bounded read hard max 1000. **Supported core references:** ProblemId, transport task reference (`provider` + `transport_task_id`), EventId. **Deferred extension:** correlation-based discovery. **Deferred / not core:** ExecutionId discovery. DG-004 CLOSED / QUALIFIED. DG-005 UNCHANGED. |
...
| **Priority** | P1 |
| **Status** | **CLOSED / QUALIFIED** |
| **Related implementation** | DG-A fix `24506c3c14e30984d78b7b22c5cd4c42e711d125`; APP-DIAG baseline program (see OBSERVABILITY plan § Phase APP-DIAG); **R1 root-cause audit** — [`DG_004_ASYNC_CAUSAL_CONTINUITY_ROOT_CAUSE_AUDIT.md`](DG_004_ASYNC_CAUSAL_CONTINUITY_ROOT_CAUSE_AUDIT.md); **R2 real qualification** — [`DG_004_REAL_ASYNC_CAUSAL_CONTINUITY_R2_QUALIFICATION.md`](DG_004_REAL_ASYNC_CAUSAL_CONTINUITY_R2_QUALIFICATION.md); **R3 final closure** — [`DG_004_FINAL_CLOSURE.md`](DG_004_FINAL_CLOSURE.md) |
| **Qualification result after fix** | **CLOSED / QUALIFIED.** Canonical async transport causal continuity is qualified end-to-end for the platform background-admission mechanism. Real LKW Kafka E0–E6 qualification proved: transport → execution identity → required causal evidence → durable cross-process persistence → execution read → ExecutionReconstructor → DiagnosticOrchestrator. Exact File Watcher replay is not a core blocker because the File Watcher path conforms to the same canonical admission/evidence mechanism. See R3 closure doc. |
