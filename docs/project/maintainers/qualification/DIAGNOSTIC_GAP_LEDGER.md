...
| **Related implementation** | HOST-DIAG-2 typed `DiagnosticSubjectRef` + `signal_subjects` on `DiagnosticOrchestrationRequest`; HOST-DIAG-3 `HostedApplicationDiagnosticEventPublisher` for bounded `APPLICATION_FAILED` projection when product composition supplies tenant binding; **R1 audit** — [`DG_001_PRE_EXECUTION_STARTUP_FAILURE_VISIBILITY_AUDIT.md`](DG_001_PRE_EXECUTION_STARTUP_FAILURE_VISIBILITY_AUDIT.md); **DG-001B R1 architecture freeze** — [`DG_001B_WORKER_BOOTSTRAP_TYPED_FAILURE_PRODUCER_ARCHITECTURE.md`](DG_001B_WORKER_BOOTSTRAP_TYPED_FAILURE_PRODUCER_ARCHITECTURE.md); **DG-001B R5 real qualification** — [`DG_001B_REAL_CONTROLLED_BOOTSTRAP_FAILURE_QUALIFICATION_R5.md`](DG_001B_REAL_CONTROLLED_BOOTSTRAP_FAILURE_QUALIFICATION_R5.md) |
| **Qualification result after fix** | **Partial (DG-001 overall).** **DG-001B REAL QUALIFICATION PASSED (R5-R1-A):** corrected generic `BackgroundWorkerConstructor` seam; qualification-owned child process (`scripts/proof/dg001b_r5_worker_child.py`) invokes canonical production composition with injected failing constructor; controlled B6 `worker_construction` failure; Elasticsearch observability export; HOST-DIAG-3 projection; durable Mongo Problem; separate-process `DiagnosticReadService` read; `APPLICATION_INSTANCE` subject; no execution identity; identity fidelity 100%. Historical R5-A remains functionally pass / architectural correction required. **Remaining DG-001 surfaces:** supervisor pre-engine, public launcher/bootstrap, earlier B0–B5-only failures, default un-wired production paths without diagnostic publisher. |
