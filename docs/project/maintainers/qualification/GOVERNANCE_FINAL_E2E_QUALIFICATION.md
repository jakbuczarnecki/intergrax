...
Authoritative applicability vs coverage: `GR10_INFERENCE_CAPABILITY_SEMANTICS` in `tests/qualification/governance/strategy/catalog.py` and architecture §9 matrix in `GOVERNED_EXECUTION.md`. **GR-10-FINAL (2026-09-18):** INFERENCE PRE_MODEL **QUALIFIED** on production `InferenceExecutor` path; **GR-10-R4 (2026-09-18):** INFERENCE root admission **NOT_APPLICABLE** (no independent production root — internal `StrategyExecutionRouter` delegate only). Active strategy-wide blockers are **partial matrix rows** (inner governance, governance evidence adoption) and residual **AGENTIC** coverage beyond **GR-10-R3** (UAEP kernel policy authority **CANDIDATE CLOSED** — G3B deny proof). HITL, Reliability, MSE, Continuation, Decision-bound effect, and root admission are **NOT_APPLICABLE** on the inference strategy path — not open gaps.
