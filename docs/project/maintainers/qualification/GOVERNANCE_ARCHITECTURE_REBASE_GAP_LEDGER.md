...
| GOV-GAP-007 | P1 | Evidence | Legacy `audit_payload` remains diagnostic-only | Typed `GovernanceDecisionEvidenceFact` + `GovernanceEvidencePersistencePort` on root admission + MSE spine | Residual inner evaluation points / full strategy matrix | Duplicate emission guarded by idempotency key | Extend emission to remaining GEPs under GR-10/13 | Governance + Evidence | GR-8 | `governed_execution_governance_evidence.py`, `governance_evidence_persistence.py` | **CANDIDATE CLOSED — spine complete**; residual GEP adoption → GR-10/13; independent audit required |
...
| GR-8 | Governance Evidence Integration | **CANDIDATE CLOSED** — awaiting independent GitHub audit | Typed facts + pluginable persistence; root + MSE wired | GR-1 | — | Yes | `test_gr8_governance_evidence_spine.py`, `test_gr8_governance_evidence_architecture_gates.py` |
