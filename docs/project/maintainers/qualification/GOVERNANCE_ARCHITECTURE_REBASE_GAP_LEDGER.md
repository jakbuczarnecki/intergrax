...
| GOV-GAP-005 | P1 | Execution boundary | GR-3 + GR-3-R1: contract-only `MeaningfulSideEffectAuthorizationBoundary`; default guard wired only in `intergrax/runtime/governance/meaningful_side_effect_authorization_composition.py`; four-ID inner enforcement + `authorize_and_execute` allowlist gate | Side effects only inside admitted Execution | Residual Reliability delivery semantics (GR-7) | Hidden concrete guard in policy consumer | GR-3-R1 explicit composition + architecture import gate | GR-1, GR-2 | GR-7 | `intergrax/contracts/canonical_inner_governance.py`, `meaningful_side_effect_authorization.py`, `meaningful_side_effect_authorization_composition.py`, `test_gr3_*`, `test_gr3_r1_*` | **CANDIDATE CLOSED** (gap: GR-7; **GR-3-R1 VERIFIED** — H9.2C independent audit at ledger audit HEAD) |
...
| GR-3 | Canonical Inner Enforcement | **VERIFIED** (GR-3-R1); **not CLOSED** (PG-FIX-A harness drift) | Active Execution four-ID binding before meaningful side effects; no second executor | GR-1, GR-2 | PG-FIX-A membership proofs | Yes | `test_gr3_*`, `test_gr3_r1_*`, architecture AST gates |
| GR-4 | Policy Resolution & Catalog Requalification | **IN PROGRESS** (GR-3-R1 audit unblocks qualification) | Close PG-FIX-B/D qualification gaps | GR-3 | G2C, PG-FIX-B/D | Yes | Precedence + catalog tests |
