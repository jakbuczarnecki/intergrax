...
| GOV-GAP-008 | P1 | Strategy coverage | G3B Nexus/UAEP-heavy COVERED rows | Matrix for INFERENCE/AGENTIC/ORCHESTRATION | “Nexus works” ≠ platform proof | **INFERENCE PRE_MODEL** wired but **identity BLOCKED** (`agent_id=""`; `evaluate_pre_llm` **REVISION_REQUIRED** per [ADR-GR-10-001](../../technical/adr/entries/2026-09-18/ADR-GR-10-001.md)) | `tests/qualification/governance/strategy/` + matrix §9 | GR-10 | GR-10-R2-C1 → GR-10-R2-R1 | `GOVERNED_EXECUTION.md` §G3B | **PARTIAL** |
...
| GR-10-R2-ADR1 | PRE_MODEL identity + intake boundary | **CLOSED** (docs/ADR) | [ADR-GR-10-001](../../technical/adr/entries/2026-09-18/ADR-GR-10-001.md); intake **KEEP_REQUIRED**; carrier **ActiveExecutionGovernanceIdentity** | GR-10-R1 | — | No (ADR only) | `test_gr10_adr1_pre_model_identity_boundary.py` |
