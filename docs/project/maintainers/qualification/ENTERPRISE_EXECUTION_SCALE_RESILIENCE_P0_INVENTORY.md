...
| DeclarativeToolInvoker | `ThreadPoolExecutor()` | Nexus tools | tool invoke isolation | shared pool per invoker | no | per tool call | process-local | default pool size | invoke timeout paths | partial | implicit queue | pool exhaustion / slow tools block workers |
