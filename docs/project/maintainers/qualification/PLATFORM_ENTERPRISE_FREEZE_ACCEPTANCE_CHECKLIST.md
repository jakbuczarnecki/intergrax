...
| FRZ-CTR-01 | Contracts / abstraction | consumers depend on contracts | EBH-2*, EBH-3, EBH-4 | OPEN | EBH-2F-R2-R1-R1-R1-R1-R1 independently closed (`be8c6aaec48431d74dc93572fa27e34433e7c4c5`); EBH-2F-R2 parent recert on HEAD `26f529e8cac295a9791f2b5763ae9d43da7633c5` — evidence contribution only. |
| FRZ-CTR-02 | Contracts / abstraction | concrete provider leakage = 0 | EBH-2*, EBH-3, EBH-4 | OPEN | — |
| FRZ-CTR-03 | Contracts / abstraction | no pseudo-contract dicts | EBH-2*, EBH-3, EBH-4 | OPEN | Historical `EBH-2F-R2-BLOCKER` (`8c89bdfa…`) superseded by remediation; registration uses explicit `contract_specs` / `integration_contract_specs()` only (`plugin_register.py` gate). Parent recert EBH-2F-R2 CURRENT. |
| FRZ-CTR-04 | Contracts / abstraction | public/internal contracts classified | EBH-2*, EBH-3, EBH-4 | OPEN | — |
| FRZ-CTR-05 | Contracts / abstraction | contract responsibility narrow/cohesive | EBH-2*, EBH-3, EBH-4 | OPEN | Accepted child + parent recert: `IntegrationPlugin` / `IntegrationFactory` owned by `catalog_factory.py`; `registry_v2` imports canonical alias — FRZ OPEN at platform scope. |
| FRZ-CTR-06 | Contracts / abstraction | contracts do not encode concrete vendor/runtime implementation | EBH-2*, EBH-3, EBH-4 | OPEN | — |
| FRZ-TYP-01 | Strong typing | semantic boundaries strongly typed | EBH-2* | OPEN | Parent recert HEAD `26f529e8…`: typed `IntegrationPlugin.create_integration` + canonical `IntegrationFactory` — evidence contribution; criterion remains OPEN globally. |
| FRZ-TYP-02 | Strong typing | Any at semantic boundaries = 0 unless evidence-backed transport reason | EBH-2* | OPEN | Child closed `be8c6aa…`; parent recert revalidated catalog/profile result types on HEAD `26f529e8…`. |
| FRZ-TYP-03 | Strong typing | generic object semantic seams = 0 | EBH-2* | OPEN | Child closed `be8c6aa…`; `_require_platform_integration_contract` vs `_require_category_integration_instance` on current HEAD. |
...
| FRZ-REG-02 | Regression / qualification infrastructure | every corrected blocker has regression gate | QUAL-X | OPEN | EBH-2F-R2: `test_ebh_2f_r2_integration_plugin_contract_typing_gate.py` green on parent recert HEAD `26f529e8…` (16 gate tests). |
...
| FRZ-REG-09 | Regression / qualification infrastructure | critical invariant protected by code/test, not docs only | QUAL-X | OPEN | R2 gate AST proofs for `resolve` / `resolve_from_profile` / `IntegrationFactory` + pyright subprocess on resolver (parent recert HEAD `26f529e8…`). |
...
| EBH-2F-R2-PARENT-RECERT | `26f529e8cac295a9791f2b5763ae9d43da7633c5` | Cursor AI final parent recertification: full R2 closed-world inventory, historical blocker revalidation, 543 targeted tests + R2 architecture gate + `pyright` on `factory.py`; integration tree unchanged vs `be8c6aa…`. Status **READY FOR AUDIT** (not independent CLOSED). | FRZ-BND-02..06, FRZ-OWN-01..05, FRZ-CTR-01..06, FRZ-TYP-01..06, FRZ-PLG-01..08, FRZ-REG-02, FRZ-REG-09, FRZ-RPL-02..04 | Evidence contribution only; all FRZ rows remain OPEN. |
