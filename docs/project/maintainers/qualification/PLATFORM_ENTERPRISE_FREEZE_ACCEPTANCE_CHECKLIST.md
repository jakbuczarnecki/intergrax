...
| FRZ-CTR-01 | Contracts / abstraction | consumers depend on contracts | EBH-2*, EBH-3, EBH-4 | OPEN | EBH-2F-R2-R1-R1-R1-R1-R1 independently closed (`be8c6aa…`); EBH-2F-R2 parent recert HEAD `26f529e8…` rejected residual semantic `Any` on `resolve_typed.resolve_contract()` — tracked EBH-2F-R2-R2. |
| FRZ-CTR-02 | Contracts / abstraction | concrete provider leakage = 0 | EBH-2*, EBH-3, EBH-4 | OPEN | — |
| FRZ-CTR-03 | Contracts / abstraction | no pseudo-contract dicts | EBH-2*, EBH-3, EBH-4 | OPEN | Historical `EBH-2F-R2-BLOCKER` superseded by remediation; registration uses explicit `contract_specs` / `integration_contract_specs()` only (`plugin_register.py` gate). EBH-2F-R2 BLOCKED pending R2-R2. |
| FRZ-CTR-04 | Contracts / abstraction | public/internal contracts classified | EBH-2*, EBH-3, EBH-4 | OPEN | Parent recert `26f529e8…`: generic `resolve_contract()` overload/impl returned semantic `Any` vs profile-aware `CategoryIntegrationInstance` — EBH-2F-R2-R2. |
| FRZ-CTR-05 | Contracts / abstraction | contract responsibility narrow/cohesive | EBH-2*, EBH-3, EBH-4 | OPEN | Accepted child + parent recert: `IntegrationPlugin` / `IntegrationFactory` owned by `catalog_factory.py`; generic resolution helper result typing gap on `resolve_typed.py` — EBH-2F-R2-R2. |
| FRZ-CTR-06 | Contracts / abstraction | contracts do not encode concrete vendor/runtime implementation | EBH-2*, EBH-3, EBH-4 | OPEN | — |
| FRZ-TYP-01 | Strong typing | semantic boundaries strongly typed | EBH-2* | OPEN | Parent recert HEAD `26f529e8…`: `resolve_typed.resolve_contract` semantic result still `Any` — EBH-2F-R2-R2. |
| FRZ-TYP-02 | Strong typing | Any at semantic boundaries = 0 unless evidence-backed transport reason | EBH-2* | OPEN | Child closed `be8c6aa…`; parent recert found semantic `Any` on generic integration resolution helper (`resolve_contract` return). |
| FRZ-TYP-03 | Strong typing | generic object semantic seams = 0 | EBH-2* | OPEN | Child closed `be8c6aa…`; generic helper no-expected overload must be `CategoryIntegrationInstance` not `Any` — EBH-2F-R2-R2. |
...
| FRZ-TYP-06 | Strong typing | type-ignore/cast cannot mask architecture mismatch | EBH-2* | OPEN | EBH-2F-R2-R2: closure must not use cast/type-ignore on `resolve_contract` result; AST gate enforces. |
...
| FRZ-REG-02 | Regression / qualification infrastructure | every corrected blocker has regression gate | QUAL-X | OPEN | EBH-2F-R2-R2 extends R2 gate with `resolve_typed.py` AST + pyright proofs for `resolve_contract`. |
...
| FRZ-REG-09 | Regression / qualification infrastructure | critical invariant protected by code/test, not docs only | QUAL-X | OPEN | R2 gate: resolver + `resolve_typed.resolve_contract` overload/impl annotations; pyright on `factory.py` + `resolve_typed.py`. |
...
| EBH-2F-R2-R2 | — | CURRENT — generic integration resolution helper strong typing closure (`resolve_contract` → `CategoryIntegrationInstance` / `T`); Cursor AI evidence pending independent exact-SHA audit. | FRZ-CTR-01, FRZ-CTR-04, FRZ-CTR-05, FRZ-TYP-01, FRZ-TYP-02, FRZ-TYP-03, FRZ-TYP-06, FRZ-OWN-02, FRZ-OWN-05, FRZ-PLG-01, FRZ-PLG-02, FRZ-REG-02, FRZ-REG-09 | Evidence contribution only; all FRZ rows remain OPEN. |
