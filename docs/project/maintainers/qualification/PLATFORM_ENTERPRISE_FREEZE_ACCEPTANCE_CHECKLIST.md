...
| FRZ-CTR-01 | Contracts / abstraction | consumers depend on contracts | EBH-2*, EBH-3, EBH-4 | OPEN | EBH-2F independently **CLOSED** (`765fabe…`); R2 (`f7fabab…`); EBH-2G CURRENT — RAG retrieval contracts audited, typing remediation **EBH-2G-R1** BLOCKED. |
...
| FRZ-REG-09 | Regression / qualification infrastructure | critical invariant protected by code/test, not docs only | QUAL-X | OPEN | EBH-2F CLOSED (`765fabe…`): integration gates + 559 tests; EBH-2G finding: `rag-guard.yml` omits `search_provider` / `document_parser` / `rerank_provider` / `graph_store` paths — extend after ownership-confirmed scope (**EBH-2G-R1**). |
