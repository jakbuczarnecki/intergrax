...
LKW readiness status: Slack `slack_conversation` and Microsoft Graph
`teams_chat` are **`LKW_READY`** for their scoped Durable, Indexed and Live
paths. Microsoft Graph `mail` is **`NEAR_READY`** for the bounded Durable and
Indexed bridge: its provider materializer, canonical `KnowledgeDocument`
mapping and generic index handoff are proven, while the complete
connection→discovery→binding→sync→Search/Ask→Live application E2E remains
pending. This means the provider-neutral connection/factory lifecycle, restart
rehydration and bounded LKW application proof are wired and tested;
**`LKW_READY` is not commercially GA**. For Graph Teams Chat, item/per-user
ACL semantics remain unproven/limited, content/history is bounded, mentions,
reactions, attachments and hosted content remain excluded where not supported,
and commercial GA/SLA is not implied.
...
| Microsoft Graph | `mail` | ACCEPTED | IMPLEMENTED_UNREVIEWED | IMPLEMENTED_UNREVIEWED | ACCEPTED | DURABLE, INDEXED, LIVE | adapter + provider materializer + generic bridge + Graph live bundle | NEAR_READY / FOCUSED_TESTS | folder-scoped delta/removal | provider, tenant, source binding; item/per-user UNPROVEN | bounded body/metadata projection; thread messages, attachment inventory and attachment bytes excluded; full application E2E pending | `test_msgraph_mail_knowledge_adapter.py`, `test_msgraph_mail_knowledge_sync.py`, `test_connected_source_materializer.py`, `test_msgraph_mail_live.py` |
...
| Microsoft Graph | `ms365_graph / collaboration_suite` | `mail` | `ACCEPTED` | `IMPLEMENTED_UNREVIEWED` | YES | YES | YES | YES | YES | YES | FOCUSED_TESTS | Provider-owned Mail materialization now feeds the generic application sink/index bridge; the complete source-specific application Search/Ask/Live E2E remains unproven. | `PARTIAL` | YES | YES | YES | YES | YES | FOCUSED_TESTS | Folder-scoped reconciliation and application sink wiring exist; full restart-safe application proof remains pending. | `ACCEPTED / CLOSED` | YES | YES | YES | YES | YES | YES | YES | FOCUSED_TESTS | Mail live handler and registration remain bounded; body projection is available, while thread messages, attachment inventory and attachment bytes remain deferred. | NONE | `MSGRAPH-KNOWLEDGE-ADAPTERS-1B-MAIL`; `MSGRAPH-KNOWLEDGE-LIVE-CAPABILITY-1B-MAIL`; `test_connected_source_materializer.py` | Complete the bounded Mail application E2E without changing folder/removal semantics. |
