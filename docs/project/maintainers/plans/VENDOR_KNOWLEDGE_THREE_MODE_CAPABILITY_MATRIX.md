...
Google Workspace `calendar` is **`NEAR_READY`**: its Durable and Indexed
paths are accepted through the shared provider materializer and generic
application bridge, while Live remains intentionally `UNSUPPORTED`.
Google Workspace `docs` and `sheets` are **`NEAR_READY`** with accepted Durable
and bounded structured Indexed paths. Google Workspace `drive` remains
...
| Google Workspace | `docs` | ACCEPTED | ACCEPTED | ACCEPTED | UNSUPPORTED | DURABLE, INDEXED | adapter + provider materializer + generic bridge | FOCUSED_TESTS / NEAR_READY | snapshot absence is not authoritative; no invented tombstones | provider, tenant, source binding; item ACL UNPROVEN | known-document scope; structured tabs/segments are bounded; no broad discovery, organization-wide traversal, authoritative deletion, item ACL projection or Live handler | `test_google_workspace_docs_knowledge_adapter.py`, `test_google_workspace_docs_knowledge_sync.py`, `applications/local_workspace_application/tests/workspaces/test_connected_source_materializer.py`, `tests/unit/runtime/vendor_knowledge/test_provider_coverage.py` |
| Google Workspace | `sheets` | ACCEPTED | ACCEPTED | ACCEPTED | UNSUPPORTED | DURABLE, INDEXED | adapter + provider materializer + generic bridge | FOCUSED_TESTS / NEAR_READY | snapshot absence is not authoritative; no invented tombstones | provider, tenant, source binding; item ACL UNPROVEN | known-spreadsheet scope; bounded structured/tabular values; formulas are not evaluated, XLSX extraction is not claimed, arbitrary ranges and item ACL projection are excluded | `test_google_workspace_sheets_knowledge_adapter.py`, `test_google_workspace_sheets_knowledge_sync.py`, `applications/local_workspace_application/tests/workspaces/test_connected_source_materializer.py`, `tests/unit/runtime/vendor_knowledge/test_provider_coverage.py` |
| Google Workspace | `calendar` | ACCEPTED | ACCEPTED | ACCEPTED | UNSUPPORTED | DURABLE, INDEXED | adapter + provider materializer + generic bridge | FOCUSED_TESTS | cancellation/tombstone and reconciliation semantics remain source-owned; snapshot absence is not authoritative | provider, tenant, source binding; item ACL UNPROVEN | bounded structured event projection; attachment bytes, external document bodies, complete recurrence expansion, conference transcripts, historical versions and organization-wide attendee ACLs excluded; Live intentionally unsupported; commercial GA/SLA not implied | `test_google_workspace_calendar_knowledge_adapter.py`, `test_google_workspace_calendar_knowledge_sync.py`, `test_connected_source_materializer.py`, `test_provider_coverage.py` |
...
| Google Workspace | `google_workspace / collaboration_suite` | `drive` | `ACCEPTED` | `FOUNDATION_ONLY` | YES | NO | NO | NO | NO | NO | UNPROVEN | Drive content is `BINARY` for blobs and Google-native exports; folders, shortcuts and unsupported native types have no truthful text projection. Exact blocker: `REQUIRES_GENERIC_BINARY_CONTENT_EXTRACTION_CAPABILITY`. | `ACCEPTED` | YES | YES | YES | YES | YES | FOCUSED_TESTS | Inventory plus explicit change/tombstone reconciliation; ordinary absence is not deletion. | `FOUNDATION_ONLY` | YES | NO | YES | YES | YES | YES | NO | UNPROVEN | No Google Drive live handler or application invocation. | NONE | `test_google_workspace_drive_knowledge_adapter.py`; `test_google_workspace_drive_knowledge_sync.py` | Preserve the binary boundary and explicit tombstone semantics; do not add a Drive materializer without text extraction. |
| Google Workspace | `google_workspace / collaboration_suite` | `docs` | `ACCEPTED` | `ACCEPTED` | YES | YES | YES | YES | NO | YES | FOCUSED_TESTS | Provider-owned bounded structured tabs/segments feed the generic KnowledgeDocument/index bridge; known-document scope remains exact and no authoritative deletion is invented. | `ACCEPTED` | YES | YES | YES | YES | YES | FOCUSED_TESTS | One known-document reconciliation; broad discovery and deletion authority remain absent. | `FOUNDATION_ONLY` | YES | NO | YES | YES | YES | NO | UNPROVEN | No Google Docs live handler or application invocation. | NONE | `test_google_workspace_docs_knowledge_adapter.py`; `test_google_workspace_docs_knowledge_sync.py`; `applications/local_workspace_application/tests/workspaces/test_connected_source_materializer.py` | Preserve exact known-document scope, bounded content and ACL limitations. |
| Google Workspace | `google_workspace / collaboration_suite` | `sheets` | `ACCEPTED` | `ACCEPTED` | YES | YES | YES | YES | NO | YES | FOCUSED_TESTS | Provider-owned bounded tabular values feed the generic KnowledgeDocument/index bridge; formulas are not evaluated and workbook-wide discovery is not claimed. | `ACCEPTED` | YES | YES | YES | YES | YES | FOCUSED_TESTS | One known-spreadsheet reconciliation; broad discovery and deletion authority remain absent. | `FOUNDATION_ONLY` | YES | NO | YES | YES | YES | NO | UNPROVEN | No Google Sheets live handler or application invocation. | NONE | `test_google_workspace_sheets_knowledge_adapter.py`; `test_google_workspace_sheets_knowledge_sync.py`; `applications/local_workspace_application/tests/workspaces/test_connected_source_materializer.py` | Preserve exact known-spreadsheet scope, bounded values, formula/XLSX boundaries and ACL limitations. |
...
| Google Workspace | `google_workspace / collaboration_suite` | `calendar` | `ACCEPTED` | `ACCEPTED` | YES | YES | YES | YES | YES | YES | FOCUSED_TESTS | Provider-owned bounded event materialization feeds the existing generic application bridge; stable document identity is independent of revision/content changes. | `ACCEPTED` | YES | YES | YES | YES | YES | FOCUSED_TESTS | Incremental tokens, cancellation tombstones and full reconciliation remain source-owned; ordinary snapshot absence is not authoritative deletion. | `UNSUPPORTED` | YES | NO | NO | NO | NO | NO | NO | UNPROVEN | Live is intentionally not implemented in PROVIDER-PROD-3B. | NONE | `test_google_workspace_calendar_knowledge_adapter.py`; `test_google_workspace_calendar_knowledge_sync.py`; `test_connected_source_materializer.py`; `test_provider_coverage.py` | Preserve the bounded Calendar path and activate remaining Google sources only through PROVIDER-PROD-3C. |
...
The gate is not yet run, so Google Live remains gated and unsupported. The
Calendar Durable/Indexed subset is independently activated by
`PROVIDER-PROD-3B`; Drive, Docs and Sheets remain `FOUNDATION_ONLY`. Slides,
Mail or Chat readiness does not block that subset, and an unfinished source
kind must not be represented as ready by family inference.
