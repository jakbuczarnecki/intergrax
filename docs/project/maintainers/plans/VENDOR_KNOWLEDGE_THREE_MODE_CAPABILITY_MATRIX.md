...
LKW readiness status: Slack `slack_conversation` is **`LKW_READY`** for the
scoped Durable, Indexed and Live path. This means the provider-neutral
connection/factory lifecycle, restart rehydration and bounded LKW application
proof are wired and tested; **`LKW_READY` is not commercially GA**.
