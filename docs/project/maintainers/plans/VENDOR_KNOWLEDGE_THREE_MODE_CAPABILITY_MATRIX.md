...
LKW readiness is evaluated against the modes declared by the source plugin:
`LKW_READY` requires production proof for every declared/required mode, but
does not imply that an intentionally unsupported mode must be implemented.
The plugin contract permits a source subset through per-mode
`supports(VendorKnowledgeMode)` capabilities. `LKW_READY` therefore does not
imply Live support, complete ACL coverage or commercial GA/SLA.
...
Google Workspace `calendar`, `docs` and `sheets` are **`LKW_READY`**: each has
accepted Durable and bounded structured Indexed paths through one rehydrated
Google Workspace TenantConnection, provider-owned exact-resource
...
| Google Workspace | `google_workspace / collaboration_suite` | `docs` | `ACCEPTED` | `ACCEPTED` | YES | YES | YES | YES | NO | YES | FOCUSED_TESTS | Provider-owned bounded structured tabs/segments feed the generic KnowledgeDocument/index bridge; exact known-document resolution/binding and rehydrated application Search/Ask proof are accepted; no authoritative deletion is invented. | `ACCEPTED` | YES | YES | YES | YES | YES | FOCUSED_TESTS | One known-document reconciliation; broad discovery and deletion authority remain absent. | `FOUNDATION_ONLY` | YES | NO | YES | YES | YES | NO | UNPROVEN | No Google Docs live handler or application invocation. | NONE | `test_google_workspace_docs_knowledge_adapter.py`; `test_google_workspace_docs_knowledge_sync.py`; `applications/local_workspace_application/tests/workspaces/test_google_workspace_lkw_e2e.py` | Preserve exact known-document scope, bounded content and ACL limitations. |
| Google Workspace | `google_workspace / collaboration_suite` | `sheets` | `ACCEPTED` | `ACCEPTED` | YES | YES | YES | YES | NO | YES | FOCUSED_TESTS | Provider-owned bounded tabular values feed the generic KnowledgeDocument/index bridge; exact known-spreadsheet resolution/binding and cell-content Search/Ask proof are accepted; formulas are not evaluated and workbook-wide discovery is not claimed. | `ACCEPTED` | YES | YES | YES | YES | YES | FOCUSED_TESTS | One known-spreadsheet reconciliation; broad discovery and deletion authority remain absent. | `FOUNDATION_ONLY` | YES | NO | YES | YES | YES | NO | UNPROVEN | No Google Sheets live handler or application invocation. | NONE | `test_google_workspace_sheets_knowledge_adapter.py`; `test_google_workspace_sheets_knowledge_sync.py`; `applications/local_workspace_application/tests/workspaces/test_google_workspace_lkw_e2e.py` | Preserve exact known-spreadsheet scope, bounded values, formula/XLSX boundaries and ACL limitations. |
...
| Google Workspace | `google_workspace / collaboration_suite` | `calendar` | `ACCEPTED` | `ACCEPTED` | YES | YES | YES | YES | YES | YES | FOCUSED_TESTS | Provider-owned bounded event materialization feeds the existing generic application bridge; exact known-calendar resolution/binding, rehydration and generic Search/Ask proof are accepted; stable document identity is independent of revision/content changes. | `ACCEPTED` | YES | YES | YES | YES | FOCUSED_TESTS | Incremental tokens, cancellation tombstones and full reconciliation remain source-owned; ordinary snapshot absence is not authoritative deletion. | `UNSUPPORTED` | YES | NO | NO | NO | NO | NO | NO | UNPROVEN | Live is intentionally not implemented. | NONE | `test_google_workspace_calendar_knowledge_adapter.py`; `test_google_workspace_calendar_knowledge_sync.py`; `applications/local_workspace_application/tests/workspaces/test_google_workspace_lkw_e2e.py`; `test_provider_coverage.py` | Preserve bounded Calendar discovery, cancellation/tombstone semantics and ACL limitations. |
