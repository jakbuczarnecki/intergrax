...
| **EBH-2F-R2** | Integration Plugin Contract Typing & Registration Purity | Independent parent recertification wykryła residual semantic `Any` w publicznym `resolve_typed.resolve_contract()` boundary. | **[ ] BLOCKED** |
| **EBH-2F-R2-R2** | Generic Integration Resolution Helper Strong Typing Closure | Usunięcie ostatniego semantic `Any` z generic integration resolution helpera (`resolve_contract` → `CategoryIntegrationInstance` / `T`). | **[ ] CURRENT** |
...
| EBH-2F-R2-R2 | — | CURRENT — generic integration resolution helper strong typing closure; Cursor AI evidence contribution pending independent exact-SHA audit. |
