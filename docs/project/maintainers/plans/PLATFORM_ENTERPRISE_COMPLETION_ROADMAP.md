...
| **EBH-2F-R2-R1-R1-R1-R1** | Category Integration Result Contract Typing Integrity | Profile-aware APIs (`instance_for_category`, `resolve_from_profile`) deklarują `CategoryIntegrationInstance`; independent audit (`77054d545143b86a2fb3082448919adfcc1e3a5a`) wykrył zbyt szeroki catalog-only `resolve()` → `CategoryIntegrationInstance` mimo `IntegrationFactory` → `PlatformIntegrationContract`. Parent BLOCKED do domknięcia EBH-2F-R2-R1-R1-R1-R1-R1. | **[ ] BLOCKED** |
| **EBH-2F-R2-R1-R1-R1-R1-R1** | Catalog vs Profile Resolution Result Precision | Catalog-only `resolve()` → `PlatformIntegrationContract`; profile-aware `resolve_from_profile()` / `instance_for_category()` zachowują `CategoryIntegrationInstance`; `IntegrationFactory` pozostaje catalog-only; mechanical gates + ownership audit aliasu. | **[ ] CURRENT** |
...
| EBH-2F-R2-R1-R1-R1-R1-R1 | — | CURRENT — catalog vs profile resolution result precision (`resolve` → `PlatformIntegrationContract`; profile surfaces → `CategoryIntegrationInstance`). |
