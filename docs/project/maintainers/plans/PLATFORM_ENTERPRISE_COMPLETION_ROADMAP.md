...
| **EBH-2F-R2-R1-R1-R1** | DI-only Category Contract Resolution Integrity | Jeden canonical category-contract resolver obsługuje registry-backed i DI-only (`external_work` → `ExternalWorkIntegration`) bez local exceptions w `IntegrationProfile`. | **[ ] CURRENT** |
...
| EBH-2F-R2-R1-R1-R1 | — | CURRENT — unified category contract resolution for registry-backed and DI-only categories. |
