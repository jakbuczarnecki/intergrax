...
| EBH-2F | Integrations & Hosting Boundary Hardening | Certyfikacja integrations/hosting: provider-neutral contracts, composition ownership, vendor independence, replaceability, brak host/runtime bypassów i concrete coupling. Parent pozostaje otwarty do finalnej recertyfikacji po zamknięciu host-execution children. | [ ] BLOCKED |
| **EBH-2F-R1** | Host Execution Boundary Decoupling | Finalna recertyfikacja childa po niezależnym zamknięciu R1-R1. Zweryfikować pełny efekt migracji production Tier-3 na canonical `HostTaskExecutionPort` / `runtime.execution`, brak application/shared concrete Nexus execution coupling, exactly-one materialization owner, strong typing, brak dual path i zachowanie Execution/Governance semantics. | **[ ] CURRENT** |
...
| EBH-2F-R1 | `fd718b7f969a800cf459c0c70aa3e458d479868a` | Independent audit rejected R1 closure. Production Tier-3 migration to `runtime.execution` is directionally correct; R1-R1 has now closed the identified concrete Nexus/weak-typing blocker. R1 is CURRENT for final recertification of the complete child outcome. |
| EBH-2F-R1-R1 | `c3d5e377db989f4d7c3112f65fce552b88ee360a` | Independently audited on exact GitHub SHA. `harness_host_task_execution_wiring.py` is governance-only and contains no `NexusLoop`; canonical `EffectiveProfileRevisionAdmissionPort` is restored; Nexus-backed convenience builders moved to `tests/fixtures`; production Tier-3 continues to consume `runtime.execution`; architecture gates protect raw Nexus leakage and weak revision-admission typing. One SQLite bootstrap failure remains evidence-classified outside R1-R1 scope and must be rechecked during parent recertification. |
