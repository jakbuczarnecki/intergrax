...
| **EBH-2G** | RAG Contract Boundary Hardening | Certyfikacja retrieval/search/reranking/storage jako jednego pluginowalnego subsystemu z canonical contracts, jednym ownership i wymiennymi backendami. | **[ ] BLOCKED** (child **EBH-2G-R1**) |
| **EBH-2G-R1** | Retrieval Contract Typing, ABI Integrity & Qualification Coverage | Usunięcie semantic `Any` z composition seam, jeden jawny `RetrievalHit` candidate ABI, targeted pyright na retrieval/tool surfaces, mechanical regression gates i reconciled `rag-guard` ownership. | **[ ] CURRENT** |
