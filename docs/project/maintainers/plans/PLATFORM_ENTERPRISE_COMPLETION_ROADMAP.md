...
| EBH-2H | Memory Contract Boundary Hardening | Certyfikacja Memory: exactly-one ownership, canonical contracts, backend replaceability, brak bocznych persistence/context paths. | **[x] CLOSED** (independent exact-SHA audit accepted at `86fafaa24ccdfb47faba44ddcec1b6eaec5bf195`) |
| EBH-2I | Final EBH-2 Rescan | Ponowny przekrojowy audit wszystkich subsystem boundaries po lokalnych hardeningach. Sprawdzenie, czy poprawki nie stworzyły nowych cross-layer zależności, bypassów lub duplicated ownership. | **[ ] CURRENT** |
