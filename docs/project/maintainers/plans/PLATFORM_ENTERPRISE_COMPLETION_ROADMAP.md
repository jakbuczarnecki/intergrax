...
| **EBH-2F-R2-R1** | Integration Resolution & Contract Spec Typing Closure | Strong typing całego flow contract spec → catalog factory → registration → resolution → `PlatformIntegrationContract`; gates na `factory.py` + `contract_spec.py` bez drugiego resolvera. | **[ ] CURRENT** |
...
| EBH-2F-R2-R1 | — | CURRENT — integration resolution & contract spec typing closure (audit blocker at `7611cc8b352c29af38c964e70f675967d2e959cd`). |
