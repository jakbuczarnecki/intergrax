...
**Last updated:** 2026-08-16 — **TRACE-BITEMP-ARCH-SYNC-R3** revision watermark semantics + serialization decision boundary · **TRACE-1B** / **TRACE-1B-HOS-FIX** Done / Closed (`dd4cd73598652119010502bfd0f4ff06d4db0ed8`) · **TRACE-1C** Planned / In Review · **TRACE-1B-ARCH-HOS** Done (`1a3dfa9244a052bd13417562dccc104048d0a492`) · **TRACE-BITEMP-ARCH-SYNC-R2** deterministic correction ordering · pre-production clean-cut policy.
...
**Design:** **TRACE-1** identity + journal + as-of + bitemporal target canon — **Done** (TRACE-ARCH-SYNC-1, TRACE-BITEMP-ARCH-SYNC, TRACE-BITEMP-ARCH-SYNC-R2, TRACE-BITEMP-ARCH-SYNC-R3, 2026-08-16)  
...
**Design:** **Done** (TRACE-BITEMP-ARCH-SYNC, TRACE-BITEMP-ARCH-SYNC-R2, TRACE-BITEMP-ARCH-SYNC-R3, 2026-08-16) · **Implementation:** **Planned** — no runtime code yet.
...
| ID | Priority | Status | Goal | Dependency | Acceptance (summary) |
|----|----------|--------|------|------------|----------------------|
| **TRACE-BITEMP-1** | P1 | Planned | Canonical temporal semantics and contracts **only** (no storage): typed temporal basis; valid-time and system-time semantics; interval/boundary rules; **deterministic knowledge/revision ordering** — exact typed revision-position contract; exact watermark contract; acceptance semantics; authoritative ordering scope; serialization authority; concurrency behavior; uniqueness; monotonicity; idempotency; failure atomicity; relationship to system time; wall-clock → watermark resolution; relationship to `revision_id` / `supersedes`; partition/scoping semantics. **MUST** compare viable serialization strategies (single writer, sequencer, storage-native sequence, transactional allocation, scoped sequencer, optimistic concurrency / CAS, or equivalent) **before** selecting one. Ordering/inclusivity for temporal axes | TRACE-1C; TRACE-ASOF-1 | Identity and execution boundary stable; correction ordering is not timestamp-only; `supersedes` does not define total ordering; serialization mechanism and ordering scope selected only after explicit trade-off against the architecture serialization contract; watermark contract defined; wall-clock → watermark resolution specified; no storage implementation |
| **TRACE-BITEMP-2** | P1 | Planned | Bitemporal revision/fact persistence **implementing the TRACE-BITEMP-1 contract**: revision acceptance and authoritative position persisted consistently; no accepted revision without authoritative ordering identity; no ambiguous partial acceptance; idempotent retries; deterministic concurrent correction acceptance; repository can query by revision position / watermark; correction ordering reconstructable without timestamp heuristics; preserves prior system-time belief; additive correction/backdating/future effectiveness and causal lineage; no destructive overwrite; explicit provenance linkage. Serialization implementation belongs here **only after** TRACE-BITEMP-1 selects the contract/mechanism. **No** database vendor selection | TRACE-BITEMP-1 | Corrections additive; prior belief preserved; ordering auditable from persistence; no half-accepted revision; retries idempotent; concurrent acceptance deterministic |
| **TRACE-BITEMP-3** | P1 | Planned | Historical execution reconstruction: knowledge state at revision watermark K; wall-clock system-time basis resolved to K; execution state at E; combined E + K + valid/system-time basis where needed. Keep all semantics distinct. Do **not** call E + K + temporal basis “bitemporal state”. Distinguish execution-boundary state, valid-time state, system-time historical belief, knowledge/revision-position/watermark state, and combined historically reproducible execution state; execution ordering and knowledge/revision ordering remain independent | TRACE-ASOF-2; TRACE-BITEMP-2 | Reconstruction modes remain distinct; bitemporal state remains valid-time + system-time only; watermark is not a temporal axis |
| **TRACE-BITEMP-4** | P1 | Planned | Temporal query/audit surface, including: What was the authoritative knowledge watermark at system time S? What revisions were accepted ≤ K? What was known at K? What did execution E operate against using knowledge watermark K? What do we now know was valid for the time associated with E? What was believed to be valid at the time E executed? Plus valid-time as-of; system-time as-of; combined bitemporal basis; correction-order queries (before/after revision position K; authoritative K1 → K2 → K3 order); correction history; provenance to source revisions/events. Wall-clock audit questions must **not** rely on timestamp sorting alone | TRACE-BITEMP-3 | Typed query API; deterministic correction-order and watermark-resolution answers; no vendor lock-in |
| **TRACE-BITEMP-5** | P1 | Planned | Production proof/invariants: **concurrent acceptance** (two concurrent corrections receive deterministic distinct order); **clock skew** (misleading producer timestamps do not affect authoritative order); **idempotent retry** (same acceptance retry does not create duplicate semantic revision); **allocation/persistence failure** (no ambiguous half-accepted revision history); **watermark resolution** (wall-clock system-time query resolves deterministically to the correct authoritative knowledge boundary); **scoped ordering** (proof matches the scope chosen in TRACE-BITEMP-1); **cross-scope behavior** (if ordering is partitioned, composition semantics are deterministic and documented); **repeated historical query** (same E/K/temporal basis returns deterministic equivalent state); plus existing: concurrent corrections on same fact; delayed correction (later acceptance, earlier valid time, prior belief reconstructable); multiple correction chain P1 → P2 → P3 (identity, lineage, ordering position); equal/ambiguous timestamps; execution independence (post-E42 correction does not rewrite E42); backdated correction; future-effective change; historical-belief reconstruction; current-corrected-history reconstruction; no history destruction | TRACE-BITEMP-4 | All required platform proof scenarios pass |

**TRACE-BITEMP-1 decision ownership:** contract/design only. It **MUST** explicitly decide typed revision-position, watermark, acceptance, ordering scope, serialization authority, concurrency, uniqueness, monotonicity, idempotency, failure atomicity, relationship to system time, wall-clock → watermark resolution, relationship to `revision_id` / `supersedes`, and partition/scoping semantics. It **MUST NOT** skip the serialization-strategy comparison. TRACE-BITEMP-2 **MUST NOT** invent the mechanism.
