...
| **TRACE-ASOF-2** | P1 | Planned / In Review | First canonical logical execution projection: `RunExecutionAsOfProjection` from positioned `RuntimeEvent` prefix via pure reducer `project_run_execution_as_of`; read orchestration `reconstruct_run_execution_as_of` + `load_positioned_run_journal_through`; `RunExecutionLifecycleStatus` from `RuntimeEventType` only (no payload parsing); attempt-aware; `HistoricalEventReference` provenance; logical-only (no materialization) | TRACE-ASOF-1; TRACE-BITEMP-1 | Logical projection rebuildable from journal; no new source of truth; prefix completeness verified (limit pagination fail-closed); unknown history fails explicitly |
...
**Design:** **Done** (TRACE-BITEMP-ARCH-SYNC, TRACE-BITEMP-ARCH-SYNC-R2, TRACE-BITEMP-ARCH-SYNC-R3, TRACE-BITEMP-ARCH-SYNC-R4, TRACE-BITEMP-ARCH-SYNC-R5, TRACE-BITEMP-ARCH-SYNC-R6, 2026-08-17) · **TRACE-BITEMP-1** typed contracts **Done / Closed** (`d68c72177403fb634fd4ede2d0252e9814d7adee`) · production persistence **Planned** (TRACE-BITEMP-2).
