...
**CURRENT implementation (descriptive, OBS-REBASE-1 + OBS-BOUNDARY-1 + OBS-CAUSAL-2 + OBS-COVERAGE-1 + OBS-RECONSTRUCTION-1 + OBS-ASOF-REBASE + OBS-BITEMP-REBASE 2026-09-16):** TRACE-1A–1C **Done / Closed**. `RuntimeEvent` contract requires **five-ID** spine including `ExecutionId` (`intergrax/runtime/events/runtime_event.py`). **OBS-ASOF-REBASE** / **OBS-BITEMP-REBASE** **closed** — canonical **E** and **E/K/V/S** composition via `HistoricalReconstructionService`; **TRACE-ASOF-4** / **TRACE-BITEMP-4** remain **conditional** public query surfaces. **OBS-COVERAGE-1** closed **PASS WITH P2 LIMITATIONS** — P1 execution-scoped emit paths certified (gates in `test_obs_coverage_1_certification.py`); **DG-005** cross-topology RuntimeEvent persistence remains **NOT PROVEN**. **OBS-BOUNDARY-1** closed: one authority per layer; factual vs diagnostic split frozen in architecture hubs. **OBS-CAUSAL-2** closed: `RuntimeExecutionRef` + `platform_causal_evidence.v2` require `ExecutionId` on execution-scoped causal targets; background transport identity mints root `ExecutionId` at bootstrap (Execution authority, not Observability). Factual reconstruction: `ExecutionReconstructor` under `intergrax.runtime.observability.reconstruction` + `HistoricalReconstructionService`; **OBS-RECONSTRUCTION-1** closed (DIAG imports neutral reconstruction). Functional evidence contracts under `intergrax.contracts.functional_evidence` — **OBS-FUNCTIONAL-CONTRACTS-1**. Event delivery **ADR-OBS-005** / P1B-R3 **implemented** (see architecture hub maturity).
...
**High-level migration order:** Observability hub [OBS-REBASE-1 SSOT](../../architecture/OBSERVABILITY.md#obs-rebase-1--platform-ssot-frozen-2026-09-13) + Phase OBS roadmap (below).
...
## Phase OBS — Post-core SSOT reconciliation (active roadmap)

**Architecture:** hub [OBS-REBASE-1](../../architecture/OBSERVABILITY.md#obs-rebase-1--platform-ssot-frozen-2026-09-13). **Delivery rule:** one OBS row per PR unless explicitly bundled (boundary freeze).
...
TRACE-BITEMP-3
    K-only provider-independent reconstruction (CLOSED)

        ↓

TRACE-BITEMP-4
    temporal query/audit: Valid Time + System Time, T→K, E+K composition
