...
**Last updated:** 2026-08-16 — **TRACE-1B** / **TRACE-1B-HOS-FIX** Done / Closed (`dd4cd73598652119010502bfd0f4ff06d4db0ed8`) · **TRACE-1C** Planned / In Review · **TRACE-1B-ARCH-HOS** Done (`1a3dfa9244a052bd13417562dccc104048d0a492`) · **TRACE-BITEMP-ARCH-SYNC-R2** deterministic correction ordering · pre-production clean-cut policy.
...
**Design:** **TRACE-1** identity + journal + as-of + bitemporal target canon — **Done** (TRACE-ARCH-SYNC-1, TRACE-BITEMP-ARCH-SYNC, TRACE-BITEMP-ARCH-SYNC-R2, 2026-08-15)  
...
**Design:** **Done** (TRACE-BITEMP-ARCH-SYNC, TRACE-BITEMP-ARCH-SYNC-R2, 2026-08-15) · **Implementation:** **Planned** — no runtime code yet.
...
| ID | Priority | Status | Goal | Dependency | Acceptance (summary) |
|----|----------|--------|------|------------|----------------------|
| **TRACE-BITEMP-1** | P1 | Planned | Canonical temporal semantics and contracts: typed temporal basis; valid-time and system-time semantics; interval/boundary rules; **deterministic knowledge/revision ordering** contract (authoritative position, uniqueness, ordering under concurrent ingestion, relationship to system time and to `revision_id` / `supersedes`, boundary/inclusivity, acceptance into knowledge history); ordering/inclusivity for temporal axes; **no** storage implementation | TRACE-1C; TRACE-ASOF-1 | Identity and execution boundary stable; correction ordering is not timestamp-only; `supersedes` does not define total ordering |
| **TRACE-BITEMP-2** | P1 | Planned | Bitemporal revision/fact persistence: every persisted correction/revision has immutable identity and deterministic knowledge/revision position; preserves prior system-time belief; supports additive correction/backdating/future effectiveness and causal lineage; no destructive overwrite; repository/query contract exposes enough information to reconstruct correction ordering **without timestamp heuristics**; persisted correction ordering is auditable; explicit provenance linkage; **no** database vendor selection | TRACE-BITEMP-1 | Corrections additive; prior belief preserved; ordering auditable from persistence |
| **TRACE-BITEMP-3** | P1 | Planned | Historical execution reconstruction integration: combine `AsOfBoundary` + valid-time basis + system-time basis + knowledge/revision-position state where correction history must be deterministic; distinguish execution-boundary state, valid-time state, system-time historical belief, knowledge/revision-position state, and combined historically reproducible execution state; execution ordering and knowledge/revision ordering remain independent; do **not** label the combined result “bitemporal state” | TRACE-ASOF-2; TRACE-BITEMP-2 | Five semantic reconstruction modes distinct; bitemporal state remains valid-time + system-time only |
| **TRACE-BITEMP-4** | P1 | Planned | Temporal query/audit surface: valid-time as-of; system-time as-of; combined bitemporal basis; correction-order queries (before/after revision position K; authoritative K1 → K2 → K3 order); execution E42 operated-against vs current-corrected valid-at-E42 vs system-belief-at-E42; correction history; provenance to source revisions/events; **no** timestamp-only replay for correction-order questions | TRACE-BITEMP-3 | Typed query API; deterministic correction-order answers; no vendor lock-in |
| **TRACE-BITEMP-5** | P1 | Planned | Production proof/invariants: **clock skew** (misleading wall-clock, deterministic revision order); **concurrent corrections** on same fact; **delayed correction** (later acceptance, earlier valid time, prior belief reconstructable); **multiple correction chain** P1 → P2 → P3 (identity, lineage, ordering position); **equal/ambiguous timestamps**; **execution independence** (post-E42 correction does not rewrite E42); plus existing cases — backdated correction, future-effective change, historical-belief reconstruction, current-corrected-history reconstruction, deterministic repeated query, no history destruction | TRACE-BITEMP-4 | All required platform proof scenarios pass |
