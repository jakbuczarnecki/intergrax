...
| **TRACE-BITEMP-3** | P1 | Done / Closed | Provider-independent deterministic historical knowledge reconstruction at finalized watermark **K** via `RevisionOrderingAuthority.records_through` + typed `KnowledgeRevisionReader` + pure reducer (`knowledge_reconstruction.py`). `KnowledgeRevisionPositionRecord.accepted_revision_id` exposes canonical **K → KnowledgeRevisionId** for `ACCEPTED` positions only; `TERMINAL_NON_COMMITTED` and orphaned physical writes excluded from reconstruction. Combined E+K projection and valid/system-time filtering remain out of scope | TRACE-ASOF-2; TRACE-BITEMP-2 | Reconstruction depends on `RevisionOrderingAuthority` contract only; same finalized K → same result after later appends, restart, orphan discovery; incomplete prefix fails closed; focused proof tests pass |
