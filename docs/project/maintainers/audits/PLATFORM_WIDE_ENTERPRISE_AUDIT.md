...
| P2-003 | `contract_capture` reflection at registration | `integrations/registry/contract_capture.py` | **MIGRATION_REQUIRED** — canonical future provider discovery rules documented in [`INTEGRATIONS.md`](../../architecture/INTEGRATIONS.md#provider-discovery-contract-canonical--p2-003-doc) (P2-003-DOC); P2-003-C **not** started — B1 DATA & PERSISTENCE WAVE **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT**; B1-R1 category-level explicit-spec gate (no `SLUG_CATEGORY` origin check) **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** (44 explicit B1 keys); R2 H-INT-GRAPH regression closure (Neptune, OrientDB, ArangoDB explicit `contract_specs` + bootstrap restoration) **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT**; B2 MESSAGING & COLLABORATION **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** (49 explicit B2 keys); B3 INFRASTRUCTURE & OPERATIONS **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** (52 explicit B3 keys); B4 SECURITY / SPECIALIZED RUNTIME / MEDIA **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** (21 explicit B4 keys); B5 REMAINING TYPED PROVIDERS **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** (24 explicit B5 keys: 12 `search_provider`, 8 `document_parser`, 2 `rerank_provider`, 2 `crm`; ordinary reflective typed = 0; `llm_guardrail` typed-but-deferred 9 slugs via centralized `register_all` + no per-slug `integration.py`; `managed_retrieval` staged explicit `("openai", "managed_retrieval")` pending P2-003-C); P2-003-C must delete `contract_capture` / resolve deferred guardrails |
...
| P2-007 | OpenAI SDK in tool service layer | `tools/providers/openai_vector_store/service.py` | **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** — `ManagedRetrievalBackend` canonicalized in Integrations catalog (`IntegrationCategory.MANAGED_RETRIEVAL`); OpenAI SDK isolated in provider adapter; special `materialization.py` provider selector removed; independent audit pending |
