...
| P2-003 | `contract_capture` reflection at registration | `integrations/registry/contract_capture.py` | **MIGRATION_REQUIRED** — B1 DATA & PERSISTENCE WAVE **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT**; B1-R1 category-level explicit-spec gate (no `SLUG_CATEGORY` origin check) **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** (44 explicit B1 keys; 143 reflective built-ins remain outside B1); R2 H-INT-GRAPH regression closure (Neptune, OrientDB, ArangoDB explicit `contract_specs` + bootstrap restoration) **IMPLEMENTED_PENDING_INDEPENDENT_AUDIT** |
