...
| P2-002 | LLM/embedding outside Integrations catalog | `llm_adapters/`, `rag/embedding/` | **IMPLEMENTATION_IN_PROGRESS — B1 foundation complete** ([ADR_PLATFORM_LLM_EMBEDDING_INTEGRATION_BOUNDARY.md](../architecture/ADR_PLATFORM_LLM_EMBEDDING_INTEGRATION_BOUNDARY.md)): Option C hybrid — LLM Adapters remain dedicated domain; `embedding_provider` category foundation landed (enum, profile slot, typed contract, `PROVIDER_CATEGORY_CONTRACT_REGISTRY`, fail-closed explicit specs); **no RAG provider migration**; `EmbeddingProvider` runtime stays in RAG; provider packages + profile cutover pending B2+. |
