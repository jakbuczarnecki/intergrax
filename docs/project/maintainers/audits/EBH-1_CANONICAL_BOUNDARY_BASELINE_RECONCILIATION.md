...
| COLLABORATIVE_WORK | Workspace, delegation, ContextView composition authority | Execution graph owner | EAC-CON-013, 014 | CE, GE read | CW stores | CONFIRMED* |
...
| MEMORY / RAG / CE / UCL | Per EAC-1 authorities | Cross-absorbing truth | EAC-CON-020…028, 072 | Agents, CE, Nexus consumer | Provider SPIs | CONFIRMED with CL risks |
...
| CONTEXT_VIEW_COMPOSITION_AUTHORITY | CONTEXT_ENGINEERING (+ CW policy inputs) | `context_view_*` | **NO** |
...
Modules mixing authorities @ spot check: **Nexus tool inner governance** (uncommitted WIP) — potential **orchestration-side policy**; requires **EBH-1-R1** review against GE boundary (not certified here).
...
| CE owns ContextView composition contracts | **CONFIRMED**; impl split with Nexus context engine (**CL-EAC1-003**) |
| CW contributes principal-scoped sources | **CONFIRMED** (`collaborative_work/context_view_*`) |
...
|------------------|--------|----------------------|--------|----------|-----------|
| Nexus not on public contract surface | EAC2, RB0 | 3× `contracts` → `runtime.nexus` | **DRIFTED** | HIGH | EBH-3 / EBH-1-R1 |
...
|----|----------|-------|----------|----------|------|------------------|
| EBH-F-H-001 | HIGH | PLATFORM_FOUNDATION + AGENT_CONTRACTS | Public contracts → Nexus | `intergrax/contracts/host_profile_slices.py`, `runtime_cost.py`, `runtime_mapping.py` | Stable Nexus coupling; blocks LangChain-independence / vendor-neutral contracts | **EBH-3** — extract DTOs to `contracts/`; **EBH-1-R1** if urgent |
...
5. **EBH-1-R1** (optional) — Remove Nexus types from the three `intergrax/contracts` modules if ADR approves quick win.
...
```text
CW authority → ContextView source ports → CE composition → Agent/Nexus consumer
...
Memory/RAG/UCL → reference read ports → CE ContextView
```
...
| Result | **EBH-1 — CONDITIONAL / BASELINE ESTABLISHED** |
|--------|-----------------------------------------------|

**Rationale:** Domain and peer-authority taxonomy from EAC **holds** on current `development`. **Material drift** persists on Nexus privacy (public contracts + agents) and **ADR-GOV-01**. No duplicate execution lifecycle owner found. Baseline artifact is fit for EBH-2+ only after acknowledging open HIGH findings.

**Next recommended task:** **EBH-3** (if prioritizing Nexus/agent leaks) or **EBH-2** (contract ownership @ 605 modules) per program table.
