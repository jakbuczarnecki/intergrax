# dispute_analyst agent — Implementation Plan
...
| Field | Value |
|-------|-------|
| Agent id | `dispute_analyst` |
| Class | `DisputeAnalystAgent` |
| Primary capability | `dispute.analyze` |
| Tier | Tier-2 (`agents/dispute_analyst`) |
...
| DISPUTE_ANALYST-3 | Register skills/tools on `contract.py` | Planned | Medium | See `docs/project/architecture/SKILLS.md` |
| DISPUTE_ANALYST-4 | Agent smoke test green | Done | High | `tests/test_dispute_analyst_agent.py` |
| DISPUTE_ANALYST-5 | Mount in Tier-3 host (optional) | Planned | Medium | `AgentBinding.mount(DisputeAnalystAgent, ...)` |
...
```bash
uv run pytest agents/dispute_analyst/tests -q
```
