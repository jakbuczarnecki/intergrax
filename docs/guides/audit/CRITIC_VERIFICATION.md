# Critic and Verification — Domain Layer Audit Instruction
...
# TASK: Deep production audit — Critic and Verification (`CRITIC_VERIFICATION`)

You are an **implementation audit agent** for the Intergrax Harness AI platform.

Perform a **rigorous, evidence-backed audit** of the **Critic and Verification** domain — architecture canon, implementation plan, source code, tests, and CI gates. Compare against production-grade systems in this problem space. Do **not** produce a shallow documentation survey.
...
2. `docs/architecture/CRITIC_VERIFICATION.md` — current architecture canon
3. `docs/plan/CRITIC_VERIFICATION.md` — implementation status and gap registers
...
```text
critic modules, verification layers, eval runners
...
1. Critic invoked at defined runtime points (pre-output, post-step, final).
2. Verdict types and escalation to HITL/policy.
3. Trajectory and rubric evaluation harness.
4. Separation: closeout documentation vs execution depth.
5. Trace of critic decisions and scores.
6. False positive/negative handling and retry semantics.
...
High-volume eval, multi-layer critics, latency impact on user path.
...
Critic profiles per agent/application, custom rule plugins.
...
- `audit-and-fix`: update `docs/plan/CRITIC_VERIFICATION.md` gap rows and `docs/architecture/CRITIC_VERIFICATION.md` audit register if present; **no code changes** unless user requests separately
