# Observability Spine — Domain Layer Audit Instruction
...
# TASK: Deep production audit — Observability Spine (`OBSERVABILITY`)

You are an **implementation audit agent** for the Intergrax Harness AI platform.

Perform a **rigorous, evidence-backed audit** of the **Observability Spine** domain — architecture canon, implementation plan, source code, tests, and CI gates. Compare against production-grade systems in this problem space. Do **not** produce a shallow documentation survey.
...
2. `docs/architecture/OBSERVABILITY.md` — current architecture canon
3. `docs/plan/OBSERVABILITY.md` — implementation status and gap registers
...
6. `docs/guides/AGENT_CREATION_GUIDE.md` **Appendix H (observability mandatory vs optional)** — control-plane wiring
...
```text
intergrax/observability/, journal, trace bus, OTel exporters, event catalog
...
1. Single observability spine — no per-agent private trace DBs.
2. Event catalog completeness for intake → plan → context → tools → LLM → policy → terminal.
3. trace_id/run_id/tenant_id propagation across async boundaries.
4. Redaction policy enforced in production profiles.
5. OTel export optional but canonical journal always populated.
6. ops:* signals, metrics, SLI hooks for SLO monitoring.
7. CI: check_harness_observability_wiring.py and related gates.
...
High event volume, long runs, nested subagents, export backpressure.
...
ObservabilityProfile, sink configuration, optional external APM — not parallel event models.
...
- `audit-and-fix`: update `docs/plan/OBSERVABILITY.md` gap rows and `docs/architecture/OBSERVABILITY.md` audit register if present; **no code changes** unless user requests separately
