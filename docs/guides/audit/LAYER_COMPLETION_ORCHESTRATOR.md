...
**Per-domain LCM canon:** [`LAYER_COMPLETION_MODE.md`](../LAYER_COMPLETION_MODE.md)
...
Use `docs/guides/audit/results/YYYY-MM-DD/progress.json` with `"mode": "layer_completion"`.
...
For each `<DOMAIN>`, execute [`LAYER_COMPLETION_MODE.md`](../LAYER_COMPLETION_MODE.md) Steps 1 → 1A → 2 → 3 → 4 → 5 → 6.
...
Paste `05_closeout_all_domains.txt` + `RESUME: docs/guides/audit/results/YYYY-MM-DD/progress.json`.
