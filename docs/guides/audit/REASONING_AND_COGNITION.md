...
Perform a **rigorous, evidence-backed audit** of the **Reasoning and Cognition** domain — architecture canon, implementation plan, source code, tests, and CI gates. Compare against production-grade systems in this problem space. Do **not** produce a shallow documentation survey.
...
2. `docs/architecture/REASONING_AND_COGNITION.md` — current architecture canon
3. `docs/plan/REASONING_AND_COGNITION.md` — implementation status and gap registers
...
6. `docs/guides/AGENT_CREATION_GUIDE.md` **Appendix I §I.4** — control-plane wiring
...
```text
planners, classifiers, DecisionRecord, cognition runtime steps
...
1. Planning as structured contract — not free-text-only plans.
2. DecisionRecord or equivalent for major choices.
3. Planning strategies: none, deterministic, LLM, graph — explicit and wired.
4. Reasoning separated from tool execution in UAEP steps.
5. Classifier/planner outputs validated against typed schemas.
6. Reasoning failures classified separately from tool/runtime failures.
7. Prompt compilation layers feed cognition inputs.
...
Complex multi-step plans, replanning loops, classifier fan-out.
...
Planning strategy per profile, custom PlannerProtocol implementations.
...
- `audit-and-fix`: update `docs/plan/REASONING_AND_COGNITION.md` gap rows and `docs/architecture/REASONING_AND_COGNITION.md` audit register if present; **no code changes** unless user requests separately
