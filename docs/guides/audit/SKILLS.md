...
Perform a **rigorous, evidence-backed audit** of the **Skill Library** domain — architecture canon, implementation plan, source code, tests, and CI gates. Compare against production-grade systems in this problem space. Do **not** produce a shallow documentation survey.
...
2. `docs/architecture/SKILLS.md` — current architecture canon
3. `docs/plan/SKILLS.md` — implementation status and gap registers
4. `docs/guides/INTEGRAX_HARNESS_AUDIT_MAP.md` — layers 12
...
```text
intergrax/skills/, SkillResolverProtocol, skill→tool resolution
...
1. SkillContract: skill_id, required tools, schemas, risk inheritance.
2. SkillResolver resolves skill_ids → allowed_tools with policy checks.
3. Single registration/bootstrap path; no agent self-registration.
4. Skill plugins and scaffold alignment.
5. Observability on skill resolution and invocation chain.
6. Clear separation: skill (composition) vs tool (atomic operation).
...
Deep skill graphs, many skills per agent, resolution caching.
...
SkillProfile, custom SkillResolverProtocol at Tier-3.
...
- `audit-and-fix`: update `docs/plan/SKILLS.md` gap rows and `docs/architecture/SKILLS.md` audit register if present; **no code changes** unless user requests separately
