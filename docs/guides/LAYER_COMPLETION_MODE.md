...
**Strategic frame:** the Harness is the product; agents are replaceable. Default platform queue is gate maintenance ([`plan/PLATFORM_FOUNDATION.md`](../plan/PLATFORM_FOUNDATION.md) §6.1) unless the operator selects otherwise. Phase K and §6.3 — **do not start** without explicit reprioritization.
...
Significant architecture decisions require an ADR per [`docs/adr/README.md`](../adr/README.md) (or explicit “no ADR needed” in plan/journal).
...
- path under `docs/guides/implementation-journal/entries/YYYY-MM-DD/` (when required).
