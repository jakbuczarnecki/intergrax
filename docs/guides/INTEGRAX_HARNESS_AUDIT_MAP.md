...
**Per-domain copy-paste prompts:** For deep single-domain audits (RAG, Tools, Memory, UAEP, …), use [`../audit/README.md`](../audit/README.md) — 22 prompts aligned 1:1 with `architecture/<DOMAIN>.md` ↔ `plan/<DOMAIN>.md`. **Multi-domain orchestration** (all 22 pairs in one Cursor session): [`../bootstrap/`](../bootstrap/README.md). Use this document for layer map, scoring, and output format; use `../audit/<DOMAIN>.md` for the runnable agent instruction.
