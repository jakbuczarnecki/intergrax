...
```bash
# 1) Repo wiring + gate scripts (not live runtime proof)
uv run intergrax doctor

# 2) Deterministic CORE scenario contracts + report (mock evidence, no network/real LLM)
...
| Surface | Command / gate | Question | HEP-1 semantics |
|---------|----------------|----------|-----------------|
