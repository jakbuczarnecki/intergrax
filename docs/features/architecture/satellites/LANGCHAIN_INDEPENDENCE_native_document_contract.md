...
| `descriptor.identity.parent_remote_id` | `identity.parent_document_id` | optional; maps to lineage |
...
| `parent_document_id` | `None` for source document. Required non-empty for direct chunk or derived document. Must not equal `document_id`. |
...
5. `parent_document_id != document_id` when `parent_document_id` is set.
6. Reserved canonical field names must not appear in `metadata`.
7. Typed field vs `metadata` conflict → validation error (fail-closed).
...
- **Direct fragment:** `parent_document_id` points to immediate parent (source or parent chunk).
- **Root lineage:** `root_document_id` always points to the original source document id.
- Chunkers, loaders, and bridges own ID generation policy; the contract only validates shape and consistency.
...
`str`, `int`, `float`, `bool`, `null`, `list`, `dict` with string keys — recursively JSON-compatible.

### 8.2 Forbidden in metadata

- Class instances, callables, `bytes`
- Credentials, tokens, passwords
- Duplicate of any canonical/reserved field
...
If `metadata` contains a reserved key that duplicates a typed field with a different value → **validation error**. Bridge and serializers must not silently pick one source.
...
| Dump format | Deterministic JSON-compatible dict |
...
| Reserved key in metadata with conflicting typed value | Error |
...
- Identity inconsistency (`parent == self`, `root` mismatch rules) → `ValidationError`
