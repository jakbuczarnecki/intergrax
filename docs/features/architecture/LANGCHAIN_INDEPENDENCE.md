...
**Current active task:** LCI-3A — Native embedding contract and result
...
**LCI-3A decision:** The embedding API accepts `KnowledgeDocument`; `EmbeddingResult` stores native documents and readonly float32 vectors separately. Vectors are never stored in native metadata. Legacy indexing temporarily uses `embed_texts` until LCI-3B.
