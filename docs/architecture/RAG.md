...
**Last architecture audit:** 2026-06-13 (Layer Completion — GraphRAG usage hardening: 3-channel fusion + structured provenance; see §GraphRAG architecture · §Engine depth audit register) · 2026-06-12 (GraphRAG platform G1–G3) · 2026-06-10 (M-RAG-DEPTH)
...
| `INTERGRAX_RAG_GRAPH_STORE` | Backend id — `inmemory` \| `neo4j` \| `memgraph` \| `falkordb` (registry: M-RAG.38) |
...
| GraphRAG backend registry | **Closed M-RAG.38** | `graph/bootstrap/backend_registry.py` — `inmemory` / `neo4j` / `memgraph` / `falkordb` |
