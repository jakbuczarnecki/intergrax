...
Next approved HEP wave: **EVID-COST Cost Evidence** — small evidence packaging path over existing local budget/cost/trace information; not a billing engine, provider pricing system, cloud cost estimator, or real LLM usage metering.  
Highest-ROI remaining evidence path: EVID-COST C2–C5 implementation followed by final operator-path closeout, end-to-end evidence smoke audit, and onboarding documentation. EVID-COST C1 (Mode I / spec) is **Done**. The detailed task count and roadmap live in `HARNESS_EVIDENCE_PACK.md` § Evidence ROI roadmap.  
