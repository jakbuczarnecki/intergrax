...
Table of contents + sections for audit-map layers **3, 28** + registers tied to **Known open gaps**. Skip historical paydown logs unless a gap ID points there.
...
- Otherwise: `IDEAL_HARNESS_AI_ARCHITECTURE.md` — sections for layers **3, 28** only
- `INTEGRAX_HARNESS_AUDIT_MAP.md` — layers **3, 28** + maturity §5 only
...
1. `docs/guides/IDEAL_HARNESS_AI_ARCHITECTURE.md` — **layers 3, 28 only** (see §0)
2. `docs/architecture/TIER3_APPLICATION_ENVIRONMENT.md` — **scoped sections** (see §0)
3. `docs/plan/TIER3_APPLICATION_ENVIRONMENT.md` — **scoped sections only** (see §0) — do **not** load the full file
4. `docs/guides/INTEGRAX_HARNESS_AUDIT_MAP.md` — **layers 3, 28** + §5 maturity
