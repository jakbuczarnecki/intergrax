...
Table of contents + sections for audit-map layers **25 (depth)** + registers tied to **Known open gaps**. Skip historical paydown logs unless a gap ID points there.
...
- Otherwise: `IDEAL_HARNESS_AI_ARCHITECTURE.md` — sections for layers **25 (depth)** only
- `INTEGRAX_HARNESS_AUDIT_MAP.md` — layers **25 (depth)** + maturity §5 only
...
1. `docs/guides/IDEAL_HARNESS_AI_ARCHITECTURE.md` — **layers 25 (depth) only** (see §0)
2. `docs/architecture/CRITIC_VERIFICATION.md` — **scoped sections** (see §0)
3. `docs/plan/CRITIC_VERIFICATION.md` — **scoped sections only** (see §0) — do **not** load the full file
4. `docs/guides/INTEGRAX_HARNESS_AUDIT_MAP.md` — **layers 25 (depth)** + §5 maturity
