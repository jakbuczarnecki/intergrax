...
**Load first:** [`docs/guides/audit_slices/RAG.md`](../guides/audit_slices/RAG.md) — compact slice (layers **14**); replaces bulk IDEAL + AUDIT_MAP + full plan/arch reads.
...
1. **`docs/guides/audit_slices/RAG.md`** — mandatory; follow slice plan/arch/IDEAL scope lines
2. `docs/architecture/RAG.md` — hub read-scope + one `architecture/satellites/` satellite max
3. `docs/plan/RAG.md` — hub + one `plan/satellites/` satellite max
4. `docs/audit/README.md` — shared production Harness checklist
5. `@docs/guides/AGENT_CREATION_GUIDE.md` **Appendix K §K.5** — on demand
...
See **Code entry** in `docs/guides/audit_slices/RAG.md` — then inspect:
