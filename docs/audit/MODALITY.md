...
**Load first:** [`docs/guides/audit_slices/MODALITY.md`](../guides/audit_slices/MODALITY.md) — compact slice (layers **29**); replaces bulk IDEAL + AUDIT_MAP + full plan/arch reads.
...
1. **`docs/guides/audit_slices/MODALITY.md`** — mandatory; follow slice plan/arch/IDEAL scope lines
2. `docs/architecture/MODALITY.md` — hub read-scope + one `architecture/satellites/` satellite max
3. `docs/plan/MODALITY.md` — hub + one `plan/satellites/` satellite max
...
See **Code entry** in `docs/guides/audit_slices/MODALITY.md` — then inspect:
