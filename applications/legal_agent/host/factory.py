...
from legal_agent.serving.fastapi_router import mount_legal_agent_routes
...
from legal_agent.host.wiring import build_legal_agent
...
def create_legal_backend_app(*, settings: Optional[LegalBackendSettings] = None) -> FastAPI:
    ...
    Environment variables are read when ``settings`` is omitted (see :mod:`legal_agent.host.settings`).
