...
"""Construct Tier-2 Legal Agent and dependencies from :class:`~legal_agent.host.settings.LegalBackendSettings`."""

from __future__ import annotations

from legal_agent.config.legal_agent_product_profiles import LegalAgentProductProfile
...
def build_legal_agent(settings: LegalBackendSettings) -> LegalAgent:
    ...
    Tier-2 configuration from :class:`~legal_agent.config.legal_agent_product_profiles.LegalAgentProductProfile`.
    ``production_mode`` stays ``False`` on the agent until host wires governance + ``RuntimeConfig.trace_db_path``
    (see Nexus ``RuntimeContext.build`` requirements for ``production_mode=True``).
    """
    try:
