...
from legal_agent.config.legal_failure_policy import LegalFailurePolicy
from legal_agent.memory.legal_memory_policy import (
    LegalMemoryPolicy,
    LegalMemoryPolicyPresets,
)
from legal_agent.governance.legal_response_governance_port import (
    LegalResponseGovernancePort,
)
from legal_agent.governance.legal_tool_plan_governance_port import (
...
class LegalAgentConfig(BaseModel):
    ...
    Optional SKU defaults: :class:`~legal_agent.config.legal_agent_product_profiles.LegalAgentProductProfile`.
    Failure & degradation contract: :class:`~legal_agent.config.legal_failure_policy.LegalFailurePolicy`.
    """
    ...
    memory_policy: LegalMemoryPolicy = Field(
        ...
        description=(
            ...
            "(build :class:`~legal_agent.memory.legal_memory_policy.LegalMemoryPolicy` "
    ...
    failure_policy: LegalFailurePolicy = Field(
        ...
        description=(
            ...
            "changing SKU guarantees; see :class:`~legal_agent.config.legal_failure_policy.LegalFailurePolicy`."
        ),
    )
