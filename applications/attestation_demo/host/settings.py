...
class AttestationDemoSettings:
    ...
    @classmethod
    def from_env(cls) -> AttestationDemoSettings:
        ...
        prefix = (
            os.getenv("ATTESTATION_DEMO_ROUTE_PREFIX") or "/v1/attestation_demo"
        ).strip() or "/v1/attestation_demo"
        host = (os.getenv("ATTESTATION_DEMO_BACKEND_HOST") or "127.0.0.1").strip()
        port_raw = (os.getenv("ATTESTATION_DEMO_BACKEND_PORT") or "8097").strip()
        include_interactions = (
            os.getenv("ATTESTATION_DEMO_INCLUDE_INTERACTIONS") or "false"
        ).strip().lower() in {"1", "true", "yes", "on"}
        include_scheduler = (
            os.getenv("ATTESTATION_DEMO_INCLUDE_SCHEDULER") or "false"
        ).strip().lower() in {"1", "true", "yes", "on"}
        interaction_prefix = (
            os.getenv("ATTESTATION_DEMO_INTERACTION_ROUTE_PREFIX") or "/v1/interactions"
        ...
        interaction_surface = (
            os.getenv("ATTESTATION_DEMO_INTERACTION_SURFACE") or "auto"
        ).strip().lower() or "auto"
        include_mcp = (os.getenv("ATTESTATION_DEMO_INCLUDE_MCP") or "false").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
        mcp_mount = (os.getenv("ATTESTATION_DEMO_MCP_MOUNT_PATH") or "/mcp").strip() or "/mcp"
        include_task_control = (
            os.getenv("ATTESTATION_DEMO_INCLUDE_TASK_CONTROL") or "true"
        ).strip().lower() not in {"0", "false", "no"}
        include_queue_worker = (
            os.getenv("ATTESTATION_DEMO_INCLUDE_QUEUE_WORKER") or "false"
        ).strip().lower() in {"1", "true", "yes", "on"}
        task_control_prefix = (
            os.getenv("ATTESTATION_DEMO_TASK_CONTROL_ROUTE_PREFIX") or "/v1/tasks"
        ).strip() or "/v1/tasks"
        return cls(
