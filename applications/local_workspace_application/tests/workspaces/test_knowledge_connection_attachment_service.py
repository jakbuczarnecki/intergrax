...
def _attachment_row(**overrides: object) -> WorkspaceConnectionAttachment:
    payload = {
        ...
        "workspace_id": _WORKSPACE,
        "connection_ref": _CONNECTION,
        "safe_display_label": "Primary Connection",
...
def test_successful_attach_applied() -> None:
    ...
    assert result.attachment.safe_display_label == "Primary Connection"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({(_TENANT, _CONNECTION): _safe_connection(administrative_status=TenantConnectionAdministrativeStatus.DISABLED)}, "connection_unavailable"),
        ({(_TENANT, _CONNECTION): _safe_connection(administrative_status=TenantConnectionAdministrativeStatus.REVOKED)}, "connection_unavailable"),
    ],
)
def test_connection_boundary(connections, error_code) -> None:
    service, _, _ = _build_stack(connections=connections)
...
def test_safe_display_label(requested: str | None, error: str | None) -> None:
    ...
    if error is None:
        ...
        expected = requested.strip() if requested is not None else "Primary Connection"
...
def test_handler_stage_inspect_cleanup_and_foreign_ownership() -> None:
    ...
    handler = AttachConnectionMutationHandler()
    mutation = _mutation()
    ...
    handler.stage(repository=repo, mutation=mutation, target_revision=1, intent=intent, now=_NOW)
    inspection = handler.inspect_staged(repository=repo, mutation=mutation)
    assert inspection.state is WorkspaceKnowledgeStageStateV1.COMPLETE_VALID
    assert handler.cleanup_staged(repository=repo, mutation=mutation, inspection=inspection)
    repo.put_knowledge_connection_attachment_version_if_absent(
        _attachment_row(mutation_id="foreign-mutation")
    )
    assert handler.inspect_staged(repository=repo, mutation=mutation).state is WorkspaceKnowledgeStageStateV1.ABSENT
...
def test_handler_cleanup_failure_and_preserves_committed() -> None:
    ...
    assert repo.get_knowledge_connection_attachment_version(
        tenant_id=_TENANT, workspace_id=_WORKSPACE, attachment_id=committed.attachment_id, effective_revision=1
    )
    ...
    m = _mutation()
    assert not handler.cleanup_staged(
        repository=repo2,
        mutation=m,
        inspection=handler.inspect_staged(repository=repo2, mutation=m),
    )
