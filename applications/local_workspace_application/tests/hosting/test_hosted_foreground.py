...
def _supervisor_result(kind: HostedApplicationExitKind) -> HostedApplicationSupervisorResult:
...
def test_run_local_workspace_hosted_application_wiring(
    ...
    monkeypatch.setattr(foreground_module, "build_local_workspace_hosted_profile", _fake_build)
...
def test_main_emits_safe_json(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _fake_run(*, settings: LocalWorkspaceBackendSettings | None = None) -> HostedApplicationSupervisorResult:
        del settings
        return expected
