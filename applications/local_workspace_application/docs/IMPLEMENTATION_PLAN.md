...
**Next planned task:** **LKW.4C** — background ingest enqueue helper. Platform/app/agent/provider boundaries are documented in [`ARCHITECTURE.md`](ARCHITECTURE.md) §8.7 (LKW.4-ARCH-1 closed; LKW.4B closed).
...
**Execution gate:** LKW.4B is closed; LKW.4C may begin. LKW.4C must build an application helper/service only — no worker and no provider-specific code. LKW.4C–LKW.4F depend on documented platform boundaries ([`ARCHITECTURE.md`](ARCHITECTURE.md) §8.7) and must not introduce LKW-specific queue code.
