...
set "PROOF=%SCRIPT_DIR%run-lkw-windows-interaction-proof.py"
set "COMPOSE_CONFIG=%TEMP%\intergrax_lkw_windows_interaction_proof_compose_%RANDOM%%RANDOM%.yml"
...
echo LKW Windows PowerShell interaction platform proof helper (LKW.6C)
echo.

echo Step 1/4: validating Windows, powershell.exe and Docker
if /I not "%OS%"=="Windows_NT" (
    echo proof_result=FAIL
    echo failure_reason=windows_required
    exit /b 1
)
...
if errorlevel 1 (
    echo proof_result=FAIL
    echo failure_reason=windows_powershell_unavailable
    exit /b 1
)
...
echo The live pytest test owns the hosted LKW process and PowerShell adapter.
echo.

echo Step 2/4: validating MongoDB Compose overlay
...
echo.
echo Step 3/4: start MongoDB and Mongo Express and run the live proof with receipt recording
...
echo Filter:
echo   proof_kind = platform_windows_interaction
