...
LOCAL_WORKSPACE_HOSTING_BOUNDARY_COMPONENT_ID = "local_workspace_hosting_boundary"

LOCAL_WORKSPACE_HOSTING_BOUNDARY_COMPONENT_TYPE_ID = (
    "local_workspace_application.hosting.boundary_component.v1"
)

LOCAL_WORKSPACE_BEFORE_READY_HOOK_ID = "lkw_hosted_before_ready_boundary"
...
class _LocalWorkspaceHostingBoundary:
    ...
    async def start(
        ...
        if self._started:
            raise RuntimeError("local workspace hosting boundary already started")
        self._started = True
        self._before_ready_verified = False
    ...
    async def mark_before_ready(
        ...
        if not self._started:
            raise RuntimeError("local workspace hosting boundary is not started")
    ...
    async def health(
        ...
        if (
            self._state is HostedApplicationComponentState.READY
            and self._before_ready_verified
        ):
            return HostedApplicationComponentHealth(
                component_id=self.component_id,
                enabled=True,
